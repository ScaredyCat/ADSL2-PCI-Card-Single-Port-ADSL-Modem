...
/***
 * try_to_wake_up - wake up a process
 *
        ...
        if (unlikely(!spin_trylock(&busiest->lock))) {
                if (busiest < this_rq) {
                        ...
                        if (idle || (this_rq->nr_running > this_rq->prev_nr_running[this_cpu]))
                                ...
                                nr_running = this_rq->prev_nr_running[this_cpu];                } else
                        spin_lock(&busiest->lock);
        }
        return nr_running;
}
...
new_array:
      /* Start searching at priority 0: */ +       idx = 0;
skip_bitmap:
      if (!idx)
 ...
 * wake everything up.  If it's an exclusive wakeup (nr_exclusive == small +ve+  * number) then we wake all the non-exclusive tasks and one exclusive task.
 *
