...
sys_clock_settime(clockid_t which_clock, const struct timespec __user *tp)
{
	...
	/*
	 * This is revolting, but the div causes errors, where he gives
	 * us negative nsecs in the range (-999, -1), to go away so we
	 * miss the error.  Not a problem in 2.6 as there we stay in
	 * nsecs so do_sys_settimeofday() properly catches them.
	 */
	if ((unsigned long)new_tp.tv_nsec >= NSEC_PER_SEC)
		return -EINVAL;

	new_tv.tv_usec = new_tp.tv_nsec / NSEC_PER_USEC;
	new_tv.tv_sec = new_tp.tv_sec;
	return do_sys_settimeofday(&new_tv, NULL);
}
