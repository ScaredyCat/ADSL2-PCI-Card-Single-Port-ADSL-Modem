...
#ifdef CONFIG_DEVFS_FS
/** handles for Dev FS */
static devfs_handle_t tcrypt_devfs_handle[2];
#endif
int tcrypt_major=43;
		      ...
		      unsigned int tcount)
{
	...
	struct hash_testvec *hash_tv = NULL;
	unsigned int tsize;
			      ...
			      int blen)
{
	...
	//local_bh_disable();  //because dma uses interrupt in the moment
	//local_irq_disable();
...
out:
	//local_irq_enable();  //because dma uses interrupt in the moment
	//local_bh_enable();
...
static void do_test(int test_mode)
{
	switch (test_mode) {
	...
	case 31:
		test_cipher ("aes", MODE_ECB, ENCRYPT, aes_enc_tv_template2, AES_ENC_TEST_VECTORS2);
		test_cipher ("aes", MODE_ECB, DECRYPT, aes_dec_tv_template2, AES_DEC_TEST_VECTORS2);
		break;	
	case 32:
		test_cipher ("des", MODE_ECB, ENCRYPT, des_enc_tv_template2, DES_ENC_TEST_VECTORS2);
		test_cipher ("des", MODE_ECB, DECRYPT, des_dec_tv_template2, DES_DEC_TEST_VECTORS2);
		break;	
...
static int tcrypt_release(struct inode *inode, struct file *file)
{
       MOD_DEC_USE_COUNT;
       return 0;
}
...
static int tcrypt_ioctl(struct inode *inode, struct file *file,
...
static struct file_operations tcrypt_fops = {
	owner:		THIS_MODULE,
	open:		tcrypt_open,
        release:        tcrypt_release,
        ioctl:          tcrypt_ioctl,
};
...
#ifdef CONFIG_DEVFS_FS
   memset (&(tcrypt_devfs_handle), 0x00, sizeof(tcrypt_devfs_handle));
   for(i=0;i<2;i++)
   {
         sprintf (buf, "tcrypt%d",i);
         if (( tcrypt_devfs_handle[i]  = devfs_register( NULL,
                                                    buf,
                                                    DEVFS_FL_DEFAULT,
                                                    tcrypt_major,
                                                    i,
...
#else
    result = register_chrdev(tcrypt_major,"tcrypt",&tcrypt_fops);
    if (result < 0) {
...
{
    int i;
    TRACE("tcrypt cleanup\n");
#ifdef CONFIG_DEVFS_FS
    for(i=0;i<2;i++)
    devfs_unregister(tcrypt_devfs_handle[i]);
#else
    unregister_chrdev(tcrypt_major, "tcrypt");
#endif
