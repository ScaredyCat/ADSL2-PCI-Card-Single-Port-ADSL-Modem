 ...
 */
	...
	kmem_cache_init();
	sti();
	printk("Syam before calibrate_delay\n");
	calibrate_delay();
	printk("Syam after calibrate_delay\n");
#if defined(CONFIG_MIPS) && defined(CONFIG_NEW_TIME_C)
...
#endif
#ifdef CONFIG_DANUBE
	extern int danube_dma_init(void);
	danube_dma_init();
#endif
#ifdef CONFIG_AMAZON_SE
	extern int amazon_se_dma_init(void);
	amazon_se_dma_init();
#endif
