...
/* Allocates basic system resources for this USB host controller, 
 * and then invokes the start() method for the HCD. */

int usb_hcd_probe (const struct hc_driver* driver, 
		int irq, unsigned long resource, unsigned long len)
{
	void							*base = NULL;
	struct usb_bus		*bus;
	struct usb_hcd		*hcd;
	...
	if (!irq) {
		err ("irq parameter is 0, expected >0.");
		return -ENODEV;
	}
...
clean_1:
			release_mem_region (resource, len);
			err ("init %s failed, %d", driver->description, retval);
			return retval;
		}

	} else {
		// XXX: we suppose, the registers are memory mapped
		BUG();
	}
...
clean_2:
		if (driver->flags & HCD_MEMORY) {
			...
			release_region (resource, len);
			err ("init %s fail, %d", driver->description, retval);
			return retval;
		}
	}
	...
	bufp = __irq_itoa(irq);
#endif
	if (request_irq (irq, hcd_irq, SA_SHIRQ, hcd->description, hcd) != 0) {
		err ("request interrupt %s failed", bufp);
		retval = -EBUSY;
clean_3:
		driver->hcd_free (hcd);
	...
	hcd->irq = irq;
	hcd->iomem_start = resource;
	hcd->iomem_len = len;

	hcd->regs = base;
	printk (KERN_INFO "%s: irq %s, %s %p\n",
			hcd->description, bufp,
			(driver->flags & HCD_MEMORY) ? "addr " : "io base", base);
	...
	hcd->bus = bus;
	bus->bus_name = (char*)driver->description;
	hcd->product_desc = (char*)driver->description;
	bus->hcpriv = (void *) hcd;
...
/* Reverses the effect of usb_hcd_probe(), first invoking
 * the HCD's stop() method. */

void usb_hcd_remove (struct usb_hcd	*hcd)
{
	struct usb_device	*hub;
	...
	if (hcd->driver->flags & HCD_MEMORY) {
		// XXX: ioremap problem on DANUBE...
		// iounmap (hcd->regs);
		release_mem_region (hcd->iomem_start, hcd->iomem_len);
	} else {
		// XXX: we suppose, the registers was memory mapped
		BUG();
	}
...
static int hcd_submit_urb (struct urb *urb)
{
	...
	if (urb->dev == hcd->bus->root_hub) {
		...
		if(hcd->driver->flags & HCD_NONPCI) {
			if (usb_pipecontrol (urb->pipe)) {
				unsigned long addr = (unsigned long) urb->setup_packet;
				dma_cache_wback_inv(addr, sizeof(struct usb_ctrlrequest));
				urb->setup_dma = __pa(urb->setup_packet);		
			}

			if (urb->transfer_buffer_length != 0) {
				unsigned long addr = (unsigned long) urb->transfer_buffer;
				dma_cache_wback_inv(addr, urb->transfer_buffer_length);
				urb->transfer_dma = __pa(urb->transfer_buffer);		
			}
		} else {
			if (usb_pipecontrol (urb->pipe))
...
void usb_hcd_giveback_urb (struct usb_hcd *hcd, struct urb *urb, struct pt_regs *regs)
{
	...
	if(hcd->driver->flags & HCD_NONPCI) {
	    ...
	    {
	        unsigned long addr = (unsigned long) __va(urb->setup_dma);
	        dma_cache_wback_inv(addr, sizeof (struct usb_ctrlrequest));
	    }
		if ((urb->transfer_buffer_length != 0) && !is_root_hub_operation) {
				unsigned long addr = (unsigned long) __va(urb->transfer_dma);
				dma_cache_wback_inv(addr, urb->transfer_buffer_length);
			}
	} else {	
