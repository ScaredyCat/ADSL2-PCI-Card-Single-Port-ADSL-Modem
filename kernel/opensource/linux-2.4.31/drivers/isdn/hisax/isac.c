...
#define ARCOFI_USE 0 //HANK2007/1/5 05:47¤U¤È
...
#else
static char *ISACVer[] __devinitdata =
{"2086/2186 V1.4", "2085 B1", "2085 B2",
 "2085 V2.3"};
#endif
...
#ifdef CONFIG_HISAX_PEB3086		
	cs->writeisac(cs, ISAC_CIX0, (command << 4) | 3);
#else	
	...
	if ((cs->rcvidx + count) >= MAX_DFRAME_LEN_L1) {
		if (cs->debug & L1_DEB_WARN)
			debugl1(cs, "isac_empty_fifo overrun %d",
				cs->rcvidx + count);
#ifdef CONFIG_HISAX_PEB3086						
		cs->writeisac(cs, ISAC_CMDRD, 0x80);
#else
...
#ifdef CONFIG_HISAX_PEB3086				
	cs->writeisac(cs, ISAC_CMDRD, more ? 0x8 : 0xa);
#else
...
isac_interrupt(struct IsdnCardState *cs, u_char val)
{
	...
	if (val & 0x80) {	/* RME */
#ifdef CONFIG_HISAX_PEB3086						
		exval = cs->readisac(cs, ISAC_RSTAD);
#else
...
#ifdef CONFIG_HISAX_PEB3086							
			count = cs->readisac(cs, ISAC_RBCLD) & 0x1f;
#else
	...
	if (val & 0x20) {	/* RFO */
		/* never */
		if (cs->debug & L1_DEB_WARN)
			debugl1(cs, "ISAC Receive Frame Overflow interrupt");
	}
#endif	
	if (val & 0x10) {	/* XPR */
...
#ifdef CONFIG_HISAX_PEB3086	/* CIC */
	if (val & 0x08) {
		debugl1(cs, "ISAC XMR");
	...
	if(val & ISTA_CIC) {
		...
		if (exval & CIR0_CIC0) {
			cs->dc.isac.ph_state = (exval >> 4) & 0xf;
			if (cs->debug & L1_DEB_ISAC)
	...
	if (val & ISTA_ST) {	/* ST */
		exval = cs->readisac(cs, ISAC_STI);
		if (cs->debug & L1_DEB_ISAC)
			debugl1(cs, "ISAC STI interrupt %02X", exval);
	}
	if (val & ISTA_TRAN) {	/* TRAN */
		exval = cs->readisac(cs, ISAC_ISTATR);
		if (cs->debug & L1_DEB_ISAC)
			debugl1(cs, "ISAC ISTATR interrupt %02X", exval);
	}
	if (val & ISTA_MOS) {	/* MOS */
		exval = cs->readisac(cs, ISAC_MOSR);
		if (cs->debug & L1_DEB_ISAC)
			debugl1(cs, "ISAC MOSR interrupt %02X", exval);
	}
#else /* CISQ */
	if (val & 0x04) {	
		exval = cs->readisac(cs, ISAC_CIR0);
...
dbusy_timer_handler(struct IsdnCardState *cs)
{
	...
	if (test_bit(FLG_DBUSY_TIMER, &cs->HW_Flags)) {
		rbch = cs->readisac(cs, ISAC_RBCHD);
		star = cs->readisac(cs, ISAC_STARD);
		if (cs->debug) 
			debugl1(cs, "D-Channel Busy RBCHD %02x STARD %02x",
				rbch, star);
		if (star & STARD_XACI) { /* D-Channel Busy */
			...
			cs->writeisac(cs, ISAC_CMDRD, CMDRD_XRES); /* Transmitter reset */
...
initisac(struct IsdnCardState *cs)
{
	...
	init_timer(&cs->dbusytimer);
#ifdef CONFIG_HISAX_PEB3086  		
  	cs->writeisac(cs, ISAC_MASKD, 0xff);
  	cs->dc.isac.mocr = 0xa0;
		cs->writeisac(cs, ISAC_IOM_CR, IOM_CR_SPU);
	cs->writeisac(cs, ISAC_MODED, 0xc9);
	cs->writeisac(cs, ISAC_CMDRD, CMDRD_RMC|CMDRD_XRES);
		ph_command(cs, ISAC_CMD_RS);
	cs->writeisac(cs, ISAC_MASK, 0x0);	
	cs->writeisac(cs, ISAC_MASKD, 0x0);
#else
...
clear_pending_isac_ints(struct IsdnCardState *cs)
{
	int val, eval;
#ifdef CONFIG_HISAX_PEB3086
	val = cs->readisac(cs, ISAC_STARD);
	debugl1(cs, "ISAC STARD %x", val);
	val = cs->readisac(cs, ISAC_MODED);
	debugl1(cs, "ISAC MODED %x", val);
	val = cs->readisac(cs, ISAC_ISTAD);
	debugl1(cs, "ISAC ISTAD %x", val);
	val = cs->readisac(cs, ISAC_CIR0);
	debugl1(cs, "ISAC CIR0 %x", val);
	cs->dc.isac.ph_state = (val >> 4) & 0xf;
	isac_sched_event(cs, D_L1STATECHANGE);
	/* Disable all IRQ */
	cs->writeisac(cs, ISAC_MASK, 0xFF);
#else
