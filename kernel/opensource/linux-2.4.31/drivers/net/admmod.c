...
** HISTORY
** $Date        $Author         $Comment
**  1 SEP 2004  Joe Lin         Initiate Version
**  2 JUN 2005  Mars Lin        Add VLAN support.
** 28 JUL 2005  Mars Lin        Support MDIO/EEPROM config mode.
...
#if 0 //060629:henryhsu Reset ADM6996i to default setting for Danube.
...
//  printk ("\n %0x \n",val1);
    ...
    //605301:fchang.removed if ((adm6996_mode==adm6996lc)||(adm6996_mode==adm6996i)){
	// ifx_sw_write(0x29,0xc000);
        // ifx_sw_write(0x30,0x0985);

	//605301:fchang.removed.start We just use the default reset values in 6996I chipid 71023
	//ifx_sw_read(0xa0, &val);
	//if (val == 0x1021) // for both 6996LC and 6996I, only AB version need the patch
	//	ifx_sw_write(0x29, 0x9000);
	//ifx_sw_write(0x30,0x0985);
	//605301:fchang.removed.end
    //605301:fchang.removed }
...
//060620:henryhsu modify for vlan
   ...
   */
//060628:henryhsu : When we want to enable or disable VLAN function , reset ADM6996i
#if 0   //Use RCU to do hardware reset
	*DANUBE_RCU_RST_REQ |= (1<<0);
	udelay(100000);
	*DANUBE_RCU_RST_REQ &= (~(1<<0));
	...
	cleanup_adm6996_module();
  #else
        //  xuliang: cleanup_adm6996_module can't be called directly
        printk("Free 6996 device driver... \n");
        unregister_chrdev(69, "adm6996");
  #endif
	udelay(200000);
