...
extern void HwNatAddPPPoESID(unsigned short num, unsigned char *mac);
#endif
#endif
...
/* [ fchang: added, to keep the value in a global variable gPppoeSid */
__u16		gPppoeSid = 0;
unsigned	char		gPppoeDestMac[6];
struct		net_device	*gPppoeDev = NULL;
/* ] end of change */
...
int pppoe_release(struct socket *sock)
{
	...
	/* [ fchang: added to refresh the global variables for iproxyd */
	gPppoeDev = NULL;
	gPppoeSid = 0;
	gPppoeDestMac[0] = 0;
	gPppoeDestMac[1] = 0;
		  ...
		  int sockaddr_len, int flags)
{
	...
	if (sp->sa_addr.pppoe.sid != 0) {
		...
		/* [ fchang: added to refresh the global variables for iproxyd */
		gPppoeDev = po->pppoe_dev;
		gPppoeSid = sp->sa_addr.pppoe.sid;
		gPppoeDestMac[0] = po->pppoe_pa.remote[0];
		gPppoeDestMac[1] = po->pppoe_pa.remote[1];
		gPppoeDestMac[2] = po->pppoe_pa.remote[2];
		gPppoeDestMac[3] = po->pppoe_pa.remote[3];
		gPppoeDestMac[4] = po->pppoe_pa.remote[4];
		gPppoeDestMac[5] = po->pppoe_pa.remote[5];
		/* Debug Printing*/
		/*printk("pppoe.c: pppoeDev is:%s SID is:%08X DestMac is:%02X:%02X:%02X:%02X:%02X:%02X\n",
			gPppoeDev->name,
			gPppoeSid,
			gPppoeDestMac[0],
			gPppoeDestMac[1],
...
#if 1 /* [ Additional safety check - Ritesh, 16Feb,2005 */
	if (dev == NULL || !(dev->flags & IFF_UP)) {
		error = -ENOTCONN;
		printk("pppoe_sendmsg() - trying to send with device down/null!\n");
		goto end;
	}
#endif /* ] */
...
#ifdef CONFIG_IP_NF_TURBONAT
#ifdef CONFIG_ADM8668_HW_NAT
        HwNatAddPPPoESID(ntohs(sk->num), &sk->protinfo.pppox->pppoe_pa.remote[0]);
#endif
#endif
...
/* [ fchang: added, to keep the value in a global variable gPppoeSid */
EXPORT_SYMBOL(gPppoeSid);
EXPORT_SYMBOL(gPppoeDestMac);
EXPORT_SYMBOL(gPppoeDev);
/* ] end of change */
