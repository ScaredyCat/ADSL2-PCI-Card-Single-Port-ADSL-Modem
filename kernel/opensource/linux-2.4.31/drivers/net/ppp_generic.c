...
//165060:henryhsu 2006/06/09 Add ppp extension
...
#if defined(CONFIG_DANUBE_PPA) && defined(CONFIG_PPPOE)
  #include <net/sock.h>
#endif

#if defined(CONFIG_PPP_IFX_IDLETIME_EXTENSION) || defined(CONFIG_PPP_IFX_IDLETIME_EXTENSION_MODULE)
	int (*ifx_nfext_ppp_idletime_xmit)(void *, struct sk_buff *, int) = NULL;
	EXPORT_SYMBOL(ifx_nfext_ppp_idletime_xmit);
	int (*ifx_nfext_ppp_idletime_recv)(void *, struct sk_buff *, int) = NULL;
	EXPORT_SYMBOL(ifx_nfext_ppp_idletime_recv);
#endif
...
#if defined(CONFIG_PPP_IFX_IDLETIME_EXTENSION) || defined(CONFIG_PPP_IFX_IDLETIME_EXTENSION_MODULE)
			if (ifx_nfext_ppp_idletime_xmit != NULL)
			{
				if (ifx_nfext_ppp_idletime_xmit((void *)ppp, skb, 0) == 1)
...
ppp_receive_nonmp_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (npi < 0) {
		...
		*skb_push(skb, 2) = 0;
		...
		{
#if defined(CONFIG_PPP_IFX_IDLETIME_EXTENSION) || defined(CONFIG_PPP_IFX_IDLETIME_EXTENSION_MODULE)
			if (ifx_nfext_ppp_idletime_recv != NULL)
			{
				if (ifx_nfext_ppp_idletime_recv((void *)ppp, skb, 1) == 1)
...
#if defined(CONFIG_DANUBE_PPA) && defined(CONFIG_PPPOE)
int32_t pppoe_get_pppoe_addr(struct net_device *dev, struct pppoe_addr *pa)
{
    /* XXX: needs to be in drivers/net/pppoe.c */
	struct ppp *ppp = (struct ppp *)dev->priv;
	struct list_head *list;
	struct channel *pch;
	struct sock *sock;
    ...
    pch = list_entry(list->next, struct channel, clist);

    spin_lock_bh(&pch->downl);
    if ( pch->chan )
    {
        sock = (struct sock *)pch->chan;
        if ( sock->protinfo.pppox )
        {
    		*pa = (sock->protinfo.pppox->proto.pppoe.pa);
            ret = 0;
        }
    }
    else
...
__u16 ppa_get_pkt_pppoe_session_id(struct sk_buff *skb)
{
    ...
    if ( !((struct ppp *)(skb->dev->priv))->channels.next )
        return 0;

    pch = list_entry(((struct ppp *)(skb->dev->priv))->channels.next, struct channel, clist);
    if ( !pch->chan || !((struct sock *)pch->chan)->protinfo.pppox )
        return 0;

    return ((struct sock *)pch->chan)->protinfo.pppox->proto.pppoe.pa.sid;
}
#endif
