/******************************************************************************
**
** FILE NAME    : danube_mei_mib.c
** PROJECT      : Danube
** MODULES      : MEI
**
** DATE         : 1 Jan 2006
** AUTHOR       : TC Chen
** DESCRIPTION  : MIB part of MEI Driver
...
** $Version $Date      $Author     $Comment
   0.99.02             TC Chen     Fixed getting wrong data of GAINpsds/GAINpsus
                                        / BITSpsds/ BITSpsus parameters.
		                   Fixed getting wrong data of SNRpsxx and 
                                        ATCATPxx paraeters.
                                   Enabled some MIB APIs in loop diagnostics 
                                        mode.
   0:99.03  02/11/2006 TC Chen     Fix Data rate is zero issue
				   Fix SNRMds issue
   0.99.04  22/11/2006 TC Chen	   INFO 69 6 for alls output power upstream calls (dmt,2&2+). (per input form Knut Dettmr on 21/11/2006)
*******************************************************************************/

#ifndef EXPORT_SYMTAB
#define EXPORT_SYMTAB
#endif
#define DANUBE_MEI_MIB_RFC3440
//#define DANUBE_PORT_RTEMS
...
static char DANUBE_MEI_MIB_VERSION[]="0.90.04";

#define DANUBE_MEI_DEBUG
#ifdef DANUBE_MEI_DEBUG
#define DANUBE_MEI_DMSG(fmt, args...) printk( KERN_DEBUG  "%s: " fmt,__FUNCTION__, ## args)
#else
...
extern int showtime;
extern u16 RxMessage[MSG_LENGTH];
extern u16 TxMessage[MSG_LENGTH];  

static danube_mei_mib * current_intvl;
static struct list_head interval_list;
static danube_mei_mib * mei_mib;
...
extern u32 adsl_mode,adsl_mode_extend; // adsl mode : adsl/ 2/ 2+
static adslInitStats AdslInitStatsData;
    
#ifdef DANUBE_MEI_MIB_RFC3440
		...
		if(IS_FLAG_SET((&(pts.adslAtucChanInfo_pt->flags)), ATUC_CHAN_INTLV_DELAY_FLAG)){
			if((chantype.interleave!=1) || (chantype.fast==1)){
				...
				if(meiCMV(TxMessage, YES_REPLY,RxMessage)!=MEI_SUCCESS){
					...
					//KD
					pts.adslAtucChanInfo_pt->interleaveDelay=RxMessage[4];
				}
			}
		}
		...
		if(IS_FLAG_SET((&(pts.adslAturChanInfo_pt->flags)), ATUR_CHAN_INTLV_DELAY_FLAG)){
			if((chantype.interleave!=1) || (chantype.fast==1)){
				...
				if(meiCMV(TxMessage, YES_REPLY,RxMessage)!=MEI_SUCCESS){
					...
					//KD
					pts.adslAturChanInfo_pt->interleaveDelay=RxMessage[4];
				}
			}
		}
	...
	case GET_ADSL_ATUC_PERF_DATA:
		...
		if(IS_FLAG_SET((&(pts.atucPerfDataEntry_pt->flags)), ATUC_PERF_CURR_1DAY_LOFS_FLAG)){		
			...
			for(ptr=interval_list.next; ptr!=&(current_intvl->list); ptr=ptr->next){
				mib_ptr = list_entry(ptr, danube_mei_mib, list);
		...
		if(pts.adslAtucIntvlInfo_pt->IntervalNumber <1){
			...
			for(ptr=(current_intvl->list).prev; ptr!=&interval_list; ptr=ptr->prev){
				...
				if(i==pts.adslAtucIntvlInfo_pt->IntervalNumber){
					temp_intvl = list_entry(ptr, danube_mei_mib, list);
		...
		if(IS_FLAG_SET((&(pts.adslLineRateInfo_pt->flags)), LINE_RATE_ATTNDRUS_FLAG)){
			...
			}else
			{
				...
				if (loop_diagnostics_completed==1)
				    return -ERESTARTSYS;
				if (ifx_me_hdlc_send((char *)&hdlc_cmd[0],4)!= -EBUSY)
				{
					MEI_WAIT(1);		
					hdlc_rx_len=0;			
					hdlc_rx_len = ifx_mei_hdlc_read((char *)&hdlc_rx_buffer,32*2);
					if (hdlc_rx_len <=0)
					{
                				meierr = -ERESTARTSYS;
...
GET_ADSL_LINE_RATE_END:						
	...
	case GET_ADSL_LINE_INFO:
		...
		if(IS_FLAG_SET((&(pts.adslLineInfo_pt->flags)), LINE_INFO_ACATPDS_FLAG)){
			//KD: The upstream output power does not change when 
			//in showtime. 
		 	//Hence no nececcity to read it through OHC
			//info 69 6 can be used to read out the upstream 
			//output power in all dsl modes
...
#endif
	DANUBE_MEI_DMSG("Inside mib poll loop ...\n");
	i=0;
	while(danube_mei_mib_shutdown==0){
		if(i<MIB_INTERVAL)
		{
		...
		if(showtime==1){
			...
			if(time_now.tv_sec - current_intvl->start_time.tv_sec>=900){
				if(current_intvl->list.next!=&interval_list){
					current_intvl = list_entry(current_intvl->list.next, danube_mei_mib, list);
					do_gettimeofday(&(current_intvl->start_time));
				}
				else{
					mib_ptr = list_entry(interval_list.next, danube_mei_mib, list);
					list_del(interval_list.next);
					memset(mib_ptr, 0, sizeof(danube_mei_mib));
...
{
	DANUBE_MEI_DMSG("Starting mib_poll...\n");
  	danube_mei_mib_shutdown = 0;
...
#if 0	// move to danube_mei_core.c	
	// read adsl mode
...
int mei_mib_adsl_link_down(void)
{
	...
	adsl_mode = 0;
	memset(&mib_pread,0,sizeof(mib_pread));
...
int danube_mei_mib_init(void)
{
	int i=0;
        ...
        printk("Danube MEI MIB version:%s\n", DANUBE_MEI_MIB_VERSION);
	...
	mei_mib = (danube_mei_mib *)kmalloc((sizeof(danube_mei_mib)*INTERVAL_NUM), GFP_KERNEL);
	if(mei_mib == NULL){
		return -1;
	}
	memset(mei_mib, 0, (sizeof(danube_mei_mib)*INTERVAL_NUM));
	...
	current_intvl = list_entry(interval_list.next, danube_mei_mib, list);
	do_gettimeofday(&(current_intvl->start_time)); 
#if defined(__LINUX__)	
...
void danube_mei_mib_cleanup(void)
{
	danube_mei_mib_shutdown = 1;
