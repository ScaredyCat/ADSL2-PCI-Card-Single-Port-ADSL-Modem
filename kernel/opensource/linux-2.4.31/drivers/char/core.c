 ...
 *
 *  $Id: core.c,v 1.1 2006/03/13 09:53:14 huang Exp $
 *
 */
...
#endif
extern void amazonasc_console_init (void);
/*
...
static void uart_change_speed (struct uart_info *info,
 ...
 */
void
uart_event (struct uart_info *info, int event)
{
	info->event |= 1 << event;
...
static void
uart_stop (struct tty_struct *tty)
{
...
static void
uart_tasklet_action (unsigned long data)
{
...
static inline void
uart_update_altspeed (struct uart_info *info)
{
...
static int
uart_startup (struct uart_info *info)
{
	unsigned long flags;
	unsigned long page;
	int retval = 0;
 ...
 */
static void
uart_shutdown (struct uart_info *info)
{
	unsigned long flags;
...
static inline u_int
uart_calculate_quot (struct uart_info *info, u_int baud)
{
	u_int quot;
...
#define RELEVENT_IFLAG(iflag)	((iflag) & (IGNBRK|BRKINT|IGNPAR|PARMRK|INPCK))
	...
	info->ops->change_speed (port, cflag, info->tty->termios->c_iflag,
				 quot);
}

static void
uart_put_char (struct tty_struct *tty, u_char ch)
{
	struct uart_info *info = tty->driver_data;
	unsigned long flags;
	...
	spin_lock_irqsave (&info->lock, flags);
	if (CIRC_SPACE (info->xmit.head, info->xmit.tail, UART_XMIT_SIZE) !=
	    0) {
		info->xmit.buf[info->xmit.head] = ch;
		info->xmit.head =
			(info->xmit.head + 1) & (UART_XMIT_SIZE - 1);
	}
	spin_unlock_irqrestore (&info->lock, flags);
}

static void
uart_flush_chars (struct tty_struct *tty)
{
	uart_start (tty);
}

static int
uart_write (struct tty_struct *tty, int from_user,
...
static int
uart_write_room (struct tty_struct *tty)
{
	struct uart_info *info = tty->driver_data;
...
static int
uart_chars_in_buffer (struct tty_struct *tty)
{
	struct uart_info *info = tty->driver_data;
...
static void
uart_flush_buffer (struct tty_struct *tty)
{
	struct uart_info *info = tty->driver_data;
	unsigned long flags;
 ...
 */
static void
uart_send_xchar (struct tty_struct *tty, char ch)
{
	struct uart_info *info = tty->driver_data;
...
static void
uart_throttle (struct tty_struct *tty)
{
	struct uart_info *info = tty->driver_data;
	unsigned long flags;
...
static void
uart_unthrottle (struct tty_struct *tty)
{
...
static int
uart_get_info (struct uart_info *info, struct serial_struct *retinfo)
{
 ...
 */
static int
uart_get_lsr_info (struct uart_info *info, unsigned int *value)
{
	u_int result;
	unsigned long flags;
...
static int
uart_get_modem_info (struct uart_info *info, unsigned int *value)
{
...
static int
uart_set_modem_info (struct uart_info *info, unsigned int cmd,
		     unsigned int *value)
{
...
static void
uart_break_ctl (struct tty_struct *tty, int break_state)
{
	struct uart_info *info = tty->driver_data;
	unsigned long flags;
...
static int
uart_do_autoconfig (struct uart_info *info)
{
	struct uart_port *port = info->port;
	int flags, ret;
 ...
 */
static int
uart_ioctl (struct tty_struct *tty, struct file *file,
	    unsigned int cmd, unsigned long arg)
{
	...
	case TIOCMBIC:
	case TIOCMSET:
		ret = uart_set_modem_info (info, cmd, (unsigned int *) arg);
		break;
...
static void
uart_set_termios (struct tty_struct *tty, struct termios *old_termios)
{
	struct uart_info *info = tty->driver_data;
	unsigned long flags;
	...
	if ((cflag ^ old_termios->c_cflag) == 0 &&
	    RELEVENT_IFLAG (tty->termios->c_iflag ^ old_termios->c_iflag) ==
	    0)
		return;
 ...
 */
static void
uart_close (struct tty_struct *tty, struct file *filp)
{
	struct uart_driver *drv =
		(struct uart_driver *) tty->driver.driver_state;
	struct uart_info *info = tty->driver_data;
...
static void
uart_wait_until_sent (struct tty_struct *tty, int timeout)
{
	struct uart_info *info = (struct uart_info *) tty->driver_data;
	unsigned long char_time, expire;

	if (info->port->type == PORT_UNKNOWN || info->port->fifosize == 0)
		return;
 ...
 */
static void
uart_hangup (struct tty_struct *tty)
{
	struct uart_info *info = tty->driver_data;
	struct uart_state *state = info->state;
...
static int
uart_block_til_ready (struct tty_struct *tty, struct file *filp,
...
uart_get (struct uart_driver *drv, int line)
{
...
static const char *
uart_type (struct uart_port *port)
{
	const char *str = NULL;
...
static int
uart_line_info (char *buf, struct uart_driver *drv, int i)
{
	...
	ret = sprintf (buf, "%d: uart:%s port:%08X irq:%d",
		       port->line, uart_type (port), port->iobase, port->irq);
...
static int
uart_read_proc (char *page, char **start, off_t off,
 ...
 */
void __init
uart_parse_options (char *options, int *baud, int *parity, int *bits,
		    int *flow)
{
	char *s = options;
 ...
 */
static int
uart_pm_set_state (struct uart_state *state, int pm_state, int oldstate)
{
	struct uart_port *port = state->port;
	struct uart_ops *ops = port->ops;
	int running = state->info && state->info->flags & ASYNC_INITIALIZED;
 ...
 */
static int
uart_pm_set_wakeup (struct uart_state *state, int data)
{
	int err = 0;
...
static int
uart_pm (struct pm_dev *dev, pm_request_t rqst, void *data)
{
 ...
 */
int
uart_register_port (struct uart_driver *drv, struct uart_port *port)
{
 ...
 */
void
uart_unregister_port (struct uart_driver *drv, int line)
{
	struct uart_state *state;
