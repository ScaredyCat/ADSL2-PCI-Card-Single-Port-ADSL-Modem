...
//606151:fchang added.start Port IGMP proxy and snooping to kernel 2.4.31
#if defined(CONFIG_IFX_IGMP_PROXY) || defined(CONFIG_IFX_IGMP_PROXY_MODULE)
/* fc: Added for IGMP proxy */
...
extern int ifx_mfc_add (struct mfcctl *mfc, int flag);
extern int ifx_mfc_del(struct mfcctl *mfc);
// extern int adm_process_mac_table_request (unsigned int cmd, struct _MACENTRY_ *mMACENTRY);
// EXPORT_SYMBOL (adm_process_mac_table_request);
EXPORT_SYMBOL(ifx_mfc_del);
EXPORT_SYMBOL(ifx_mfc_add);
EXPORT_SYMBOL(ifx_mrt_exit);
EXPORT_SYMBOL(ifx_mrt_init);
EXPORT_SYMBOL(ifx_del_vif);
EXPORT_SYMBOL(ifx_add_vif); 
EXPORT_SYMBOL(ifx_is_dev_bridge);
