...
//060620:henryhsu Move vlan code from Amazon to Danube
	...
	/* by MarsLin, for V-Bridge CPU Egress, LOCAL_OUT */
	if ((ifx_vbridge_hook != NULL) && (ifx_vbridge_hook(&skb, skb->dev, 3) < 0))
		return -1;
#endif
	br = dev->priv;
	read_lock(&br->lock);
...
/* Santosh: Added for IGMP Proxy/Snooping */
int ifx_is_dev_bridge (struct net_device *dev)
{
