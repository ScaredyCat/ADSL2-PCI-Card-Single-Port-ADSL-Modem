...
#include <linux/netfilter_ipv4/listhelp.h>
  ...
  #include <asm/danube/danube_ppa_api.h>
#endif
...
static void death_by_timeout(unsigned long ul_conntrack)
{
	struct ip_conntrack *ct = (void *)ul_conntrack;
        ...
        if (ct->hw_nat_enable == 1) {
                ret_que = HwNatEntryFunc(ct, 6); //Query

                if ( ret_que == 1 ) { // The traffic in HW NAT is alive
                        printk("HWNAT entry still alive! \n");
                        ct->hwnat.flag |= DEAD_TIMER;
                        if ( ct->state_timeout != 0 )
                                ip_ct_refresh(ct, ct->state_timeout);
                        else
                                ip_ct_refresh(ct, 60*HZ); //to check again after default seconds
                        return;
                }
                else {
			HwNatEntryFunc(ct, 2); //Delete
                }
        }
#endif
#ifdef CONFIG_DANUBE_PPA
	if ( ppa_hook_inactivity_status_fn != NULL )
	{
	       ...
	       struct sk_buff *skb)
{
	...
	atomic_set(&conntrack->ct_general.use, 1);
        ...
        conntrack->in_route.maniptype = -1;
        conntrack->out_route.maniptype = -1;
#ifdef CONFIG_ADM8668_HW_NAT
        conntrack->hw_nat_enable = 0;
#endif
#endif
...
/* Doesn't cover locally-generated broadcast, so not worth it. */
...
#ifdef CONFIG_IFX_ALG_QOS
                /*
                 * Mark the ICMP packets for ICMP ALG
                 * Mark the ifx_alg_qos_mark field of the conn_track for ICMP packets.
                 * Note that this is ALG application family marking
                 */
                if ( proto->proto == IPPROTO_ICMP ) {
                        ct->ifx_alg_qos_mark = IFX_ALG_APP_ICMP;  //ICMP = 0x01000000
                        IFX_ALG_QOS_DBG("\n***Marked for ICMP ct_alg:%x \n",ct->ifx_alg_qos_mark);
                        IFX_ALG_QOS_DBG("\n after marking ICMP : schduled if:%s",(*pskb)->dev->name);
                }
#endif
	...
	if (ret != NF_DROP && ct->helper) {
		ret = ct->helper->help((*pskb)->nh.iph, (*pskb)->len,
                ...
                IFX_ALG_QOS_DBG("\n[ip_conntrack_in:after helper:ct->ifx_alg_qos_mark: %x ]\n",
                                ct->ifx_alg_qos_mark);
    ...
    if ( ret == NF_ACCEPT && ct != NULL && ppa_hook_session_add_fn != NULL )
    {
        ...
        ppa_hook_session_add_fn(*pskb, ct, flags);
    }
#endif
...
void ip_ct_refresh(struct ip_conntrack *ct, unsigned long extra_jiffies)
{
...
#ifdef CONFIG_ADM8668_HW_NAT
	ct->state_timeout = extra_jiffies;
#endif
	...
	if (!is_confirmed(ct))
...
#ifdef CONFIG_ADM8668_HW_NAT
                if (ct->hw_nat_enable && (ct->hwnat.flag & DEAD_TIMER)) {
                        ct->timeout.expires = jiffies + extra_jiffies;
                        add_timer(&ct->timeout);
                }
#endif
    	...
    	{
    		ppa_hook_set_inactivity_fn((PPA_U_SESSION *)ct, 0);
    	}
#endif
	}
	WRITE_UNLOCK(&ip_conntrack_lock);
