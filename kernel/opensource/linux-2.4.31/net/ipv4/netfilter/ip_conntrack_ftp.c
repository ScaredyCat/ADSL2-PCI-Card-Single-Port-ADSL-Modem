			...
			int (*getnum)(const char *, size_t, u_int32_t[], char))
{
	...
	*numlen = getnum(data + i, dlen - i, array, term);
	...
	return 1;
         ...
         */
static int ftp_expectfn(struct ip_conntrack *ct)
{
...
#ifdef CONFIG_IP_NF_MATCH_IFXATTACK
        /* Nirav : For FTP PORT Restricted */
#if 1
        if (search[i].ftptype == IP_CT_FTP_PORT) {
                //printk("Found FTP port command with port %d\n",array[4] << 8 | array[5]);
                ct_ftp_info->active_ftp_port = array[4] << 8 | array[5];
        }
#endif
#endif
...
#ifdef CONFIG_IFX_ALG_QOS
                IFX_ALG_QOS_DBG("SRC PORT %d",exp->tuple.src.u.tcp.port);
                IFX_ALG_QOS_DBG("DST PORT %d",exp->tuple.dst.u.tcp.port);
                /*
