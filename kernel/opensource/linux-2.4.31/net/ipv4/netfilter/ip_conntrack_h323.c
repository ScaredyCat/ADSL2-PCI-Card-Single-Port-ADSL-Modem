...
static int rtp_help (const struct iphdr *iph, size_t len,
             struct ip_conntrack *ct,
             enum ip_conntrack_info ctinfo)
{
        ct->ifx_alg_qos_mark = IFX_ALG_APP_H323 | IFX_ALG_PROTO_RTP ;
        DEBUGP ("\nrtp_help marked ct->ifx_alg_qos_mark to : %x ***\n", ct->ifx_alg_qos_mark );
}

/* RTP helper is not registered! */
static struct ip_conntrack_helper rtp_helper =
    { { NULL, NULL },
          "RTP",                /* name */
...
static int h245_expect(struct ip_conntrack *ct)
{
    WRITE_LOCK(&ip_conntrack_lock);

    //ct->ifx_alg_qos_mark = IFX_ALG_APP_H323 | IFX_ALG_PROTO_CTRL ;
    ct->ifx_alg_qos_mark = IFX_ALG_APP_H323 | IFX_ALG_PROTO_RTP ;

    DEBUGP("h245_expect: helper for %p added\n", ct);
    // RTP helper
    //ct->helper = &rtp_helper;
...
#ifdef CONFIG_IFX_ALG_QOS //  Suresh
        ct->ifx_alg_qos_mark = IFX_ALG_APP_H323;
        DEBUGP ("\nH323_ALG h225_help marked ct->ifx_alg_qos_mark to : %x ***\n",
                                ct->ifx_alg_qos_mark );
#endif
