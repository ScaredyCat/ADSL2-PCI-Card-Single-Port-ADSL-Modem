...
/* Santosh: handle cache report */

#if defined (CONFIG_IFX_IGMP_PROXY) || defined (CONFIG_IFX_IGMP_PROXY_MODULE)
int (*mcast_hook_mfc_cache) (struct sk_buff *) = NULL;
...
#if defined (CONFIG_IFX_IGMP_PROXY) || defined (CONFIG_IFX_IGMP_PROXY_MODULE)
/*
 *  Santosh: mcast routing utils for IGMP Proxy/Snooping - Begin
 */
...
int ifx_add_vif (struct vifctl *vif, int flag)
{
	int ret;
	
	ret = vif_add (vif, flag);
	return ret;
}

int ifx_del_vif (int vifi)
{
	rtnl_lock ();
	vif_delete (vifi);	
	rtnl_unlock ();	
...
int ifx_mfc_add (struct mfcctl *mfc, int flag)
{
	int ret = 0;
	rtnl_lock ();
	ret = ipmr_mfc_add (mfc, flag);
	rtnl_unlock ();	
	
	return ret;
}

int ifx_mfc_del (struct mfcctl *mfc)
{
	int ret = 0;
	rtnl_lock ();
...
/*
 *  Santosh: mcast routing utils for IGMP Proxy/Snooping - End
 */
#endif
...
ipmr_cache_unresolved(vifi_t vifi, struct sk_buff *skb)
{
	...
	if (c == NULL) {
		 ...
		 * The kernel puts a query to "mrouted" if the cache entry is not known.
		 * We try to send this request to our IGMP Proxy/Snooping
		 * 
		 */
	...
	if (nomore) {
		if ( mcast_hook_mfc_cache != NULL) {	
			mcast_hook_mfc_cache (skb);
		}
	}
#endif
	spin_unlock_bh(&mfc_unres_lock);
...
static int ipmr_mfc_delete(struct mfcctl *mfc)
{
	...
	DBPRINT ("ipmr_mfc_delete - group: %u.%u.%u.%u , line: %d\n", NIPQUAD(mfc->mfcc_mcastgrp.s_addr), line);
	...
	for (cp=&mfc_cache_array[line]; (c=*cp) != NULL; cp = &c->next) {
		if (c->mfc_origin == mfc->mfcc_origin.s_addr &&
			...
			*cp = c->next;
			...
			DBPRINT ("ipmr_mfc_delete - calling kmem_cache_free, group: %u.%u.%u.%u \n", NIPQUAD(c->mfc_mcastgrp));
			kmem_cache_free(mrt_cachep, c);
			return 0;
		}
	}
	printk ("ipmr_mfc_delete - No entry\n");
	return -ENOENT;
}
...
static int ipmr_mfc_add(struct mfcctl *mfc, int mrtsock)
{
	...
	line=MFC_HASH(mfc->mfcc_mcastgrp.s_addr, mfc->mfcc_origin.s_addr);
	DBPRINT ("ipmr_mfc_add - group: %u.%u.%u.%u , line: %d\n", NIPQUAD(mfc->mfcc_mcastgrp.s_addr), line);
	...
	if (c != NULL) {
		DBPRINT ("ipmr_mfc_add - c!=NULL, updating\n");
		write_lock_bh(&mrt_lock);
...
#endif
		write_unlock_bh(&mrt_lock);
		DBPRINT ("ipmr_mfc_add - c!=NULL, return 0\n");
		return 0;
	}
	...
	DBPRINT ("ipmr_mfc_add - calling ipmr_cache_alloc\n");
	c=ipmr_cache_alloc();
	if (c==NULL)
	{
		DBPRINT ("ipmr_mfc_add - ipmr_cache_alloc returned NULL\n");
		return -ENOMEM;
	}

	DBPRINT ("ipmr_mfc_add - cache alloc, group: %u.%u.%u.%u \n", NIPQUAD(mfc->mfcc_mcastgrp.s_addr));
	c->mfc_origin=mfc->mfcc_origin.s_addr;
	c->mfc_mcastgrp=mfc->mfcc_mcastgrp.s_addr;
	c->mfc_parent=mfc->mfcc_parent;
	ipmr_update_threshoulds(c, mfc->mfcc_ttls);
	DBPRINT ("ipmr_mfc_add - updated thresholds\n");
	...
	if (uc) {
		DBPRINT ("ipmr_mfc_add - calling ipmr_cache_resolve, group: %u.%u.%u.%u \n", NIPQUAD(c->mfc_mcastgrp));
		ipmr_cache_resolve(uc, c);
		DBPRINT ("ipmr_mfc_add - calling kmem_cache_free, group: %u.%u.%u.%u \n", NIPQUAD(uc->mfc_mcastgrp));
...
#if defined (CONFIG_IFX_IGMP_PROXY) || defined (CONFIG_IFX_IGMP_PROXY_MODULE)
static void mroute_clean_tables(void)
#else
			   ...
			   int vifi, int last)
{
	...
	struct sk_buff *skb2;
	 ...
	 DBPRINT("ipmr_queue_xmit - IP packet from %u.%u.%u.%u -> %u.%u.%u.%u with NextProtocol= %d\n",
		NIPQUAD(skb->nh.iph->saddr), NIPQUAD(skb->nh.iph->daddr), skb->nh.iph->protocol);
	...
	dev = rt->u.dst.dev;
	DBPRINT ("ipmr_queue_xmit - Outgoing if: %s\n",dev->name);
	...
	iph = skb2->nh.iph;
	ip_decrease_ttl(iph);
	DBPRINT ("ipmr_queue_xmit - after decrementing TTL: %d\n",skb2->nh.iph->ttl);
...
int ip_mr_forward(struct sk_buff *skb, struct mfc_cache *cache, int local)
{
	...
	DBPRINT("ip_mr_forward - IP packet from %u.%u.%u.%u -> %u.%u.%u.%u with NextProtocol= %d\n",
		NIPQUAD(skb->nh.iph->saddr), NIPQUAD(skb->nh.iph->daddr), skb->nh.iph->protocol);

	DBPRINT (" ip_mr_forward - pkt came on skb->dev: %s\n",skb->dev->name);

	vif = cache->mfc_parent;
	cache->mfc_un.res.pkt++;
	cache->mfc_un.res.bytes += skb->len;

	if (vif_table[vif].dev != NULL)
	{
	   DBPRINT (" ip_mr_forward - vif dev: %s with vif index: %d\n",vif_table[vif].dev->name, vif);
	}
	else
	{
		DBPRINT ("ip_mr_forward - vif.dev is NULL for vif index: %d\n",vif);
	}
	...
	if (vif_table[vif].dev != skb->dev) {
		...
		if (((struct rtable*)skb->dst)->key.iif == 0) {
			 ...
			 */
			DBPRINT ("ip_mr_forward - wrong intf, dont-forward (1)\n");
			goto dont_forward;
		}
		...
		if (true_vifi >= 0 && mroute_do_assert &&
			...
			DBPRINT ("ip_mr_forward - wrong intf, dont-forward (2)\n");
		goto dont_forward;
	}

	
	DBPRINT ("ip_mr_forward - about to forward, with TTL:%d, psend: %d\n", skb->nh.iph->ttl, psend);
	vif_table[vif].pkt_in++;
	vif_table[vif].bytes_in+=skb->len;

	DBPRINT ("ip_mr_forward - cache->mfc_un.res.maxvif: %d, cache->mfc_un.res.minvif: %d\n", 
		cache->mfc_un.res.maxvif, cache->mfc_un.res.minvif);
	...
	for (ct = cache->mfc_un.res.maxvif-1; ct >= cache->mfc_un.res.minvif; ct--) {
		DBPRINT ("ip_mr_forward - for loop (1), psend: %d, ct: %d\n",psend,ct);
		if (skb->nh.iph->ttl > cache->mfc_un.res.ttls[ct]) {
			if (psend != -1)
			{
				DBPRINT ("ip_mr_forward - in for loop, calling ipmr_xmit\n");
				ipmr_queue_xmit(skb, cache, psend, 0);
			}
			DBPRINT ("ip_mr_forward - for loop (2), psend: %d\n",psend);
			psend=ct;
		}
	}
	if (psend != -1) {
		DBPRINT ("ip_mr_forward - sending once\n");
		ipmr_queue_xmit(skb, cache, psend, !local);
	}

dont_forward:
	DBPRINT ("ip_mr_forward - dont_forward\n");
	if (!local)
		kfree_skb(skb);
	return 0;
}
...
int ip_mr_input(struct sk_buff *skb)
{
	...
	DBPRINT("ip_mr_input - Rcvd IP packet from %u.%u.%u.%u -> %u.%u.%u.%u with NextProtocol= %d\n",
		NIPQUAD(skb->nh.iph->saddr), NIPQUAD(skb->nh.iph->daddr), skb->nh.iph->protocol);

	if (IPCB(skb)->flags&IPSKB_FORWARDED)
	{
		DBPRINT ("ip_mr_input - IPSKB_FORWARDED, goto drop\n");
		goto dont_forward;
	}
	...
	if (!local) {
		    if (IPCB(skb)->opt.router_alert) {
				...
				{
					DBPRINT ("ip_mr_input - ip_call_ra_chain, return 0\n");
				    return 0;
				}	
			    ...
			    if (mroute_socket) {
				    ...
				    read_unlock(&mrt_lock);
					DBPRINT ("ip_mr_input - mroute_socket, return 0\n");
				    return 0;
			    }
			    read_unlock(&mrt_lock);
		    }
	...
	read_lock(&mrt_lock);
	DBPRINT ("ip_mr_input - about to find cache entry\n");
	...
	if (cache==NULL) {
		...
		if (local) {
			...
			if (skb2 == NULL) {
				read_unlock(&mrt_lock);
				DBPRINT ("ip_mr_input - error no buffers\n");
				return -ENOBUFS;
			}
			skb = skb2;
		}
		...
		if (vif >= 0) {
			...
			read_unlock(&mrt_lock);
			DBPRINT ("ip_mr_input - vif>=0\n");

			return err;
		}
		read_unlock(&mrt_lock);
		kfree_skb(skb);
			DBPRINT ("ip_mr_input - error ENODEV\n");
		return -ENODEV;
	}

	DBPRINT("ip_mr_input - calling ip_mr_forward for IP packet from %u.%u.%u.%u -> %u.%u.%u.%u with NextProtocol= %d\n",
		NIPQUAD(skb->nh.iph->saddr), NIPQUAD(skb->nh.iph->daddr), skb->nh.iph->protocol);
