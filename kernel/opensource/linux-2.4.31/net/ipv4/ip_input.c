...
static inline int ip_rcv_finish(struct sk_buff *skb)
{
	...
	struct iphdr *iph = skb->nh.iph;
        ...
        DBPRINT("ip_rcv_finish - Rcvd IP packet from %u.%u.%u.%u -> %u.%u.%u.%u\n",
...
#ifdef CONFIG_IP_NF_TURBONAT
        if (addroute_hook_fn != NULL) {
		(*addroute_hook_fn)(skb);
        }
#endif
...
int ip_rcv(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt)
{
	 ...
	 */
        DBPRINT("ip_rcv - Rcvd IP packet from %u.%u.%u.%u -> %u.%u.%u.%u \n",
                NIPQUAD(skb->nh.iph->saddr), NIPQUAD(skb->nh.iph->daddr));
...

	{
        ...
        if (!sysctl_netfilter_prerouting_enable) {
                DBPRINT ("ip_rcv - calling ip_rcv_finish from netfilter\n");
                return ip_rcv_finish(skb);
        }
        /* 507141:tc.chen end */
#endif
        DBPRINT ("ip_rcv - calling ip_rcv_finish from outside netfilter\n");
