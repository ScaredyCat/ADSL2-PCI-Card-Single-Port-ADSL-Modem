...
int sysctl_net_qdisc_enable = 1; //507141:tc.chen enable/disable qdisc flag
#endif
...
int (*mcast_hook_proxy)(struct sk_buff *) = NULL;
#endif
...
#ifdef CONFIG_IFX_NETFILTER_OPT
       if (sysctl_net_qdisc_enable) { //507141:tc.chen
#endif
		q = dev->qdisc;
		if (q->enqueue) {
...
#ifdef CONFIG_IFX_BR_OPT
static int __bridge process_backlog(struct net_device *backlog_dev, int *budget)
#else
...
#if defined(CONFIG_IFX_IGMP_PROXY) || defined(CONFIG_IFX_IGMP_PROXY_MODULE)
		/* fc: Added for IGMP proxy */
		if ((mcast_hook_proxy != NULL) && (mcast_hook_proxy(skb) == 1))
		{
			goto lbl_skb_handled;
		}
#endif
...
int register_netdevice(struct net_device *dev)
{
	...
	dev->ifx_nfext_vbridge_priv = NULL;
#endif
