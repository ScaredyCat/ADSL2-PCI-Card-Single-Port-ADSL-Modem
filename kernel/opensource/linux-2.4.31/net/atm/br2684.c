...
/* Revision History:
 *		Henry:
 *			rfc2684 routed mode for LLC support
 *		Mars:
 *			rfc2684 routed mode for VCMUX support
 */

// /*165001:henryhsu:20050606:Modify to support "ifconfig nasx -multicast"*/
...
struct br2684_vcc {
	...
	enum br2684_payload payload; /*Add by Henry*/
	...
	struct br2684_vcc *brvcc)
{
	...
	/* Modify by Henry , for outgoing packet with bridge PDU and 
           and routed PDU.
	 */
	...
	if (brvcc->encaps == e_llc)
	{
		...
		{
			memcpy(skb->data+offset, snap_bridged, BR2684_SNAP_LEN);
			offset += BR2684_SNAP_LEN;
			/* pid_ipv4 is poorly named.  should probably be
			 * pid_ethernet
			 */
			memcpy(skb->data+offset, pid_ipv4, BR2684_PID_LEN);
			offset += BR2684_PID_LEN;
			memcpy(skb->data+offset, pad_bridged, BR2684_PAD_LEN);
			offset += BR2684_PAD_LEN;
		}
		else if (brvcc->payload == p_routed)
		{
			unsigned short prot = __constant_ntohs(skb->protocol);
			memcpy(skb->data+offset, snap_routed, BR2684_SNAP_LEN);
			offset += BR2684_SNAP_LEN;

			switch(prot) {
			 case ETH_P_IP:
				memcpy(skb->data+offset,
					ethertype_ipv4, BR2684_ETHERTYPE_LEN);
				break;
#if 0
			 case ETH_P_IPV6:
				memcpy(skb->data+offset,
					ethertype_ipv6, BR2684_ETHERTYPE_LEN);
				break;
#endif
			 default:
				dev_kfree_skb(skb);
...
#ifndef CONFIG_IFX_BR_OPT  /* by MarsLin for performance tune while bridge mode!! */
...
/* when AAL5 PDU comes in:     Modify by Henry Hsu*/
#ifdef CONFIG_AMAZON
//507261:tc.chen static void br2684_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
static void __system br2684_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
#else
...
/*Indicate to bridge or routed , routed=0,bridge=1,nothing=-1*/
	int PDU_TYPE_FLAG=-1;
	...
	if (brvcc->encaps == e_llc) {
		...
		if (memcmp(skb->data, llc_oui_pid_pad, 7))
                {				
			if (memcmp(skb->data, llc_oui_ipv4, 8 - 2) == 0 && brvcc->payload == p_routed)
                    	{
                    	    	PDU_TYPE_FLAG = p_routed;
			     	plen = sizeof(llc_oui_ipv4);
			     	if (memcmp(skb->data+6, ethertype_ipv4, 2) == 0)
			     	{
		...
		{
                    	 PDU_TYPE_FLAG = p_bridged;
			/* strip FCS if present */
			if (skb->len > 7 && skb->data[7] == 0x01)
				__skb_trim(skb, skb->len - 4);
			}

		if (PDU_TYPE_FLAG != brvcc->payload) {
			brdev->stats.rx_errors++;
...
/*165001:henryhsu:20050606:Modify for support "ifconfig nasx -multicast"*/
	if (!(brdev->net_dev.flags & IFF_MULTICAST) && (skb->pkt_type == PACKET_MULTICAST))
...
#if defined(CONFIG_DANUBE) || defined(CONFIG_AMAZON)  // 165202:henryhsu: 2006-05-16 fix eoa pack drop problem. 
...
#else
    {
        struct atm_backend_br2684_temp {
            ...
            int                     payload;        /* BR2684_PAYLOAD_*Add by Herny*/
        ...
        if ( copy_from_user(&be_temp, (void *) arg, sizeof(be_temp)) )
        {
            MOD_DEC_USE_COUNT;
            return -EFAULT;
        }

        be.backend_num  = be_temp.backend_num;
        be.fcs_in       = be_temp.fcs_in;
        be.fcs_out      = be_temp.fcs_out;
        be.fcs_auto     = be_temp.fcs_auto;
        be.encaps       = be_temp.encaps;
        be.has_vpiid    = be_temp.has_vpiid;
        be.send_padding = be_temp.send_padding;
        be.min_size     = be_temp.min_size;

        memcpy(&be.ifspec, &be_temp.ifspec, sizeof(be.ifspec));
        memcpy(&be.vpn_id, &be_temp.vpn_id, sizeof(be.vpn_id));
    }
#endif
		...
		list_for_each(lhc, &brdev->brvccs) {
			...
			if (pos-- == 0)
				return sprintf(buf, "  vcc %d.%d.%d: encaps=%s"
				    ...
				    , (brvcc->payload == p_bridged) ? "bridged" : "routed"
#ifndef FASTER_VERSION
				    , brvcc->copies_failed
				    , brvcc->copies_needed
#endif /* FASTER_VERSION */
