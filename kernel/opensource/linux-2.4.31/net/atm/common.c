//060414:leejack Change alloc_tx method to ifx proprietary
...
#define IFX_LLC_MUX_DBG(format,args...) printk(format,##args)
#else
#define DPRINTK(format,args...)
#define IFX_LLC_MUX_DBG(format,args...)
#endif

#if IFX_OAM_SUPPORT_FLAG
ifx_def_pfn_oam pfn_push_oam = NULL;
#else
#endif
		...
		struct scm_cookie *scm)
{
	...
	set_current_state(TASK_INTERRUPTIBLE);
  	error = 0;
//060414:leejack
#ifdef CONFIG_ATM_AMAZON
        while (!(skb = amazon_atm_alloc_tx(vcc,eff))) {
#else
#if !CONFIG_ATM_DANUBE
  	while (!(skb = alloc_tx(vcc,eff))) {
#else
	while ( !(skb = atm_alloc_tx(vcc,eff)) ) {
#endif
#endif
		   ...
		   char *optval, int optlen)
{
	...
	vcc = ATM_SD(sock);
	switch (optname) {
#if defined(CONFIG_DANUBE) || defined(CONFIG_AMAZON)
...
*	Function Name	:	ifx_set_oam_rcv_function
*	Description		:	This function sets function to be called whenever OAM packet receive
*   Input Values	:
*	Output Values	:
*	Return values	:	None
*	Notes			:
...
********************************************************************************************/
EXPORT_SYMBOL(ifx_set_oam_rcv_function);

void ifx_set_oam_rcv_function(ifx_def_pfn_oam pfn_oam)
{
	if(pfn_oam != NULL)
	{
		IFX_LLC_MUX_DBG("\n Setting OAM rec function \n");
		pfn_push_oam = pfn_oam;
		return;
	}
	return;
}
...
*	Function Name	:	ifx_push_oam
*	Description	:	This function is registed in atm_vcc (push_oam function pointer) so
				this function is called by driver whenever OAM Packet receives on interface
				and this function called the receive function registed by OAM module
*	Input Values	:
* 	Output Values	:
*	Return values	: None
*	Notes		:
...
void ifx_push_oam(struct atm_vcc *atmvcc, void *cell)
{
	if (pfn_push_oam != NULL)
	{
		IFX_LLC_MUX_DBG("\n Calling OAM rec function \n");
		pfn_push_oam((char *)cell);
		return;
	}
	return;
}
