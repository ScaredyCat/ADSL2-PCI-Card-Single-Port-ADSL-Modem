...
/*
 * Description:
 *  Driver for Infineon DANUBE DMA
 */
//-----------------------------------------------------------------------
...
#ifdef CONFIG_DANUBE_ETHERNET_D2
...
#if !defined(ENABLE_RX_DPLUS_PATH) || !ENABLE_RX_DPLUS_PATH
  char global_device_name[MAX_DMA_DEVICE_NUM][20]={{"PPE"},{"DEU"},{"SPI"},{"SDIO"},{"MCTRL0"},{"MCTRL1"}};
  _dma_chan_map default_dma_map[MAX_DMA_CHANNEL_NUM]={{"PPE",DANUBE_DMA_RX,0,DANUBE_DMA_CH0_INT,0},
...
#else
  char global_device_name[MAX_DMA_DEVICE_NUM][20]={{"PPE"},{"ETH2"},{"DEU"},{"SPI"},{"SDIO"},{"MCTRL0"},{"MCTRL1"}};
  _dma_chan_map default_dma_map[MAX_DMA_CHANNEL_NUM]={{"PPE",DANUBE_DMA_RX,0,DANUBE_DMA_CH0_INT,0},
                                                    {"PPE",DANUBE_DMA_TX,0,DANUBE_DMA_CH1_INT,0},
                            {"PPE",DANUBE_DMA_RX,1,DANUBE_DMA_CH2_INT,1},
                            {"PPE",DANUBE_DMA_TX,1,DANUBE_DMA_CH3_INT,1},
...
u32 g_danube_dma_int_status=0;
int g_danube_dma_in_process=0;/*0=not in process,1=in process*/
...
void rx_chan_intr_handler(int chan_no)
{
    ...
    if ( rx_desc_p->status.field.OWN == CPU_OWN && rx_desc_p->status.field.C && rx_desc_p->status.field.data_length < 1536 )
#else
...
int dma_device_register(_dma_device_info* dev)
{
...
#if 0
    if(strcmp(dev->device_name,"MCTRL0")==0||strcmp(dev->device_name,"MCTRL1")==0)
    {   /*select the port*/
        *DANUBE_DMA_PS=4;
        /*set port parameters*/
    ...
    for(i=0;i<dev->max_tx_chan_num;i++)
    {
        ...
        if(pCh->control==DANUBE_DMA_CH_ON)
        {
            ...
            *DANUBE_DMA_CS=chan_no;
            ...
            if(*DANUBE_DMA_CDBA!=(u32)CPHYSADDR(pCh->desc_base))
                *DANUBE_DMA_CDBA=(u32)CPHYSADDR(pCh->desc_base);
    ...
    for(i=0;i<dev->max_rx_chan_num;i++)
    {
        ...
        if(pCh->control==DANUBE_DMA_CH_ON)
        {
            ...
            *DANUBE_DMA_CCTRL|=2;
            while(*DANUBE_DMA_CCTRL&2){};
            *DANUBE_DMA_CIE=0x0a;/*0A fix me, should enable all the interrupts here?*/
            *DANUBE_DMA_IRNEN|=1<<chan_no;
...
#if defined(ENABLE_DANUBE_ETHERNET_D2) && ENABLE_DANUBE_ETHERNET_D2
        if ( map[i].irq != DANUBE_DMA_CH2_INT && map[i].irq != DANUBE_DMA_CH4_INT )
        {
    ...
    *DANUBE_DMA_IRNEN=0;/*disable all the interrupts first*/
    ...
    for(i=0;i<MAX_DMA_CHANNEL_NUM;i++)
    {
        ...
        *DANUBE_DMA_CPOLL= 0x80000040;
        ...
        if(i==10 || i==11)
            *DANUBE_DMA_CCTRL=(*DANUBE_DMA_CCTRL&(~(3<<9)))|(1<<9);
...
int danube_dma_init(void)
{
    ...
    danube_dma_sem=(struct semaphore*)kmalloc(sizeof(struct semaphore),GFP_KERNEL);
