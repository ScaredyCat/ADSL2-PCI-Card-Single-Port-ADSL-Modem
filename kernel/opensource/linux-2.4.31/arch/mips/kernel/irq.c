...
#define ILATENCY_INTERRUPT_OVERHEAD_START() interrupt_overhead_start()
#define ILATENCY_INTERRUPT_OVERHEAD_STOP() interrupt_overhead_stop()
#define ILATENCY_RET_FROM_EXCEPTION() intr_ret_from_exception()

#else

#define ILATENCY_INTERRUPT_OVERHEAD_START()
#define ILATENCY_INTERRUPT_OVERHEAD_STOP()
 ...
 */
#ifdef CONFIG_AMAZON
asmlinkage unsigned int __system do_IRQ(int irq, struct pt_regs *regs)
#else
