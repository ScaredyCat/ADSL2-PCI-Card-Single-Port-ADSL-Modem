...
static int show_cpuinfo(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "AMAZON_CHIP_VERSION\t: %x\n",AMAZON_MCD_CHIPID_VERSION_GET(chipid));
	seq_printf(m, "AMAZON_CHIP_PART_NUMBER\t: %x\n",AMAZON_MCD_CHIPID_PART_NUMBER_GET(chipid));
	seq_printf(m, "AMAZON_CHIP_MANID\t: %x\n",AMAZON_MCD_CHIPID_MANID_GET(chipid));
#endif
	return 0;
}
