...
extern INT RTMPGetCriticalParameter(
    IN  PCHAR   key,
    OUT PCHAR   dest,   
    IN  INT     destsize,
    IN  PCHAR   buffer);
...
{
	VIRTUAL_ADAPTER	*pVirtualAd = dev->priv;
	PRTMP_ADAPTER	pAd = pVirtualAd->RtmpDev->priv;
#ifdef RTL865X_SOC
	INT				index;
	unsigned int 	linkid;
#endif
	...
	for(index = 0; index < MAX_WDS_ENTRY; index++)
	{
		...
		{// find available 
			for (i = 0; i < 32; i++)
			{
				...
				if (pAd->MC_RowID >= 0)
					sprintf(slot_name, "wds%02d_%d", pAd->MC_RowID, index);
				else
#endif // MULTIPLE_CARD_SUPPORT //
				sprintf(slot_name, "wds%d", i);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,5,0)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
				device = dev_get_by_name(net_dev->nd_net, slot_name);
#else
...
#ifdef MULTIPLE_CARD_SUPPORT
				if (pAd->MC_RowID >= 0)
		            sprintf(dev->name, "wds%02d_%d", pAd->MC_RowID, i);
				else
#endif // MULTIPLE_CARD_SUPPORT //
			...
			char *buffer)
{
	...
	if (RTMPGetCriticalParameter("WdsKey", tmpbuf, 255, buffer))
	{			
		for (i = 0; i < MAX_WDS_ENTRY; i++)
			NdisZeroMemory(&pAd->WdsTab.WdsEntry[i].WdsKey, sizeof(CIPHER_KEY));
