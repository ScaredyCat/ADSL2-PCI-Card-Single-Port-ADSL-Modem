...
//			pAd->ApCfg.MBSSID[apidx].CapabilityInfo |= 0x0200;
	...
	if (pAd->MACVersion >= 0x28830300) // 3*3
	{
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	UINT8		BBPValue = 0;
	CHAR		chStartIdx, chEndIdx, index, curPriChIdx, curSecChIdx;
	UINT32		regValue;
...
#if 0
	printk("The Ctrl Channel=%d, SCX=%s!\n", pAd->CommonCfg.Channel, 
			(pAd->CommonCfg.RegTransmitSetting.field.EXTCHA == EXTCHA_ABOVE) ? "ABOVE" : "BELOW");
	printk("The Channel List is:");
	for (index =0 ; index < pAd->ChannelListNum; index++)
	{
		printk("%d ", pAd->ChannelList[index].Channel);
	}
	printk("\n");
#endif
...
#if 0
			printk("2.4GHz: Found the Central Channel index(%d) from the ChList, ExtCh=%d, effected Ch index range=%d~%d!\n", 
					index, pAd->CommonCfg.RegTransmitSetting.field.EXTCHA, chStartIdx, chEndIdx);
			printk("2.4GHz: Primary Channel=[idx=%d, Ch=%d], Secondary Channel=[idx=%d, Ch=%d]!\n", 
					curPriChIdx, pAd->ChannelList[curPriChIdx].Channel, curSecChIdx, pAd->ChannelList[curSecChIdx].Channel);
#endif
		}
		else
		{
...
#if 0
			printk("5.0GHz: Found the Ctrl Channel index(%d) from the ChList, ExtCh=%d, effected Ch index range=%d~%d!\n", 
					index, pAd->CommonCfg.RegTransmitSetting.field.EXTCHA, chStartIdx, chEndIdx);
			printk("5.0GHz: Primary Channel=[idx=%d, Ch=%d], Secondary Channel=[idx=%d, Ch=%d]!\n", 
					curPriChIdx, pAd->ChannelList[curPriChIdx].Channel, curSecChIdx, pAd->ChannelList[curSecChIdx].Channel);
#endif
		}
		else
	...
	for (index = chStartIdx; index<=chEndIdx; index++)
	{
		...
		//printk("Passive scanning for Channel %d.....\n", Channel);
