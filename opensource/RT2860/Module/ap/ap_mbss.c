...
#define MBSS_MAX_DEV_NUM    32
	...
	for(bss_index=FIRST_MBSSID; bss_index<max_bss_num; bss_index++)
	{
		...
		for(index=0; index<MBSS_MAX_DEV_NUM; index++)
		{
			...
			cur_dev_p = dev_get_by_name(main_dev_p->nd_net, slot_name);
#else
	...
	IN	PNET_DEV	dev_p)
{
	...
	PRTMP_ADAPTER	pAd = virtual_ad_p->RtmpDev->priv;
	...
	IN PNET_DEV dev)
{
	...
	{
		if( pAd->ApCfg.MBSSID[i].MSSIDDev == dev)
		{
			idx = i;
			break;
		}
	...
	IN	PNET_DEV	dev_p)
{
	VIRTUAL_ADAPTER	*virtual_ad_p = dev_p->priv;
	PRTMP_ADAPTER	pAd = virtual_ad_p->RtmpDev->priv;
	INT apidx = GetMbssIdx(pAd, dev_p);
