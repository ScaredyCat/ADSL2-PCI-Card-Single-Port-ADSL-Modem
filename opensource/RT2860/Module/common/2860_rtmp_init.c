	...
	IN	UCHAR			RingType)
{
	...
	switch (RingType)
	{
		...
		case QID_RX:
			...
			RxSwReadIdx = pAd->RxRing.RxSwReadIdx;
			if (pAd->RxRing.RxDmaIdx < RX_RING_SIZE)
			{
				...
				{
					if (RxSwReadIdx == pAd->RxRing.RxSwReadIdx)
						break;
					pRxD  = (PRXD_STRUC) pAd->RxRing.Cell[RxSwReadIdx].AllocVa;
                	pRxD->DDONE = 0 ;
					INC_RING_INDEX(RxSwReadIdx, RX_RING_SIZE);
				}
				RTMP_IO_WRITE32(pAd, RX_CRX_IDX, pAd->RxRing.RxCpuIdx);
			}
			else if (pAd->RxRing.RxDmaIdx == 0xffffffff)
			{
