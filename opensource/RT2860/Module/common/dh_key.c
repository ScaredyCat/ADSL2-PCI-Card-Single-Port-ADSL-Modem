...
#include "rt_config.h"
#define KMALLOC(a) kmalloc(a,MEM_ALLOC_FLAG)
#define KFREE(a) kfree(a)
#ifndef DHPRINT
#define DHPRINT(fmt, args...) printk(fmt, ## args)
#endif
...
static int EVP_DigestInit_ex(EVP_MD_CTX *ctx, const EVP_MD *type)
{
	if (ctx->digest != type)
		...
		ctx->digest=type;
		if (type->ctx_size)
			ctx->md_data=KMALLOC(type->ctx_size);
		}
...
static int bnrand(int pseudorand, BIGNUM *rnd, int bits, int top, int bottom)
{
	...
	buf=(unsigned char *)KMALLOC(bytes);
	if (buf == NULL)
		{
		goto err;
		}
                           ...
                           unsigned char *secrete_dhkey, int *secrete_dhkey_len)
{
	...
	*secrete_dhkey_len = compute_key(secrete_dhkey,&DH_PubKey_Peer,DHSecret);
	if (DH_PubKey_Peer.d != NULL) KFREE(DH_PubKey_Peer.d);
