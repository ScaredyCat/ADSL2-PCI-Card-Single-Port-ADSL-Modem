	...
	IN	UCHAR			apidx)
{
	...
	*rsnielen_cur_p = p_offset; 
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		{
			// 1. insert cipher suite
			RTMPInsertRsnIeCipher(pAd, Wpa2Ie, WepStatus, bMixCipher, pRsnIe_ex, &s_offset);

			// 2. insert AKM
			RTMPInsertRsnIeAKM(pAd, Wpa2Ie, AuthMode, apidx, pRsnIe_ex, &s_offset);

			// 3. insert capability
			RTMPInsertRsnIeCap(pAd, Wpa2Ie, apidx, pRsnIe_ex, &s_offset);
