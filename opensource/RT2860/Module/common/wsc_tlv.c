...
static BOOLEAN  wsc_tlv_init_done = FALSE;

// Global reusable buffer
// move HmacData definition to rtmp.h (for multiple card)
...
int AppendWSCTLV(USHORT index, OUT UCHAR * obuf, IN UCHAR * ibuf, IN u16 varlen)
{
	...
	len = /*cpu2be16*/ be2cpu16(wsc_tlv_0b[WSC_TLV_ENT(index)].len);
...
static VOID	WscProcessCredential(
	...
	IN  PWSC_CTRL           pWscControl)
{
	USHORT			WscType, WscLen, Cnt = 0, CurrentIdx=0, Idx;
	PUCHAR			pData, pTmp;
	...
	while (PlainLength > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_MAC_ADDR:
                RTMPMoveMemory(pProfile->Profile[CurrentIdx].MacAddr, pData, 6);
				break;
...
{
	UCHAR				TB[1];
	INT					Len = 0, templen = 0;//, DH_Len = 0;
	PUCHAR				pData = (PUCHAR)pbuf;
	PWSC_REG_DATA		pReg = (PWSC_REG_DATA) &pWscControl->RegData;
    
    //GenerateDHPublicKey(pWscControl->pPubKeyMem, pReg->EnrolleeRandom, 192, pReg->Pke, &DH_Len);
	// 1. Version
	...
	templen = AppendWSCTLV(WSC_ID_UUID_E, pData, &pAdapter->Wsc_Uuid_E[0], 0);
	pData += templen;
	Len   += templen;
	...
	//12. Manufacture
	templen = AppendWSCTLV(WSC_ID_MANUFACTURER, pData, pReg->EnrolleeInfo.Manufacturer, 0);
	pData += templen;
	Len   += templen;

	//13. Model Name
	templen = AppendWSCTLV(WSC_ID_MODEL_NAME, pData, pReg->EnrolleeInfo.ModelName, 0);
	pData += templen;
	Len   += templen;
	
	//14. Model Number
	templen = AppendWSCTLV(WSC_ID_MODEL_NUMBER, pData, pReg->EnrolleeInfo.ModelNumber, 0);
	pData += templen;
	Len   += templen;
	
	//15. Serial Number
	templen = AppendWSCTLV(WSC_ID_SERIAL_NUM, pData, pReg->EnrolleeInfo.SerialNumber, 0);
	pData += templen;
	Len   += templen;
	...
	//17. Device Name
	templen = AppendWSCTLV(WSC_ID_DEVICE_NAME, pData, pReg->EnrolleeInfo.DeviceName, 0);
	pData += templen;
	Len   += templen;
	...
	OUT	VOID *pbuf)
{
	...
	// Validate HMAC, reuse KDK buffer
    	hmac_sha(pReg->AuthKey, 32, pAdapter->pHmacData, HmacLen, KDK, 32);
    }
	// 22. Hmac
	...
	OUT	VOID *pbuf)
{
	...
	if (pReg->PinCodeLen == 4)
		hmac_sha(pReg->AuthKey, 32, pReg->PIN, 2, TB, 32);
	else
	...
	// Generate PSK2
	if (pReg->PinCodeLen == 4)
		hmac_sha(pReg->AuthKey, 32, &pReg->PIN[2], 2, TB, 32);
	else
	...
	// Validate HMAC, reuse KDK buffer
    	hmac_sha(pReg->AuthKey, 32, pAdapter->pHmacData, HmacLen, TB, 32);
    }
	...
	OUT	VOID *pbuf)
{
	...
	CerLen += AppendWSCTLV(WSC_ID_AUTH_TYPE, &TB[CerLen], (u8 *)&AuthType, 0);
	CerLen += AppendWSCTLV(WSC_ID_ENCR_TYPE, &TB[CerLen], (u8 *)&EncrType, 0);
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
		GenerateDHSecreteKey(pAdapter->ApCfg.MBSSID[MAIN_MBSSID].WscControl.pSecKeyMem, &pReg->EnrolleeRandom[0], 192, pReg->Pkr, 192, &pReg->SecretKey[0], &DH_Len);
#endif // CONFIG_AP_SUPPORT //
