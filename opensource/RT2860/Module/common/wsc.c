...
{		
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
	{
		// The first 6 bytes in Elem->Msg is the MAC address of wps station.
...
#ifdef CONFIG_AP_SUPPORT
	if ((MsgType == WSC_MSG_EAP_REG_RSP_ID) && (pAdapter->OpMode == OPMODE_AP))
	{
	...
	else if ((MsgType == WSC_MSG_EAP_ENR_RSP_ID) && (pAdapter->OpMode == OPMODE_AP))
	{
		...
		else
		{
			...
			if (pWscControl->WscMode == WSC_PBC_MODE)
			{
				...
				if ((pWscControl->WscStaPbcProbeInfo.WscPBCStaProbeCount == 1) &&
					(NdisEqualMemory(&pWscControl->WscStaPbcProbeInfo.StaMacAddr[0], &pWscControl->EntryAddr[0], 6) == FALSE))
					pWscControl->WscPBCOverlap = TRUE;
			}
				...
				(pWscControl->WscPBCOverlap == TRUE))
			{
				...
				RTMPSendWirelessEvent(pAdapter, IW_WSC_PBC_SESSION_OVERLAP, &pWscControl->EntryAddr[0], BSS0, 0); 
			}
			else if ((pWscControl->WscConfMode & WSC_PROXY_REGISTRAR) != 0)
			{
	...
	{   
#ifdef CONFIG_AP_SUPPORT	
		if ((gWscActionMode == WSC_PROXY) && (pAdapter->OpMode == OPMODE_AP))
		{
			RTMPModTimer(&pWscControl->EapolTimer, WSC_EAP_MSG_TIME_OUT);
		}
		else
	...
	IN  PWSC_CTRL       pWscControl)
{
	...
	if (bUPnPMsg)
	{
		if (
#ifdef CONFIG_AP_SUPPORT
			((MsgType == WSC_MSG_EAP_RSP_ID) && (pAdapter->OpMode == OPMODE_AP)) ||
#endif // CONFIG_AP_SUPPORT //
			(0))
		{
			//let it pass
    ...
    IN  PWSC_CTRL           pWscControl)
{
    ...
    {
#ifdef CONFIG_AP_SUPPORT    
        if ((gWscActionMode == WSC_PROXY) && (pAd->OpMode == OPMODE_AP))
        {
    		pWscControl->WscState = WSC_STATE_OFF;
        }
    	else
...
#ifdef CONFIG_AP_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
				WscSendMessage(pWscControl->pAd, WSC_OPCODE_START, NULL, 0, pWscControl, AP_MODE);
#endif // CONFIG_AP_SUPPORT //
			pWscControl->WscRetryCount++;
			RTMPModTimer(&pWscControl->EapolTimer, WSC_EAP_MSG_TIME_OUT);
		}
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
		NdisMoveMemory(&pWscControl->RegData.EnrolleeInfo.DeviceName, AP_WSC_DEVICE_NAME, sizeof(AP_WSC_DEVICE_NAME));
#endif // CONFIG_AP_SUPPORT //
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
		NdisMoveMemory(&pWscControl->RegData.EnrolleeInfo.ModelName, AP_WSC_MODEL_NAME, sizeof(AP_WSC_MODEL_NAME));
#endif // CONFIG_AP_SUPPORT //
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
		NdisMoveMemory(&pWscControl->RegData.EnrolleeInfo.PriDeviceType, &AP_Wsc_Pri_Dev_Type[0], 8);
#endif // CONFIG_AP_SUPPORT //
	...
	IN	USHORT selRegCfgMethods)
{
	...
	templen = AppendWSCTLV(WSC_ID_UUID_E, pData, &pAd->Wsc_Uuid_E[0], 0);
	pData += templen;
	Len   += templen;
		...
		if (file_w->f_op && file_w->f_op->write) 
		{
			...
			for (;;)
			{
				...
				if ((size_t)(offset - cfgData) < fileLen)
				{
...
#ifdef CONFIG_AP_SUPPORT
					((rtstrstr(pTempStr, "WPAPSK=") || rtstrstr(pTempStr, "WPAPSK1=")) &&
						!rtstrstr(pTempStr, "ApCliWPAPSK") && (pAd->OpMode == OPMODE_AP)) ||
#endif // CONFIG_AP_SUPPORT //
					(0))
					{
						int offset = 0;
						...
						if (
#ifdef CONFIG_AP_SUPPORT
							((pAd->ApCfg.MBSSID[MAIN_MBSSID].AuthMode < Ndis802_11AuthModeWPA) && (pAd->OpMode == OPMODE_AP)) ||
#endif // CONFIG_AP_SUPPORT //
							(0))
							goto next_item;
						NdisZeroMemory(pTempStr, 512);
...
#ifdef CONFIG_AP_SUPPORT
					else if (rtstrstr(pTempStr, WepKeyFormatName) &&  (pAd->OpMode == OPMODE_AP))
					{
					...
					else if (rtstrstr(pTempStr, WepKeyName) &&  (pAd->OpMode == OPMODE_AP))
					{
						if (pAd->ApCfg.MBSSID[MAIN_MBSSID].WepStatus == Ndis802_11WEPEnabled)                           
						{
							NdisZeroMemory(pTempStr, 512);
							
							/* This usage of sprintf()(i.e., sprintf(p1, "%s", p2)) */
							/* may force compiler to call undefined strcpy() on some platform. */
							/* And you will get "Unknown symbol strcpy" when you insmod. */
							//sprintf(pTempStr, "%s", WepKeyName);
                            memcpy(pTempStr, WepKeyName, strlen(WepKeyName) + 1);
							if ((pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscProfile.Profile[0].KeyLength == 5) || 
								(pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscProfile.Profile[0].KeyLength == 13))
							{
	...
	OUT PWSC_PROFILE        pWscProfile)
{
	...
	if((OpMode == AP_MODE) &&
		(WSC_SCSTATE_UNCONFIGURED == pWscControl->WscConfStatus) && (pAdapter->OpMode == OPMODE_AP))
	{
	...
	IN  PWSC_CTRL           pWscControl)
{
	...
	{
		UCHAR	count = 0, i, j = 0;
		count = pWscControl->WscStaPbcProbeInfo.WscPBCStaProbeCount;
		for (i = 0; i < count; i++)
		{
			if (pWscControl->WscStaPbcProbeInfo.WscPBCStaProbeCount == 1)
				j = i;
			...
			{
				NdisZeroMemory(&(pWscControl->WscStaPbcProbeInfo.StaMacAddr[i][0]), MAC_ADDR_LEN);
				pWscControl->WscStaPbcProbeInfo.ReciveTime[i] = 0;
				pWscControl->WscStaPbcProbeInfo.WscPBCStaProbeCount--;
			}
		}
		if (pWscControl->WscStaPbcProbeInfo.WscPBCStaProbeCount == 1)
		{
			pWscControl->WscStaPbcProbeInfo.ReciveTime[0] = pWscControl->WscStaPbcProbeInfo.ReciveTime[j];
			NdisMoveMemory(&(pWscControl->WscStaPbcProbeInfo.StaMacAddr[0][0]),
						   &(pWscControl->WscStaPbcProbeInfo.StaMacAddr[j][0]),
						   MAC_ADDR_LEN);
		}
		else
			pWscControl->WscPBCOverlap = TRUE;
