		...
		case ACTION_BSS_2040_COEXIST:	// Format defined in IEEE 7.4.7a.1 in 11n Draf3.03
			{
				...
				IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
				{
							...
							/*&& (pIntolerantReport->RegulatoryClass == get_regulatory_class(pAd))*/)
					{
						...
						printk("The pIntolerantReport len = %d, chlist=", pIntolerantReport->Len);
					...
					if (bNeedFallBack)
					{
						...
						{	
							printk("Fire the Bss2040CoexistTimer with timeout=%ld! jiffies=0x%lx\n", 
									pAd->CommonCfg.Dot11BssWidthChanTranDelay, jiffies);
						...
						{
							printk("Already fallback to 20MHz, Extend the timeout of Bss2040CoexistTimer, jiffies=0x%lx!\n", jiffies);
							// More 5 sec for the scan report of STAs.
							RTMPSetTimer(&pAd->CommonCfg.Bss2040CoexistTimer, (pAd->CommonCfg.Dot11BssWidthChanTranDelay + 5) * 1000);
						}
