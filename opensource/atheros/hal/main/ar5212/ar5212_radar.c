		      ...
		      struct ar5212RadarQElem *q, struct ar5212RadarQInfo *qInfo)
{
	...
	thisTimeStamp = (ds->ds_rxstat.rs_tstamp) & HAL_RADAR_TSMASK;
...
void
ar5212ProcessRadar(struct ath_hal *ah, struct ath_desc *ds)
{
	struct ath_hal_5212 *ahp=AH5212(ah);
	...
	if ((chan->channelFlags & CHANNEL_108G) &&
		...
		ar5212QueueRadarEvent(ah, ds, ahp->ah_arq, &ahp->ah_arqInfo);
	}
	if (chan->privFlags & CHANNEL_DFS)
		ar5212QueueRadarEvent(ah, ds, ahp->ah_radarq, &ahp->ah_radarqInfo);
}
