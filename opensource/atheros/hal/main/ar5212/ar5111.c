 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/owl_sw_dev/src/linux/madwifi/hal/main/ar5212/ar5111.c#3 $
 */
#include "opt_ah.h"
	...
	u_int16_t modesIndex, u_int16_t *rfXpdGain)
{
	...
	if (IS_CHAN_QUARTER_RATE(AH_PRIVATE(ah)->ah_curchan) ||
		IS_CHAN_HALF_RATE(AH_PRIVATE(ah)->ah_curchan)) {
        	u_int32_t	rfWaitI, rfWaitS, rfMaxTime;

        	rfWaitS = 0x1f;
        	rfWaitI = (IS_CHAN_HALF_RATE(AH_PRIVATE(ah)->ah_curchan)) ?
							0x10 : 0x1f;
        	rfMaxTime = 3;
