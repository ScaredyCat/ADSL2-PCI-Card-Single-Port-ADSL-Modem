 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/owl_sw_dev/src/linux/madwifi/hal/main/ar5210/ar5210_attach.c#7 $
 */
#include "opt_ah.h"
...
static const struct ath_hal_private ar5210hal = {{
    ...
    /* AR/Radar Functions */
    .ah_arEnable            = ar5210ArEnable,
    .ah_arDisable           = ar5210ArDisable,
    .ah_arReset         = ar5210ResetAR,
    ...
    .ah_updateCTSForBursting    = ar5210UpdateCTSForBursting,
    ...
    .ah_getDescInfo         = ar5210GetDescInfo,
    ...
    .ah_waitForBeaconDone       = ar5210WaitForBeaconDone,
 ...
 */
struct ath_hal *
ar5210Attach(u_int16_t devid, HAL_SOFTC sc, HAL_BUS_TAG st, HAL_BUS_HANDLE sh, HAL_STATUS *status)
{
#define N(a)    (sizeof(a)/sizeof(a[0]))
    struct ath_hal_5210 *ahp;
    struct ath_hal *ah;
    u_int i, loc;
    u_int32_t revid, pcicfg, sum;
    u_int16_t athvals[AR_EEPROM_ATHEROS_MAX], eeval;
    HAL_STATUS ecode;

    HALDEBUG(AH_NULL, "%s: devid 0x%x sc %p st %u sh %p\n",
        __func__, devid, sc, st, (void*) sh);

    /* NB: memory is returned zero'd */
    ahp = ath_hal_malloc(sizeof (struct ath_hal_5210));
    if (ahp == AH_NULL) {
        HALDEBUG(AH_NULL, "%s: no memory for state block\n", __func__);
        ecode = HAL_ENOMEM;
        goto bad;
    }
    ah = &ahp->ah_priv.h;
    ...
    if (!ar5210EepromRead(ah, AR_EEPROM_MAGIC, &eeval)) {
        HALDEBUG(ah, "%s: cannot read EEPROM magic number\n", __func__);
        ecode = HAL_EEREAD;
        goto eebad;
    }
    if (eeval != 0x5aa5) {
        HALDEBUG(ah, "%s: invalid EEPROM magic number 0x%x\n",
            __func__, eeval);
        ecode = HAL_EEMAGIC;
        goto eebad;
    }
    ...
    if (!ar5210EepromRead(ah, AR_EEPROM_PROTECT, &eeval)) {
        HALDEBUG(ah, "%s: cannot read EEPROM protection bits; "
            "read locked?\n", __func__);
        ecode = HAL_EEREAD;
        goto eebad;
    }
    ...
    if (ahp->ah_eeversion != 1) {
         ...
         */
        HALDEBUG(ah, "%s: unsupported EEPROM version %u (0x%x) found\n",
    ...
    if (!ar5210EepromRead(ah, AR_EEPROM_REG_DOMAIN, &eeval)) {
        HALDEBUG(ah, "%s: cannot read regulatory domain from EEPROM\n",
            __func__);
        ecode = HAL_EEREAD;
        goto eebad;
    }
    ...
    for (i = 0; i < 3; i++) {
        if (!ar5210EepromRead(ah, AR_EEPROM_MAC(i), &eeval)) {
            HALDEBUG(ah, "%s: cannot read EEPROM location %u\n",
                __func__, i);
            ecode = HAL_EEREAD;
            goto bad;
        }
...
static HAL_BOOL
ar5210GetChipPowerLimits(struct ath_hal *ah, HAL_CHANNEL *chans, u_int32_t nchans)
{
    /* XXX fill in */
    return AH_TRUE;
}
