 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/owl_sw_dev/src/linux/madwifi/hal/main/ar5210/ar5210_recv.c#1 $
 */
#include "opt_ah.h"
 ...
 */
void
ar5210SetRxDP(struct ath_hal *ah, u_int32_t rxdp)
{
 ...
 */
void
ar5210SetMulticastFilter(struct ath_hal *ah, u_int32_t filter0, u_int32_t filter1)
{
 ...
 */
HAL_BOOL
ar5210ClrMulticastFilterIndex(struct ath_hal *ah, u_int32_t ix)
{
	u_int32_t val;
 ...
 */
HAL_BOOL
ar5210SetMulticastFilterIndex(struct ath_hal *ah, u_int32_t ix)
{
	u_int32_t val;
 ...
 */
void
ar5210SetRxFilter(struct ath_hal *ah, u_int32_t bits)
{
	...
	u_int32_t pa, struct ath_desc *nds)
{
	...
	ds->ds_rxstat.rs_datalen = ads->ds_status0 & AR_DataLen;
	...
	ds->ds_rxstat.rs_tstamp = rstamp & 0x7fff;
	...
	if ((ads->ds_status1 & AR_FrmRcvOK) == 0) {
		if (ads->ds_status1 & AR_CRCErr)
			...
			ds->ds_rxstat.rs_status |= HAL_RXERR_FIFO;
		else {
			ds->ds_rxstat.rs_status |= HAL_RXERR_PHY;
			ds->ds_rxstat.rs_phyerr =
				(ads->ds_status1 & AR_PHYErr) >> AR_PHYErr_S;
		}
	}
	/* XXX what about KeyCacheMiss? */
	ds->ds_rxstat.rs_rssi = MS(ads->ds_status0, AR_RcvSigStrength);
	if (ads->ds_status1 & AR_KeyIdxValid)
		ds->ds_rxstat.rs_keyix = MS(ads->ds_status1, AR_KeyIdx);
	else
		ds->ds_rxstat.rs_keyix = HAL_RXKEYIX_INVALID;
	/* NB: caller expected to do rate table mapping */
	ds->ds_rxstat.rs_rate = MS(ads->ds_status0, AR_RcvRate);
	ds->ds_rxstat.rs_antenna  = (ads->ds_status0 & AR_RcvAntenna) ? 1 : 0;
	ds->ds_rxstat.rs_more = (ads->ds_status0 & AR_More) ? 1 : 0;
