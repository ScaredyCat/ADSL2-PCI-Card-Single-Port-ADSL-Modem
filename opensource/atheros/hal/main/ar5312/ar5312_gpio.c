 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/owl_sw_dev/src/linux/madwifi/hal/main/ar5312/ar5312_gpio.c#1 $
 */
#include "opt_ah.h"
...
{
        u_int32_t gpioOffset = (AR_GPIO_BASE - ((u_int32_t) ah->ah_sh));

	HALASSERT(gpio < AR_NUM_GPIO);

	OS_REG_WRITE(ah, gpioOffset+AR_GPIOCR, 
		  (OS_REG_READ(ah, gpioOffset+AR_GPIOCR) &~ AR_GPIOCR_CR_A(gpio))
		| AR_GPIOCR_CR_A(gpio));
...
ar5312GpioSet(struct ath_hal *ah, u_int32_t gpio, u_int32_t val)
{
	...
	reg =  OS_REG_READ(ah, gpioOffset+AR_GPIODO);
	reg &= ~(1 << gpio);
	reg |= (val&1) << gpio;

	OS_REG_WRITE(ah, gpioOffset+AR_GPIODO, reg);
	return AH_TRUE;
}
...
ar5312GpioGet(struct ath_hal *ah, u_int32_t gpio)
{
	...
	if (gpio < AR_NUM_GPIO) {
		u_int32_t val = OS_REG_READ(ah, gpioOffset+AR_GPIODI);
...
ar5312GpioSetIntr(struct ath_hal *ah, u_int gpio, u_int32_t ilevel)
{
	...
	val = OS_REG_READ(ah, gpioOffset+AR_GPIOCR);
	...
	OS_REG_WRITE(ah, gpioOffset+AR_GPIOCR, val);
