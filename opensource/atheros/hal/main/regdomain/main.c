 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/owl_sw_dev/src/linux/madwifi/hal/main/regdomain/main.c#2 $
 */
#include "opt_ah.h"
...
static const COUNTRY_CODE_TO_ENUM_RD allCountries[] = {
    ...
    {CTRY_CYPRUS,      ETSI3_WORLD,   "CY", "CYPRUS",     YES, YES, YES    },
    ...
    {CTRY_EGYPT,       ETSI3_WORLD,   "EG", "EGYPT",      YES, NO, YES     },
    ...
    {CTRY_HUNGARY,     ETSI4_WORLD,   "HU", "HUNGARY",    YES, NO, YES     },
    {CTRY_ICELAND,     ETSI1_WORLD,   "IS", "ICELAND",    YES, NO, YES     },
    {CTRY_INDIA,       NULL1_WORLD,   "IN", "INDIA",      YES, NO, YES     },
    {CTRY_INDONESIA,   APL1_WORLD,   "ID", "INDONESIA",  YES, NO, YES     },
    ...
    {CTRY_JORDAN,      APL4_WORLD,    "JO", "JORDAN",     YES, NO, YES     },
    ...
    {CTRY_LATVIA,      ETSI1_WORLD,   "LV", "LATVIA",     YES, NO, YES     },
    {CTRY_LEBANON,     NULL1_WORLD,   "LB", "LEBANON",    YES, NO, YES     },
    {CTRY_LIECHTENSTEIN,ETSI4_WORLD,  "LI", "LIECHTENSTEIN", YES, NO, YES  },
    ...
    {CTRY_SINGAPORE,   APL6_WORLD,    "SG", "SINGAPORE",  YES, YES, YES    },
    ...
    {CTRY_SWITZERLAND, ETSI4_WORLD,   "CH", "SWITZERLAND",YES, NO, YES     },
    ...
    {CTRY_UNITED_STATES_FCC49, FCC4_FCCA,   "PS", "UNITED STATES (PUBLIC SAFETY)",  YES, YES, YES},
...
main(int argc, char *argv[])
{
	...
	while ((i = getopt(argc, argv, "acdefoilm:pABGT")) != -1)
		switch (i) {
		case 'a':
			showall = AH_TRUE;
			break;
	...
	if (optind < argc)  {
		if (!cclookup(argv[optind], &ahp.ah_currentRD, &cc)) {
			if (!rdlookup(argv[optind], &ahp.ah_currentRD)) {
				const char* rdname;

				ahp.ah_currentRD = strtoul(argv[optind], NULL, 0);
				rdname = getrdname(ahp.ah_currentRD);
				if (rdname == NULL) {
		...
		if (!cclookup("US", &ahp.ah_currentRD, &cc)) {
	...
	if (cc != CTRY_DEFAULT)
		printf("\n%s (%s, 0x%x, %u) %s (0x%x, %u)\n",
			...
			getrdname(ahp.ah_currentRD), ahp.ah_currentRD, ahp.ah_currentRD);
	else
		printf("\n%s (0x%x, %u)\n",
			getrdname(ahp.ah_currentRD), ahp.ah_currentRD, ahp.ah_currentRD);
