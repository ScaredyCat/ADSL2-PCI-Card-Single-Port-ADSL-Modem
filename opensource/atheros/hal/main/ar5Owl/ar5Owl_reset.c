 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/owl_sw_dev/src/linux/madwifi/hal/main/ar5Owl/ar5Owl_reset.c#3 $
 */
#include "opt_ah.h"
...
#define	IS(_c,_f)	(((_c)->channelFlags & _f) || 0)
	...
	if ((IS(chan, CHANNEL_OFDM) ^ IS(chan, CHANNEL_CCK) ^ IS(chan, CHANNEL_HT20)) == 0) {
		HALDEBUG(ah, "%s: invalid channel %u/0x%x; not marked as "
			"OFDM or CCK\n", __func__,
			chan->channel, chan->channelFlags);
		FAIL(HAL_EINVAL);
...
#undef IS
	...
	if (bChannelChange) {
		if (chan->channelFlags & CHANNEL_108G)
			ar5212ArEnable(ah);
		else if ((chan->channelFlags & (CHANNEL_A | CHANNEL_ST | CHANNEL_A_HT20 | CHANNEL_A_HT40 )) &&
...
HAL_BOOL
ar5212Reset11n(struct ath_hal *ah, HAL_OPMODE opmode,
        HAL_CHANNEL *chan, HAL_HT *ht, HAL_BOOL bChannelChange, HAL_STATUS *status)
{
    return ar5212Reset(ah, opmode, chan, bChannelChange, status);
}
...
ar5212ChipReset(struct ath_hal *ah, HAL_CHANNEL *chan)
{
	...
	if (chan != AH_NULL) {		/* NB: can be null during attach */
		...
		if ((IS_CHAN_HT(chan) || IS_CHAN_OFDM(chan)) && 
		    (IS_CHAN_CCK(chan) || IS_CHAN_G(chan)))
			rfMode |= AR_PHY_MODE_DYNAMIC;
		else if (IS_CHAN_OFDM(chan) || IS_CHAN_HT(chan))
...
#define IQ_CAL_TRIES    10
	...
	if ((ichan->channelFlags & CHANNEL_CW_INT) == 0) {
		...
		if ((IS_CHAN_5GHZ(chan) ||
		    (IS_5112(ah) && (IS_CHAN_OFDM(chan) || IS_CHAN_HT(chan)))) && !IS_2413(ah))
...
#define	AR_PHY_BIS(_ah, _reg, _mask, _val) \
	...
	if (ahp->ah_eeversion < AR_EEPROM_VER3_3) {
		...
		if (AH_PRIVATE(ah)->ah_subvendorid == 0x1022 &&
		    (IS_CHAN_OFDM(chan) || IS_CHAN_HT(chan)))
