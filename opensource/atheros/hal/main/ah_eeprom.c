 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/owl_sw_dev/src/linux/madwifi/hal/main/ah_eeprom.c#1 $
 */
#include "opt_ah.h"
...
{
	const static u_int16_t intercepts3[] =
		{ 0, 5, 10, 20, 30, 50, 70, 85, 90, 95, 100 };
	const static u_int16_t intercepts3_2[] =
	...
	EEPROM_POWER_EXPN_5112 *pPowerExpn)
{
	...
	u_int16_t *pChanList = pCalDataset->pChannels;
	void *data;
	int i, j;
	...
	if (data == AH_NULL) {
		HALDEBUG(ah, "%s unable to allocate raw data struct (gen3)\n",
			__func__);
		return AH_FALSE;
	}
	...
	EEPROM_POWER_EXPN_5112 *pPowerExpn)
{
	...
	for (jj = 0; jj < NUM_XPD_PER_CHANNEL; jj++) {
		if (((xpdMask >> jj) & 1) > 0) {
			if (kk > 1) {
				HALDEBUG(ah, "%s: too many xpdGains in "
					"dataset: %u\n", __func__, kk);
				return AH_FALSE;
			}
			xgainList[kk++] = jj;
...
#define	EEREAD(_off) do {				\
	...
	for (mode = headerInfo11A; mode <= headerInfo11G; mode++) {
		...
		if (!eepromAllocExpnPower5112(ah, &eePower, &ee->ee_modePowerArray5112[mode])) {
			HALDEBUG(ah, "%s: did not allocate power struct\n",
				__func__);
			return AH_FALSE;
                }
                if (!eepromExpandPower5112(ah, &eePower, &ee->ee_modePowerArray5112[mode])) {
			HALDEBUG(ah, "%s: did not expand power struct\n",
				__func__);
			return AH_FALSE;
		}
	}
...
ar2413ReadCalDataset(struct ath_hal *ah, HAL_EEPROM *ee,
	...
	u_int32_t start_offset, u_int32_t maxPiers)
{
#define	EEREAD(_off) do {				\
	if (!ath_hal_eepromRead(ah, _off, &eeval))	\
		return AH_FALSE;			\
	...
	/* no 11a piers */
	/* 
	 * To-Do : verify assumption that invalid calPiers in ndis are 0xFF 
	 *  (retval of fbin2freq)
	 */
	idx = start_offset;
...
readEepromRawPowerCalInfo2413(struct ath_hal *ah, HAL_EEPROM *ee)
{
	...
	EEPROM_DATA_STRUCT_2413 pCalDataset2413[3];	/* XXX stack usage */
	EEPROM_DATA_STRUCT_2413 *pCal;
	...
	OS_MEMZERO(pCalDataset2413, sizeof(pCalDataset2413));
	off = ee->ee_eepMap2PowerCalStart;
	if (ee->ee_Bmode) {
		pCal = &pCalDataset2413[headerInfo11B];
		pCal->xpd_mask = ee->ee_xgain[headerInfo11B];
		if (!ar2413ReadCalDataset(ah, ee, pCal, off,
		    NUM_2_4_EEPROM_CHANNELS_2413)) {
			return AH_FALSE;
		}
	...
	if (ee->ee_Gmode) {
		pCal = &pCalDataset2413[headerInfo11G];
...
ath_hal_readEepromIntoDataset(struct ath_hal *ah, HAL_EEPROM *ee)
{
	...
	if (IS_5112(ah) && !ee->ee_eepMap) {
		HALDEBUG(ah, "%s: 5112 devices must have EEPROM 4.0 with the "
			"EEP_MAP set\n", __func__);
		return AH_FALSE;
	}
#endif
