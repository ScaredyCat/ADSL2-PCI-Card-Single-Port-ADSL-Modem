    ...
    while (1) {
        ...
        case PKTLOG_TYPE_RCUPDATE:
            if (filter & ATH_PKTLOG_RCUPDATE) {
                ...
                printf
                    ("RU: %4u %5d %5d %5d %5d %5u %5d %5d %5d %5u",
                ...
                if (opt_I | opt_i) {
                    ...
                    printf
                        ("                  Base packet+ACK (no c/w) time = %d usec\n",
                         $pkt_time_no_cw_usec);
                    printf
                        ("                   (note that there are slight discrepencies to be resolved)\n");
#endif
...
void pktlogdump_usage()
{
    fprintf(stderr,
            ...
            "          of the foll: rx tx rcf rcu ani [eg., pktlogdump -f rx,rcu]\n"
...
main(int argc, char *argv[])
{
    ...
    int opt_i = 0, opt_I = 0, opt_P = 0, opt_l = 0, fflag = 0, opt_a = 0;

    for (;;) {
        c = getopt(argc, argv, "l::iIlPa:");
        if (c < 0)
            break;
        switch (c) {
        case 'l':
    ...
    if (opt_a) {
        sprintf(proc_name, "/proc/" PKTLOG_PROC_DIR "/%s", ad_name);
        sprintf(sysctl_size, "/proc/sys/" PKTLOG_PROC_DIR "/%s/size",
                ad_name);
        sprintf(sysctl_enable, "/proc/sys/" PKTLOG_PROC_DIR "/%s/enable",
                ad_name);
    } else {
        sprintf(proc_name,
    ...
    if (fflag) {
        ...
        filter =
            ...
            ATH_PKTLOG_RCFIND | ATH_PKTLOG_RX | ATH_PKTLOG_TX;
    }
