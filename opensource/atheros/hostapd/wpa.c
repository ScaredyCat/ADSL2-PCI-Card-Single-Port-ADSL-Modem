...
#ifdef JUMPSTART		
	if (hostapd_set_privacy(hapd, !hapd->conf->js_p1)) {
		printf("Could not set privacy bit to %d for interface %s\n",
		       !hapd->conf->js_p1, hapd->conf->iface);
		return -1;
	}
#else
...
#ifdef JUMPSTART
	/* Do not plumb WPA/RSN IE down to the driver for P1 mode */
	if (hapd->conf->js_p1)
		goto alloc_wpa_auth;
#endif /* JUMPSTART */
...
#ifdef JUMPSTART
	/* JS P1 can now retrieve the WPA info from config */
	if (hapd->conf->js_p1)
		return 0;
#endif /* JUMPSTART */
		  ...
		  wpa_event event)
{
...
#ifdef JUMPSTART
	/* 
	 * XXX Divy. No JS code should be handled here.
	 * However, it is the only driver agnostic routine handling
	 * disassociations requests I could think of.
	 */
	...
	case WPA_AUTH:
	case WPA_ASSOC:
		/* Ignore. hostapd_new_assoc_sta() at the commands */
		break;
	case WPA_DEAUTH:
	case WPA_DISASSOC:
		/* the AP is ready to serve another P1 STA */
		if (hapd->conf->js_p1 && sta->js_session)
			hapd->jsw_profile->p1_in_progress = 0;
		js_free_station(sta);
		break;
	case WPA_REAUTH:
	case WPA_REAUTH_EAPOL:
		/* XXX. Ignore reauth for now */	
		break;
	}
wpa:
#endif /* JUMPSTART */
