...
madwifi_set_ieee8021x(void *priv, int enabled)
{
	...
	if (hapd->conf->js_p1) {
		if (set80211param(priv, IEEE80211_PARAM_AUTHMODE,
				  IEEE80211_AUTH_OPEN)) {
			hostapd_logger(hapd, NULL, HOSTAPD_MODULE_DRIVER,
				HOSTAPD_LEVEL_WARNING, "Error enabling Jumpstart P1!");
			return -1;
		}
                                            ...
                                            char *data, int len)
{
        ...
        while (pos + IW_EV_LCP_LEN <= end) {
                ...
                case IWEVREGISTERED:
                        ...
                        break;
                case IWEVCUSTOM:
                        custom = pos + (IW_EV_POINT_LEN - sizeof(caddr_t));
                        /* kernel passed Length and Flags in 4 bytes, so use any
                         * 4 byte field in the union to access them. */
                        /* we are using the "mode" just as a place holder to access length */
                        customLen = iwe->u.mode >> 16;
                        if (custom + customLen > end)
                                return;
                        buf = malloc(customLen + 1);
                        if (buf == NULL)
                                return;         /* XXX */
                        memcpy(buf, custom, customLen);
                        buf[customLen] = '\0';
