	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		*pos = '\0';
		...
		if (strcmp(buf, "interface") == 0) {
			...
			conf->js_p1 = atoi(pos);
		} else if (strcmp(buf, "jumpstart_p2") == 0) {
			conf->js_p2 = atoi(pos);
		} else if (strcmp(buf, "jumpstart_passHash") == 0) {
			if (hexstr2bin(pos, conf->js_passHash, JSW_SHA1_LEN)
				 || pos[JSW_SHA1_LEN * 2] != '\0') {
				printf("Line %d: Invalid passHash '%s'.\n", line,
				       pos);
				errors++;
			}
		} else if (strcmp(buf, "jumpstart_salt") == 0) {
			if (hexstr2bin(pos, conf->js_salt, JSW_NONCE_SIZE)
				 || pos[JSW_NONCE_SIZE * 2] != '\0') {
				printf("Line %d: Invalid passHash '%s'.\n", line,
				       pos);
				errors++;
			}	
