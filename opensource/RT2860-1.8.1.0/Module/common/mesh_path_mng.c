	...
	IN	RX_BLK	*pRxBlk)
{
	...
	if (GetMeshLinkId(pAd, pHeader->Addr2) == BSS_NOT_FOUND)
		return;

	// Check the MP whether it's Originator of PREQ.
	if (MAC_ADDR_EQUAL(pAd->MeshTab.CurrentAddress, GetMeshAddr4(pMeshHdr)))
		return;
	...
	for(i = 0; i < MAX_MESH_LINKS; i++)
	{
		if (PeerLinkValidCheck(pAd, i) == TRUE)
		{
			...
			InsertMeshHeader(pAd,
							...
							pAd->MeshTab.TTL,
							MeshHeaderSeq, pAd->MeshTab.CurrentAddress,
							NULL,
							NULL);
	...
	IN PUCHAR	pProxyAddr)
{
	...
	InsertMeshHeader(pAd,
					...
					MeshFlag.word,
					MESH_TTL,
					MeshSeq, pAd->MeshTab.CurrentAddress, NULL, NULL);
	...
	IN	RX_BLK	*pRxBlk)
{
	...
	// Check the MP whether it's Originator of PERR
	if (MAC_ADDR_EQUAL(pAd->MeshTab.CurrentAddress, GetMeshAddr4(pMeshHdr)))
		return;
