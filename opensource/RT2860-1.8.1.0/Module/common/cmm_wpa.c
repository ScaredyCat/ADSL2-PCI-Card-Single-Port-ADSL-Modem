...
UCHAR   	OUI_MSA_8021X[4]    = {0x00, 0x0F, 0xAC, 0x05};		// Not yet final - IEEE 802.11s-D1.06
UCHAR   	OUI_MSA_PSK[4]   	= {0x00, 0x0F, 0xAC, 0x06};		// Not yet final - IEEE 802.11s-D1.06
	...
	IN	UCHAR			apidx)
{
...
#ifdef MESH_SUPPORT
		if (bMeshLinkRSNIE)
			pRsnie2 = (RSNIE2*)pAd->MeshTab.RSN_IE;
		else
#endif // MESH_SUPPORT //
		{
#ifdef CONFIG_AP_SUPPORT
...
#ifdef MESH_SUPPORT
				if (bMeshLinkRSNIE)
					NdisMoveMemory(pRsnie_auth2->auth[0].oui, OUI_MSA_8021X, 4);
				else
#endif // MESH_SUPPORT //				
...
#ifdef MESH_SUPPORT
				if (bMeshLinkRSNIE)
					NdisMoveMemory(pRsnie_auth2->auth[0].oui, OUI_MSA_PSK, 4);
				else
#endif // MESH_SUPPORT //
	...
	if (bMeshLinkRSNIE)
	{
		hex_dump("RTMPMakeRSNIE for Mesh Link", pAd->MeshTab.RSN_IE, pAd->MeshTab.RSNIE_Len);
		DBGPRINT(RT_DEBUG_TRACE,("==> RTMPMakeRSNIE is completed\n"));
	}
	else
#endif // MESH_SUPPORT //
	{
