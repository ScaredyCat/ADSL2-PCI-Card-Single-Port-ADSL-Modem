...
{
	StateMachineInitEx(Sm, (STATE_MACHINE_FUNC_EX*)Trans, (ULONG)MESH_CTRL_MAX_STATES,
		(ULONG)MESH_CTRL_MAX_EVENTS, (STATE_MACHINE_FUNC_EX)DropEx, MESH_CTRL_IDLE, MESH_CTRL_IDLE);

	// MESH_CTRL_IDLE state
	StateMachineSetActionEx(Sm, MESH_CTRL_IDLE, MESH_CTRL_JOIN, (STATE_MACHINE_FUNC_EX)MeshCtrlJoinAction);

	// MESH_CTRL_DISCOVERY state
	StateMachineSetActionEx(Sm, MESH_CTRL_DISCOVERY, MESH_CTRL_DISCONNECT, (STATE_MACHINE_FUNC_EX)MeshCtrlDisconnectAtDicoveryAction);
	StateMachineSetActionEx(Sm, MESH_CTRL_DISCOVERY, APMT2_MLME_SCAN_FINISH, (STATE_MACHINE_FUNC_EX)MeshCtrlFinishDiscoveryAction);
	StateMachineSetActionEx(Sm, MESH_CTRL_DISCOVERY, APMT2_MLME_SCAN_ABORT, (STATE_MACHINE_FUNC_EX)MeshCtrlDiscoveryAbortAction);

	// MESH_CTRL_ACTIVATED state
	StateMachineSetActionEx(Sm, MESH_CTRL_ACTIVATED, MESH_CTRL_PLD, (STATE_MACHINE_FUNC_EX)MeshCtrlPeriodPLDAction);
	StateMachineSetActionEx(Sm, MESH_CTRL_ACTIVATED, MESH_CTRL_DISCONNECT, (STATE_MACHINE_FUNC_EX)MeshCtrlDisconnectAtActivatedAction);
	StateMachineSetActionEx(Sm, MESH_CTRL_ACTIVATED, MESH_CTRL_UCG_EVT, (STATE_MACHINE_FUNC_EX)MeshCtrlUcgWhenActivated);
	StateMachineSetActionEx(Sm, MESH_CTRL_ACTIVATED, MESH_PEER_UCG_EVT, (STATE_MACHINE_FUNC_EX)MeshPeerUcgWhenActivated);

	// MESH_CTRL_UCG state
	StateMachineSetActionEx(Sm, MESH_CTRL_UCG, MESH_CTRL_DISCONNECT, (STATE_MACHINE_FUNC_EX)MeshCtrlDisconnectAtUCGAction);
	StateMachineSetActionEx(Sm, MESH_CTRL_UCG, MESH_PEER_UCG_EVT, (STATE_MACHINE_FUNC_EX)MeshPeerUcgWhenUcg);
	StateMachineSetActionEx(Sm, MESH_CTRL_UCG, MESH_CTRL_MCS_TIMEOUT, (STATE_MACHINE_FUNC_EX)MeshCtrlMcsTimeoutAction);
	...
	IN PVOID SystemSpecific3)
{
	...
	MlmeEnqueueEx(pAd, MESH_CTRL_STATE_MACHINE, MESH_CTRL_PLD, 0, NULL, 0);
    MeshMlmeHandler(pAd);
	...
	IN PVOID SystemSpecific3)
{
	...
	MlmeEnqueueEx(pAd, MESH_CTRL_STATE_MACHINE, MESH_CTRL_MCS_TIMEOUT, 0, NULL, 0);
    MeshMlmeHandler(pAd);
	...
	IN USHORT idx)
{
	...
	{
		MlmeEnqueueEx(pAd, MESH_LINK_MNG_STATE_MACHINE, MESH_LINK_MNG_CNCL, 0, NULL, i);
	}
	
	MlmeEnqueue(pAd, AP_SYNC_STATE_MACHINE, APMT2_MLME_SCAN_CNCL, 0, NULL);
	...
	IN USHORT idx)
{
	...
	for (i = 0; i < MAX_MESH_LINKS; i++)
	{
		if(pAd->MeshTab.MeshLink[i].Entry.Valid)
			MlmeEnqueueEx(pAd, MESH_LINK_MNG_STATE_MACHINE, MESH_LINK_MNG_ACTOPN, 0, NULL, i);
	}
	...
	IN USHORT idx)
{
	...
	RTMPCancelTimer(&pAd->MeshTab.PldTimer, &Cancelled);
	MeshPeerUcgWhenUcg(pAd, Elem, pCurrState, idx);
	*pCurrState = MESH_CTRL_UCG;
	...
	IN PMESH_CONFIGURAION_IE pMeshConfig)
{
	...
	if (pNeighborEntry == NULL)
	{
		...
		if (i < MAX_NEIGHBOR_MP)
		{
			...
			NeighborEntrySet(pNeighborEntry, RealRssi, pPeerMacAddr, pHostName, pMeshId, MeshIdLen,
								Channel, ChBW, ExtChOffset, MeshCipher, pMeshConfig);
			Idx = GetMeshLinkId(pAd, pPeerMacAddr);
	...
	IN PMESH_CONFIGURAION_IE pMeshConfig)
{
	...
	if(strlen(pHostName) > 0 && strlen(pHostName) < MAX_HOST_NAME_LEN)
		strcpy(pNeighbor->HostName, pHostName);
	else
		strcpy(pNeighbor->HostName, DEFAULT_MESH_HOST_NAME);

	// record mesh id.
	if (MeshIdLen > 0)
	{
		NdisMoveMemory(pNeighbor->MeshId, pMeshId, MeshIdLen);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < MAX_NEIGHBOR_MP; i++)
	{
		...
		if(RTMP_TIME_AFTER(Now, pNeighborEntry->LastBeaconTime + MESH_NEIGHBOR_BEACON_IDLE_TIME))
		{
			if (MeshValid(&pAd->MeshTab)
				&& (pNeighborEntry->State == CANDIDATE_MP)
				&& (PeerLinkValidCheck(pAd, pNeighborEntry->MeshLinkIdx) == TRUE))
			{
				MlmeEnqueueEx(pAd, MESH_LINK_MNG_STATE_MACHINE, MESH_LINK_MNG_CNCL, 0, NULL, pNeighborEntry->MeshLinkIdx);
			}

			// some neighbor may not the channel as current Mesh network's.
			// so don't update such neighbors.
			if (pNeighborEntry->Channel == pAd->MeshTab.MeshChannel)
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	else
	{
		...
		for (i = 0; i < pNeighborTab->NeighborNr; i++)
		{
			...
			if ((pNeighborEntry->MeshCapability.field.AcceptPeerLinks)
				&& (pNeighborEntry->State == NEIGHBOR_MP)
	...
	IN RX_BLK *pRxBlk)
{
	...
	if ((LinkIdx = GetMeshLinkId(pAd, pHeader->Addr2)) == BSS_NOT_FOUND)
		return;
	...
	MlmeEnqueueEx(pAd, MESH_CTRL_STATE_MACHINE, MESH_PEER_UCG_EVT, sizeof(MESH_CH_SW_ANN_MSG_STRUCT), &PeerChSwAnn, 0);
	DBGPRINT(RT_DEBUG_TRACE, ("<----- %s\n", __FUNCTION__));
}
		...
		IN PUCHAR pMeshSa)
{
	...
	InsertMeshHeader(pAd, (pOutBuffer + FrameLen), &FrameLen, MeshFlag.word,
		MeshTTL, MeshSeq, pMeshSa, NULL, NULL);
