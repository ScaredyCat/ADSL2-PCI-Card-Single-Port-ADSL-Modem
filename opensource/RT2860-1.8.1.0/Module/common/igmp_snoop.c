	...
	for (i = 0; i< MAX_LEN_OF_MULTICAST_FILTER_TABLE; i++)
	{
		...
		if (pMulticastFilterTable->Content[i].Valid == TRUE)
		{
			...
			while (pMemberEntry)
			{
				...
				printk("Aid=%d, member mac=(%02x:%02x:%02x:%02x:%02x:%02x)\n", pMemberEntry->Aid,
	...
	IN MulticastFilterEntryType type)
{
	...
	do
	{
		...
		{
			InsertIgmpMember(pMulticastFilterTable, &pEntry->MemberList, Aid);
			break;
		}
	...
	do
	{
		...
		if (pEntry && (pMemberAddr != NULL))
		{
			if(APSsPsInquiry(pAd, pMemberAddr, &Sst, &Aid, &PsMode, &Rate))
				DeleteIgmpMember(pMulticastFilterTable, &pEntry->MemberList, Aid);
