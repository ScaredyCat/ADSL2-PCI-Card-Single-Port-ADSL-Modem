...
#define MAX_HOST_NAME_LEN				MAX_HOST_NAME_LENGTH
#define MAX_MESH_ID_LEN					MAX_MESH_ID_LENGTH
...
#define DEFAULT_MESH_HOST_NAME			"CMPC"
...
// Mesh status codes
#define MESH_LINK_ESTABLISHED							11
#define MESH_LINK_CLOSED								12
#define MESH_NO_LISTED_KEY_HOLDER_TRANSPORT_TYPE		13
#define MESH_NO_LISTED_KEY_HOLDER_SECURITY_HANDSHAKE	14
...
typedef struct PACKED _MESH_HEADER
{
	UINT8 MeshFlag;
	UINT8 MeshTTL;
	UINT8 MeshSeq[3];
...
typedef union PACKED _MESH_SECURITY_CONFIGURATION
{
	struct PACKED
	{
#ifdef BIG_ENDIAN
...
typedef union PACKED _MESH_HANDSHAKE_CONTROL
{
	struct PACKED
	{
#ifdef BIG_ENDIAN
...
typedef union _MESH_CAPABILITY
{
	struct
	{
		...
		UINT16 Reserv:10;
		UINT16 MDAEnable:1;
		UINT16 SyncSupportReqFromPeer:1;
		UINT16 SyncActive:1;
		UINT16 SyncEnable:1;
		UINT16 PowerSaveEnable:1;
...
// MSC_IE
typedef struct PACKED _MESH_SECURITY_CAPABILITY_IE
{	
...
// MSA_IE
typedef struct PACKED _MSA_HANDSHAKE_IE
{
	MESH_HANDSHAKE_CONTROL MeshHSControl;
	UCHAR	MA_ID[MAC_ADDR_LEN];
	UCHAR	SelectedAKM[OUI_SUITE_LEN];
	UCHAR	SelectedPairwiseCipher[OUI_SUITE_LEN];
	UCHAR	Octet[0];
...
{
	struct _MESH_PROXY_ENTRY	*pNext;
...
typedef struct _MESH_PROXY_ENTRY_TABLE {
	USHORT				Size;
	PMESH_PROXY_ENTRY	Hash[MAX_HASH_ENTRY_TAB_SIZE];
...
typedef struct PACKED _MESH_DEST_ENTRY
{
	UINT8	PerDestFlag;
    UCHAR   DestAddr[MAC_ADDR_LEN];
	UINT32	Dsn;
