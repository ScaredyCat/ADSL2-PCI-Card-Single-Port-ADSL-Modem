...
#ifdef MESH_SUPPORT
	if (pAd->MacTab.Content[Wcid].ValidAsMesh == TRUE)
	{
		...
		{
            DBGPRINT(RT_DEBUG_INFO, ("I/F(mesh%d) APSendPacket --> Drop packet before mesh secured !!!\n", pMacEntry->MatchMeshTabIdx)); 
            RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE); 
            return (NDIS_STATUS_FAILURE);
        }
	}
...
#if 0
	pHeader_802_11->Sequence   = (pTxBlk->pMacEntry) ? pTxBlk->pMacEntry->TxSeq[pTxBlk->UserPriority] : 
									pAd->Sequence;
#else
	if (pTxBlk->pMacEntry)
	{
...
#ifdef MESH_SUPPORT
		if (pEntry && (pEntry->ValidAsMesh == TRUE))
		{
			...
			{
				// signature exist, it means it's a duplicate packet.
				// so drop it.
				RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
				return;
			}
	...
	if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1))
	{
		do
		{
			...
			if ((VALID_WCID(pRxWI->WirelessCliID)
				&& (pAd->MacTab.Content[pRxWI->WirelessCliID].ValidAsMesh)))
			{
				bMeshPacket = TRUE;
				pEntry = FindMeshEntry(pAd, pRxWI->WirelessCliID, pHeader->Addr2);

				// have no valid mesh entry exist 
		...
		if (bMeshPacket)
		{
			...
			else if (RouteId >= 0)
			{
				...
				RTMP_SET_PACKET_WCID(pForwardPacket,
					pAd->MeshTab.MeshLink[PathMeshLinkIDSearch(pAd, RouteId)].Entry.MacTabMatchWCID);
