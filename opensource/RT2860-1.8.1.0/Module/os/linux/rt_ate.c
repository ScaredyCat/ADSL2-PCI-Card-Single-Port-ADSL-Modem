...
#ifdef CONFIG_AP_SUPPORT 
		// In 2860sta, these steps will fail sometime. But why ?
		//
		// Soft reset, set BBP R21 bit0=1->0,
...
//	ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R66, (0x2E + GET_LNA_GAIN(pAd)));
	...
	if (Channel > 14)
	{
		// When 5.5GHz band the LSB of TxPwr must always be one and shit one bit
		...
		case RACFG_CMD_ATE_START:
			{
            	...
            	{
            		ATEDBGPRINT(RT_DEBUG_TRACE, ("copy_to_user() fail in case RACFG_CMD_ATE_STOP\n"));
                    Status = -EFAULT;
            	}
				else
				{
                	ATEDBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_STOP is done !\n"));
				}
...
VOID CalNoiseLevel(PRTMP_ADAPTER pAd, UCHAR channel, INT32 RSSI[3][10])
{
	...
	//**********************************************************************
	// Read the value of LNA gain annd Rssi offset 
	//**********************************************************************
