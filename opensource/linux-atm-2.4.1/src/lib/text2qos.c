...
/* 507121:linmars Add for supporting ubr+ */
  ...
  struct atm_trafprm *b)
{
    ...
    while (1) {
	if (!**text) return -1;
	// Neeraj rt-vbr cdv is required
	switch (fetch(text,"max_pcr=","pcr=","min_pcr=","max_sdu=","sdu=","scr=","mbs=","cdv=",NULL)) {
	    case 0:
...
int text2qos(const char *text,struct atm_qos *qos,int flags)
{
	...
	item = fetch(&text,"!none","ubr","cbr","nrt-vbr","abr","aal0","aal5","rt-vbr","vbr","ubr+",NULL);
	switch (item) {
	    case 1:
	    case 2:
	    case 3: 
		...
		break;
/* 507121:linmars start */
	    case 9:
                traffic_class = 7;	/* alias for ATM_VBR_NRT */
		break;
/* 507121:linmars end */
	    case 5:
	    case 6:
