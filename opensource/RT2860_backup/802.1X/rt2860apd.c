...
#include "config.h"
//HANK2007/11/20 04:05¤U¤È
#define RT2860AP_SYSTEM_PATH   "/flash/RT2860AP.dat"
...
int RT_ioctl(int sid, int param, char  *data, int data_len, unsigned char apidx, int flags)
{
    char			name[12];
    int				ret = 1;
    struct iwreq	wrq;
...
{                              
	...
	if ((ethertype == ETH_P_PRE_AUTH) || (ethertype == ETH_P_PAE))	
    {
		...
		{
			DBGPRINT(RT_DEBUG_TRACE, "Receive WPA2 pre-auth packet\n");
		}
		else
		{
			DBGPRINT(RT_DEBUG_TRACE, "Receive EAP packet\n");
		}
    }
	else
	{
...
{
    DBGPRINT(RT_DEBUG_TRACE, "USAGE :  rt2860apd \n");
}
...
static rtapd * Apd_init(const char *config_file, int pid)
{
	...
	rtapd->conf = Config_read(rtapd->ioctl_sock);
	if (rtapd->conf == NULL)
    {
...
static void Handle_usr1(int sig, void *eloop_ctx, void *signal_ctx)
{
	...
	for (i = 0; i < rtapds->count; i++)
    {
		...
		newconf = Config_read(rtapd->ioctl_sock);
		if (newconf == NULL)
        {
...
int main(int argc, char *argv[])
{
    ...
    {           
        ...
        interfaces.rtapd[0] = Apd_init(RT2860AP_SYSTEM_PATH, (int)getpid());
        ...
        RT_ioctl(interfaces.rtapd[0]->ioctl_sock, RT_PRIV_IOCTL, (char *)&auth_pid, sizeof(int), 0, RT_SET_APD_PID | OID_GET_SET_TOGGLE);
