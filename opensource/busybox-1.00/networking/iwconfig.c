...
#include "inet_common.h"		/* Header */
#include "busybox.h"		/* Header */
	     ...
	     char *			ifname)
{
  ...
  if(info->b.has_freq)
    {
      iw_print_freq(buffer, sizeof(buffer), info->b.freq, info->b.freq_flags);
      printf("%s  ", buffer);
      tokens +=4;
    }
 ...
 * The main !
 */
int iwconfig_main(int	argc, char **	argv)
{
