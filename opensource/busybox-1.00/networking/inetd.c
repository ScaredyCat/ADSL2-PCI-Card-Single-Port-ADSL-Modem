 ...
 *
 * Nirav
 * IFX_INETD_ENHANCEMENT - These changes are added to make httpd,udhcpd,udhcpr and cli run through inetd. 
 * When there is no request for these daemons, they exit after the configured timeout period, thus making RAM free.
 * When the request comes for one of these daemon, inetd starts it and from then on it function as usual.
...
more:
	...
	sep->se_servicename = inetd_strdup(cp_ptr);
	...
	sep->se_serviceclass = inetd_strdup(cp_ptr);
	...
	sep->se_interface = inetd_strdup(cp_ptr);
	...
	sep->se_sockopts = inetd_strdup(cp_ptr);
#endif
	...
	while(ptemp) {
		if(strcmp(ptemp,"BROADCAST") == 0) {
			if (setsockopt(sep->se_fd, SOL_SOCKET, SO_BROADCAST, (char *) &on, sizeof(on)) == -1) {
				...
				syslog(LOG_ERR, "setsockopt (UNRECOGNIZED): %m");
			return -1;
		}
		ptemp = strtok(NULL,",");
	}
...
#ifdef IFX_INETD_ENHANCEMENT
				strcmp(sep->se_interface,cp->se_interface) == 0 &&
#endif
...
#ifdef IFX_INETD_ENHANCEMENT
			sep = (servtab_t *)xcalloc(1,sizeof (*sep));
#else
...
static void reapchild(int signum)
{
	...
	for (;;) {
		...
		for (sep = servtab; sep; sep = sep->se_next) {
			...
			if(sep->se_pid == pid) {
				servtab_t *tsep = NULL;
				printf("Child %s with pid %d exited\n",sep->se_servicename,pid);
				sep->se_running = 0;	
				sep->se_pid = 0;

				for(tsep = servtab; tsep; tsep = tsep->se_next) {
...
#ifdef IFX_INETD_ENHANCEMENT
		tmout.tv_sec = 10;
		n = select(maxsock + 1, &readable, (fd_set *)0, (fd_set *)0, &tmout);
		if(n == 0)
			goto again;
#else
...
#ifdef IFX_INETD_ENHANCEMENT
					if(pid > 0) {
						printf("Starting child %s with pid %d\n",sep->se_server,pid);
						sep->se_running = 1;
						sep->se_pid = pid;
					}
#endif
