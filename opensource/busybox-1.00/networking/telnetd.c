 ...
 * Modified to support inactivity timeout - Atanu Mondal, 2005/1/12
 *
 ...
 *
 * Atanu Mondal 2005/1/12
 * 	   Support for inactivity timeout  to reclaim resources
 *
 * Nirav
 * IFX_INETD_ENHANCEMENT - These changes are added to make telnetd run through inetd. 
 * When the first telnet session request comes, inetd starts telnetd and from then on,
 * telnetd handles all the successive request of telnet session. Thus, at a time for multiple sessions
 * only one telnetd runs while no. of shells as many as no. of session runs.
 * When no telnet session is connected, the daemon exits making RAM free.
 */
...
/* Required for inactivity timeout */
...
#ifdef CONFIG_FEATURE_TELNETD_INACTIVE_TIMEOUT
//static time_t TIMEOUT=60; /*approx 60 secs*/
static time_t TIMEOUT=2678400; //060706:henryhsu Increase telnetd timeout value for firmware test.
#endif /* CONFIG_FEATURE_TELNETD_INACTIVE_TIMEOUT */
...
#ifdef CONFIG_FEATURE_TELNETD_INACTIVE_TIMEOUT
	time_t timeout_time;/* Absolute value like jiffies which if elapsed will				timeout the session */
#endif	/* CONFIG_FEATURE_TELNETD_INACTIVE_TIMEOUT */
};
