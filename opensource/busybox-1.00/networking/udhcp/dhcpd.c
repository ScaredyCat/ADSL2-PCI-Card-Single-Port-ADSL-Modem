 ...
 *
 * Nirav
 * IFX_INETD_ENHANCEMENT - These changes are added to make udhcpd run through inetd. 
 * When there is no request for udhcpd for pre-configured timeout period, udhcpd quits.
 * When the new request for udhcpd comes, inetd runs udhcpd and from then on udhcpd function as normal.
 */
...
#endif
{
	...
	while(1) { /* loop until universe collapses */
		...
		if (retval == 0) {
			write_leases();
//HANK2007/6/1 06:03¤U¤È			
//#ifdef IFX_INETD_ENHANCEMENT
//			LOG(LOG_INFO, "Timed out. Exiting\n");
//			return(0);
//#else
			timeout_end = time(0) + server_config.auto_time;
			continue;
