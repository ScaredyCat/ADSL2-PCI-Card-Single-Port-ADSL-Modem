...
601261:leejack 2006/01/25 Fix web return empty page issue
609291: jelly, fix bug of auto logout for web page
*/
/*
 ...
 * Note that url encoding for arguments is not the same as html encoding forelse {
 ...
 *
 * Nirav
 * IFX_INETD_ENHANCEMENT - These changes are added to make httpd run through inetd. 
 * When there is no httpd request for the configured timeout period, httpd quits, thus making some RAM free.
 * When the httpd request comes the inetd starts httpd and from then on it function as usual.
*/
...
#include <syslog.h>			/* temp - check if reqd */
#include "busybox.h"
#include "ifx_config.h"
#ifdef IFX_CONFIG_CLI //000004:tc.chen
#define IFX_CLI_REQUEST
#endif //000004:tc.chen
#ifdef IFX_CLI_REQUEST
#warning XXX: IFX_CLI_REQUEST
#include <sys/un.h>
...
#ifdef IFX_WEB_SUPPORT
#include <dlfcn.h>
#include "ifx_exportFunc.h"
#endif
#define DEFAULT_WEB_PAGE    "login.asp"
#define WEB_PATH_MAX	128
...
static char web_pwd[WEB_PATH_MAX];
...
//#define fprintf(fd, fmt, args...)	{ openlog(bb_applet_name,0,LOG_USER);  syslog(LOG_ERR, fmt, ##args); closelog(); }

#define fprintf(fd,fmt,args...)		{syslog(LOG_ERR, fmt, ##args); }
...
#endif
#if !defined(CONFIG_FEATURE_HTTPD_USAGE_FROM_INETD_ONLY) || defined(IFX_INETD_ENHANCEMENT)
  int accepted_socket;
...
#ifdef IFX_WEB_SUPPORT
	char *host;	// Added for redirection support
#endif
	char *usr_agent_str; // Added for Version Control, 000001:Nirav
} HttpdConfig;
...
/* Warning: shorted equalent suffix in one line must be first */
  ...
  ".mp3", "audio/mpeg",
#ifdef IFX_WEB_SUPPORT
   ".asp", "text/html",  /* IFX added for ASP support */
   ".js", "text/html",  /* IFX added for JavaScript support */
#endif
#if 0                        /* unpopular */
  ".au", "audio/basic",
...
static int openCLIServer(void)
{
  ...
  if (bind(fd, (struct sockaddr *)&local, len) == 0) {
	...
	bb_perror_msg_and_die("cli bind");
    }
  } else {
	bb_perror_msg_and_die("cli create socket");
  }

  return fd;
}
#endif /* IFX_CLI_REQUEST */
 ...
 *
 ****************************************************************************/
int sendHeaders(HttpResponseNum responseNum,char *redirectURL)
{
  char *buf = config->buf;
...
#if 1
	if(config->found_mime_type == NULL){
  ...
  if(responseNum == 302){
    len = sprintf(buf,
      ...
      "Connection: close\r\nLocation: %s\r\n",
      responseNum, responseString, config->found_mime_type, timeStr,redirectURL);
  }
  else {
    len = sprintf(buf,
      "HTTP/1.0 %d %s\nContent-type: %s\r\nDate: %s\r\n"
...
#endif
      sendHeaders(HTTP_NOT_FOUND,NULL);
      _exit(242);
    } /* end child */
 ...
 *
 ****************************************************************************/
static int sendFile(const char *url,const char *urlArgs)
{
  char * suffix;
  int  f;
  ...
  if (f >= 0) {
    ...
    sendHeaders(HTTP_OK,NULL);
#ifdef IFX_WEB_SUPPORT
    memset(buf,0x00,MAX_MEMORY_BUFF);
		bNewFile = 1;
    while ((count = bb_full_read(f, buf, MAX_MEMORY_BUFF-1)) > 0) {
      if ((strstr(config->found_mime_type,"html") != NULL ) || (strstr(url,"asp") != NULL)
#ifdef IFX_CLI_REQUEST
			  || (strstr(url,"cli") != NULL)
#endif
			  ){
				//ifx_psendDHTMLFile
				writtenCount = (*ifx_psendDHTMLFile)(buf,config->host,count,a_c_w,&bNewFile);
        if (writtenCount != count && writtenCount >= 0)  {
          if(lseek(f,writtenCount - count,SEEK_CUR) == -1){
            fprintf(stderr,"lseek failed ERROR\n");
          }
        }
        ifx_debug_printf("#### Sent [%d] bytes\n",writtenCount);
      }
      else	{
...
#ifdef CONFIG_FEATURE_HTTPD_CGI
  /* Need to initialize referer to NULL so that previous 'free-d'
	 values are not carried over. Causes memory corruption */
  config->referer = NULL; //XXX : Added by Nirav.
#endif
#ifdef IFX_WEB_SUPPORT
  config->host = NULL;
#endif
...
//  config->remoteuser = NULL;
  ...
  config->found_mime_type = NULL; //Nirav : Fix to make sure that previos content_type is reset.
  config->ContentLength = -1;

  do {
    int  count;
    if (getLine() <= 0)
	return; // 609291: jelly, fix bug of auto logout for web page
	//break;  /* closed */
    ...
    if(purl == NULL) {
BAD_REQUEST:
      sendHeaders(HTTP_BAD_REQUEST,NULL);
      break;
    }
    *purl = 0;
#ifdef CONFIG_FEATURE_HTTPD_CGI
	...
	prequest = "POST";
	if(strcasecmp(buf, prequest) != 0) {
	    sendHeaders(HTTP_NOT_IMPLEMENTED,NULL);
	    break;
	}
    }
#else
    ...
    *purl = ' ';
    ...
    if(url == NULL) {
	sendHeaders(HTTP_INTERNAL_SERVER_ERROR,NULL);
	break;
    }
    strcpy(url, buf);
...
#endif		/* 000001:Nirav start */
      else if ((strncasecmp(buf, "User-Agent:", 11) == 0))  {
        for(test = buf + 11; isspace(*test); test++)
	      ;
	    config->usr_agent_str = strdup(test);
      }	/* 000001:Nirav end */
#endif
...
FORBIDDEN:      /* protect listing /cgi-bin */
#endif
		sendHeaders(HTTP_FORBIDDEN,NULL);
		break;
    }

#ifdef CONFIG_FEATURE_HTTPD_BASIC_AUTH
    if (credentials <= 0 && checkPerm(url, ":") == 0) {
      sendHeaders(HTTP_UNAUTHORIZED,NULL);
      break;
    }
#endif
    ...
    if(ifx_AuthHandler_ptr){
		...
		if(ifx_clireq == 0)
#endif
			if((*ifx_AuthHandler_ptr)(test,a_c_r,a_c_w) == 1)	/* 000001:Nirav */
				break;
    }
#endif
...
#ifdef IFX_WEB_SUPPORT
    else if (strncmp(test, "goform", 6) == 0){
      if(test[7] == '/' && test[8] == 0)
        goto FORBIDDEN;     // protect listing cgi-bin/

	  config->ContentLength = -1;	//Nirav: 

#if 0
      sendGoform(url, prequest, urlArgs, length, cookie, content_type);
#else
      if(ifx_psendGoform)
        (*ifx_psendGoform)(url, prequest, urlArgs, length, cookie, content_type,config->host,a_c_r,a_c_w);
      else
        goto FORBIDDEN;
#endif /* #if 0 */
    } 
#endif /* ] IFX_WEB_SUPPORT */
    else {
      if (prequest != request_GET)
        sendHeaders(HTTP_NOT_IMPLEMENTED,NULL);
      else {
...
#ifdef IFX_WEB_SUPPORT
  IFX_FREE(cookie);
  IFX_FREE(content_type);
  IFX_FREE(config->referer);
  IFX_FREE(config->host);
  IFX_FREE(config->usr_agent_str);
#else
  free(cookie);
  free(content_type);
  free(config->referer);
  free(config->usr_agent_str);
#endif
#ifdef CONFIG_FEATURE_HTTPD_BASIC_AUTH
  #ifdef IFX_WEB_SUPPORT
    IFX_FREE(config->remoteuser);
  #else
...
static int miniHttpd(int server)
{
  ...
  }else
  {
    ...
    ifx_exportedFunc.ifx_read = (IFX_READ)bb_full_read;
    ifx_exportedFunc.ifx_write = (IFX_WRITE)bb_full_write;
    ifx_exportedFunc.ifx_getHeaderData = (IFX_GETHEADERDATA)getHeaderData;
    ifx_exportedFunc.dlHandle = dlHandle;
    ifx_debug_printf("### Try to call ifx_RegisterFunc.\n");
    //(*(int (*)(int,struct ifx_wp *,int,char **))ifx_function_ptr)(0,&ifx_wp_data,no_of_arg,arg_list);
    i = (*ifx_function_ptr)(&ifx_exportedFunc);
    if(i < 0){
      ifx_debug_printf("###ifx_RegisterFunc returned error %d\n",i);
      dlclose(dlHandle);
      dlHandle = NULL;
    }else{
      ...
      if ((error = dlerror()) != NULL){
        fprintf (stderr, "%s\n", error);
        fprintf(stderr,"Error : dlsym fails. Cannot find ifx_sendGoform\n");
      }
      ifx_psendDHTMLFile = dlsym(dlHandle,"ifx_sendDHTMLFile");
      if ((error = dlerror()) != NULL){
        fprintf (stderr, "%s\n", error);
        fprintf(stderr,"Error : dlsym fails. Cannot find ifx_sendDHTMLFile\n");
      }
      ifx_AuthHandler_ptr = dlsym(dlHandle,"ifx_AuthHandler");
      if ((error = dlerror()) != NULL)
      {
        ifx_debug_printf("%s\n", error);
        ifx_debug_printf("Error : dlsym fails. Cannot find ifx_AuthHandler\n");
      }
    }
  }
...
#ifdef IFX_CLI_REQUEST
  FD_SET(cli_server, &portfd);
#endif
  while (1) {
	  /* copy the ports we are watching to the readfd set */
    readfd = portfd;

	timeout_val.tv_sec = IFX_HTTPD_INACTIVITY_TIMEOUT;
	timeout_val.tv_usec = 0;
    /* Now wait INDEFINATELY on the set of sockets! */
#ifdef IFX_CLI_REQUEST
	ifx_clireq = 0;
    selret = select((server > cli_server ? server : cli_server) + 1, &readfd, 0, 0, &timeout_val);
#else
    selret = select(server + 1, &readfd, 0, 0, &timeout_val);
#endif
	if(selret > 0)
	{
...
#ifdef IFX_WEB_SUPPORT
    /* XXX: Make everything single context - reduces httpd to an iterative sever
    But this is how our current config app runs, so probably we live with
    it */
#if 1
#define fork() 	0
#define exit(a)	a
...
#ifdef IFX_CLI_REQUEST 
	  else if (FD_ISSET(cli_server, &readfd)) 
	  {
		  int s,t;
		  int on;
		  struct sockaddr_un remote;
		  
		  ifx_debug_printf("CLI request received\n");
		  t = sizeof(remote);
		  s = accept(cli_server, (struct sockaddr *)&remote, &t);
...
#undef signal
#else
		  printf("Closing the connection with cli_be_sock\n");
		  close(s);
#endif
	  }
#endif /* IFX_CLI_REQUEST */
...
static int miniHttpd(void)
{
  ...
  sprintf(config->rmt_ip_str, "%u.%u.%u.%u",
		...
		(unsigned char)(config->rmt_ip >> 8),
				config->rmt_ip & 0xff);
#endif
  ifx_debug_printf("%s",config->rmt_ip_str);
...
# endif
...
#ifdef IFX_CLI_REQUEST
  ifx_debug_printf("CLI home found at %s\n",home_cli);
#endif
  ...
  memset(web_pwd,0x00,sizeof(web_pwd));
  getcwd(web_pwd,sizeof(web_pwd));
  ...
  chdir(web_pwd);
  if(chdir(home_cli)) {
	bb_perror_msg_and_die("can`t chdir to %s",home_cli);
  }
  chdir(web_pwd);
#endif
#ifndef CONFIG_FEATURE_HTTPD_USAGE_FROM_INETD_ONLY
...
#elif defined(IFX_INETD_ENHANCEMENT)
  if(strcmp(req_type,"httpd") == 0) {
	  server = 0;
#ifdef IFX_CLI_REQUEST
	  cli_server = openCLIServer();
#endif
