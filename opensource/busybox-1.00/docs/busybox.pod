...
addgroup [B<-g> GID] group_name [user_name]
...
arping [B<-fqbDUA>] [B<-c> count] [B<-w> timeout] [B<-I> device] [B<-s> sender] target
...
crontab [B<-c> dir] {file|-}|[B<-u>|B<-l>|B<-e>|B<-d> user]
...
fdisk [B<-luv>] [B<-C> CYLINDERS] [B<-H> HEADS] [B<-S> SECTORS] [B<-b> SSZ] DISK
...
Options:	B<-a>   get/set fs readahead
...
httpd [B<-c> <conf file>] [B<-p> <port>] [B<-u> user] [B<-r> <realm>] [B<-m> pass] [B<-h> home] [B<-d>/B<-e> <string>]
...
hwclock [B<-r>|-B<-show>] [B<-s>|-B<-hctosys>] [B<-w>|-B<-systohc>] [B<-l>|-B<-localtime>] [B<-u>|-B<-utc>]
...
install [B<-cgmops>] [sources] <dest|directory>
...
iwconfig [B<-a>] <interface> [<address>]
...
killall [B<-q>] [B<-signal>] process-name [process-name ...]
...
or: losetup [OPTION]... B<-d> LOOPDEVICE
...
od [B<-aBbcDdeFfHhIiLlOovXx>] [FILE]
...
run-parts [B<-t>] [B<-a> ARG] [B<-u> MASK] DIRECTORY
...
start-stop-daemon [OPTIONS] [-B<-start>|-B<-stop>] ... [-- arguments...]
...
tar -[czjZxtvO] [B<-X> FILE][B<-f> TARFILE] [B<-C> DIR] [FILE(s)] ...
...
udhcpc [B<-fbnqv>] [B<-c> CLIENTID] [B<-H> HOSTNAME] [B<-i> INTERFACE]
[B<-p> pidfile] [B<-r> IP] [B<-s> script]
...
wget [B<-c>|-B<-continue>] [B<-q>|-B<-quiet>] [B<-O>|-B<-output>-document file]
		[-B<-header> 'header: value'] [B<-Y>|-B<-proxy> on/off] [B<-P> DIR] url
