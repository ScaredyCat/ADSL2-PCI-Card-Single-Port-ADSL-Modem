...
void usage(void)
{
	printf("Usage: wscd [-a ipaddress] [-p port] [-f descDoc] [-w webRootDir] -m UPnPOpMode -D [-d debugLevel] -h\n");
...
int main(int argc, char **argv)
{
	...
	char *ipAddr = NULL, *descDoc = NULL, *webRootDir = NULL;
	...
	while((opt = getopt(argc, argv, "a:p:f:w:m:d:Dh"))!= -1)
	{
		switch (opt)
		{
			case 'a':
