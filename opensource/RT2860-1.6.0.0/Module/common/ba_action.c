...
#ifdef CONFIG_AP_SUPPORT
#ifdef APCLI_SUPPORT
		if (pAd->MacTab.Content[pBAEntry->Wcid].ValidAsApCli)
		{			
...
#endif // APCLI_SUPPORT //			
#ifdef WDS_SUPPORT
		if (pAd->MacTab.Content[pBAEntry->Wcid].ValidAsWDS)
		{			
					   ...
					   IN PFRAME_ADDBA_REQ pFrame)
{
	...
	if (pBAEntry != NULL)
	{
		...
		offset = MAC_WCID_BASE + pEntry->Aid * HW_WCID_ENTRY_SIZE + 4;
						 ...
						 IN      BOOLEAN         bPassive)
{
	...
	if ((TID == pBAEntry->TID) && (pBAEntry->REC_BA_Status == Recipient_Accept))
	{
		...
		offset = MAC_WCID_BASE + (pBAEntry->Wcid) * HW_WCID_ENTRY_SIZE + 4;
		RTMP_IO_READ32(pAd, offset, &VALUE);
		// bitmap field starts at 0x10000 in ASIC WCID table
		VALUE &= (~(0x10000 << pBAEntry->TID));
		RTMP_IO_WRITE32(pAd, offset, VALUE);
...
#endif // APCLI_SUPPORT //
#ifdef WDS_SUPPORT
	if (pAd->MacTab.Content[Elem->Wcid].ValidAsWDS)
	{
...
#ifdef BIG_ENDIAN
	*(USHORT *)(&ADDframe.BaParm) = SWAP16(*(USHORT *)(&ADDframe.BaParm));
	ADDframe.StatusCode = SWAP16(ADDframe.StatusCode);
	ADDframe.TimeOutValue = SWAP16(ADDframe.TimeOutValue);
#endif
...

{
	...
	//hex_dump("AddBARsp, Elem->Msg, Elem->MsgLen);
...
#ifdef CONFIG_AP_SUPPORT
#ifdef APCLI_SUPPORT
	if (pAd->MacTab.Content[Wcid].ValidAsApCli)
	{
...
#endif // APCLI_SUPPORT //
#ifdef WDS_SUPPORT
	if (pAd->MacTab.Content[Wcid].ValidAsWDS)
	{
				   ...
				   IN UCHAR				Wcid)
{
	...
	MakeOutgoingFrame(pOutBuffer,               &FrameLen,
					  sizeof(FRAME_RADIO_ACTION),      &Frame,
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU) &&  (pRxBlk->DataSize > 1520))
	{
		static int err_size;
