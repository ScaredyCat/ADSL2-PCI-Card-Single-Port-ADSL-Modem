	...
	IN  UCHAR			FrameGap)
{
...
#endif
	for (i=0; i<TXWI_SIZE; i++)  // 16-byte TXWI field
	{
		RTMP_IO_WRITE8(pAd, CTSBaseAddr + i, *ptr);
		ptr ++;
	}
	...
	for (i=0; i<10; i++)
	{
		RTMP_IO_WRITE8(pAd, CTSBaseAddr + TXWI_SIZE + i, *ptr);
		ptr ++;
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (OneSecTotalXmitCnt > 7500)
	{
		Idx = 3;
	}
	else if (OneSecTotalXmitCnt > 6500)
	{
		Idx = 2;
	}
	else if (OneSecTotalXmitCnt > 4000)
	{
		Idx = 1;
	}
	else
