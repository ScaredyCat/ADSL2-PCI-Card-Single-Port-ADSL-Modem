			...
			switch (Elem->Machine) 
			{
				...
				case APCLI_AUTH_STATE_MACHINE:
					...
					if(isValidApCliIf(apcliIfIndex))
						ApCliStateMachinePerformAction(pAd, &pAd->Mlme.ApCliAuthMachine, Elem,
				...
				case APCLI_ASSOC_STATE_MACHINE:
					apcliIfIndex = Elem->ifIndex;
					if(isValidApCliIf(apcliIfIndex))		
						ApCliStateMachinePerformAction(pAd, &pAd->Mlme.ApCliAssocMachine, Elem,
				...
				case APCLI_SYNC_STATE_MACHINE:
					apcliIfIndex = Elem->ifIndex;
					if(isValidApCliIf(apcliIfIndex))
						ApCliStateMachinePerformAction(pAd, &pAd->Mlme.ApCliSyncMachine, Elem,
				...
				case APCLI_CTRL_STATE_MACHINE:
					apcliIfIndex = Elem->ifIndex;
					if(isValidApCliIf(apcliIfIndex))
						ApCliStateMachinePerformAction(pAd, &pAd->Mlme.ApCliCtrlMachine, Elem,
				...
				case APCLI_WPA_STATE_MACHINE:
					apcliIfIndex = Elem->ifIndex;
					if(isValidApCliIf(apcliIfIndex))
						ApCliStateMachinePerformAction(pAd, &pAd->Mlme.ApCliWpaPskMachine, Elem,
...
//	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);
	...
	// execute every 500ms 
	if ((pAd->Mlme.PeriodicRound % 5 == 0) && (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_TX_RATE_SWITCH_ENABLED)))
	{
#ifdef CONFIG_AP_SUPPORT
...
//===========================================================================
	...
	pAd->CommonCfg.MinHTPhyMode.word = 0;
	pAd->CommonCfg.MaxHTPhyMode.word = 0;
	pAd->CommonCfg.HTPhyMode.word = 0;
	...
	{
		pAd->CommonCfg.TxRate = pAd->CommonCfg.MaxTxRate;
		pAd->CommonCfg.HTPhyMode.field.MCS	= (pAd->CommonCfg.MaxTxRate > 3) ? (pAd->CommonCfg.MaxTxRate - 3) : pAd->CommonCfg.MaxTxRate;
		pAd->CommonCfg.HTPhyMode.field.MODE	= (pAd->CommonCfg.MaxTxRate > 3) ? MODE_OFDM : MODE_CCK;
		
		pAd->MacTab.Content[BSSID_WCID].HTPhyMode.field.STBC	= pAd->CommonCfg.HTPhyMode.field.STBC;
		pAd->MacTab.Content[BSSID_WCID].HTPhyMode.field.ShortGI	= pAd->CommonCfg.HTPhyMode.field.ShortGI;
		pAd->MacTab.Content[BSSID_WCID].HTPhyMode.field.MCS		= pAd->CommonCfg.HTPhyMode.field.MCS;
		pAd->MacTab.Content[BSSID_WCID].HTPhyMode.field.MODE	= pAd->CommonCfg.HTPhyMode.field.MODE;
	}


	if (pAd->CommonCfg.TxRate <= RATE_11)
	{
		pAd->CommonCfg.MaxHTPhyMode.field.MODE = MODE_CCK;
		pAd->CommonCfg.MaxHTPhyMode.field.MCS = pAd->CommonCfg.TxRate;
		pAd->CommonCfg.MinHTPhyMode.field.MCS = pAd->CommonCfg.MinTxRate;
	}
	else
	{
		pAd->CommonCfg.MaxHTPhyMode.field.MODE = MODE_OFDM;
		pAd->CommonCfg.MaxHTPhyMode.field.MCS = OfdmRateToRxwiMCS[pAd->CommonCfg.TxRate];
		
		if (pAd->CommonCfg.MinTxRate >= RATE_6 && (pAd->CommonCfg.MinTxRate <= RATE_54))
			{pAd->CommonCfg.MinHTPhyMode.field.MCS = OfdmRateToRxwiMCS[pAd->CommonCfg.MinTxRate];}
		else
			{pAd->CommonCfg.MinHTPhyMode.field.MCS = pAd->CommonCfg.MinTxRate;}
	}

	pAd->CommonCfg.HTPhyMode.word = (pAd->CommonCfg.MaxHTPhyMode.word);
	if (bLinkUp && (pAd->OpMode == OPMODE_STA))
	{
			pAd->MacTab.Content[BSSID_WCID].HTPhyMode.word = pAd->CommonCfg.HTPhyMode.word;
			pAd->MacTab.Content[BSSID_WCID].MaxHTPhyMode.word = pAd->CommonCfg.MaxHTPhyMode.word;
			pAd->MacTab.Content[BSSID_WCID].MinHTPhyMode.word = pAd->CommonCfg.MinHTPhyMode.word;
	}
	else
	{
	...
	IN UCHAR		 Wcid)
{
	...
	{
		if (pAd->CommonCfg.DesiredHtPhy.bHtEnable == FALSE)
			return;
		...
		if ((pAd->CommonCfg.DesiredHtPhy.TxSTBC) && (pRtHtCap->RxSTBC) && (pAd->Antenna.field.TxPath == 2))
			pAd->CommonCfg.MaxHTPhyMode.field.STBC = STBC_USE;
		else
			pAd->CommonCfg.MaxHTPhyMode.field.STBC = STBC_NONE;
	}
	
	// Decide MAX ht rate.
	if ((pRtHtCap->GF) && (pAd->CommonCfg.DesiredHtPhy.GF))
		pAd->CommonCfg.MaxHTPhyMode.field.MODE = MODE_HTGREENFIELD;
	else
		pAd->CommonCfg.MaxHTPhyMode.field.MODE = MODE_HTMIX;
	
	if ((pAd->CommonCfg.DesiredHtPhy.ChannelWidth) && (pRtHtCap->ChannelWidth))
		pAd->CommonCfg.MaxHTPhyMode.field.BW = BW_40;
	else
		pAd->CommonCfg.MaxHTPhyMode.field.BW = BW_20;

	if (pAd->CommonCfg.MaxHTPhyMode.field.BW == BW_20)
		pAd->CommonCfg.MaxHTPhyMode.field.ShortGI = (pAd->CommonCfg.DesiredHtPhy.ShortGIfor20&pRtHtCap->ShortGIfor20);
	else
		pAd->CommonCfg.MaxHTPhyMode.field.ShortGI = (pAd->CommonCfg.DesiredHtPhy.ShortGIfor40&pRtHtCap->ShortGIfor40);

	for  (i=0;i<8;i++)		
	{
		if ((pRtHtCap->MCSSet[1]&(0x80>>i)) && (pAd->CommonCfg.DesiredHtPhy.MCSSet[1]&(0x80>>i)))
		{
			pAd->CommonCfg.MaxHTPhyMode.field.MCS = 15-i;
			break;
		}
	}
	if (i == 8)
	{
		for  (i=0;i<8;i++)		
		{
			if ((pRtHtCap->MCSSet[0]&(0x80>>i))&& (pAd->CommonCfg.DesiredHtPhy.MCSSet[0]&(0x80>>i)))
			{
				pAd->CommonCfg.MaxHTPhyMode.field.MCS = 7-i;
				break;
			}
		}
	}
	...
	pAd->CommonCfg.MinHTPhyMode.field.BW = BW_20;
	pAd->CommonCfg.MinHTPhyMode.field.MCS = 0;
	pAd->CommonCfg.MinHTPhyMode.field.STBC = 0;
	pAd->CommonCfg.MinHTPhyMode.field.ShortGI = 0;
	//If STA assigns fixed rate. update to fixed here.
	
	
	// Decide ht rate
	pAd->CommonCfg.HTPhyMode.field.STBC = (pAd->CommonCfg.MaxHTPhyMode.field.STBC);
	pAd->CommonCfg.HTPhyMode.field.BW = (pAd->CommonCfg.MaxHTPhyMode.field.BW);
	pAd->CommonCfg.HTPhyMode.field.MODE = pAd->CommonCfg.MaxHTPhyMode.field.MODE;
	pAd->CommonCfg.HTPhyMode.field.MCS = pAd->CommonCfg.MaxHTPhyMode.field.MCS;
	pAd->CommonCfg.HTPhyMode.field.ShortGI = pAd->CommonCfg.MaxHTPhyMode.field.ShortGI;

	// if this is a station and connects to a AP, Or if this is a adhoc,  copy to MACtable. 
	if (Wcid < MAX_LEN_OF_MAC_TABLE)
	{     // 1. 802.11n rate, 2.Mpdudensity, 3.Amsdu size, 4. MimoPowerSave, 5. Max Rx Factor
		pAd->MacTab.Content[Wcid].HTPhyMode.word = pAd->CommonCfg.HTPhyMode.word;
		pAd->MacTab.Content[Wcid].MaxHTPhyMode.word = pAd->CommonCfg.MaxHTPhyMode.word;
		pAd->MacTab.Content[Wcid].MinHTPhyMode.word = pAd->CommonCfg.MinHTPhyMode.word;
	}
	// use default now. rt2860
	//RTMP_IO_WRITE32(pAd, HT_BASIC_RATE, BasicMCS);
	if (pAd->CommonCfg.DesiredHtPhy.MCSSet[0] != 0xff)
		pAd->CommonCfg.bAutoTxRateSwitch = FALSE;
	else
		pAd->CommonCfg.bAutoTxRateSwitch = TRUE;
	
	DBGPRINT(RT_DEBUG_TRACE, (" MlmeUpdateHtTxRates<---.AMsduSize = %d  \n", pAd->CommonCfg.DesiredHtPhy.AmsduSize ));
	DBGPRINT(RT_DEBUG_TRACE,("TX: MCS[0] = %x (choose %d), BW = %d, ShortGI = %d, MODE = %d,  \n", pRtHtCap->MCSSet[0],pAd->CommonCfg.HTPhyMode.field.MCS,
		pAd->CommonCfg.HTPhyMode.field.BW, pAd->CommonCfg.HTPhyMode.field.ShortGI, pAd->CommonCfg.HTPhyMode.field.MODE));
	DBGPRINT(RT_DEBUG_TRACE,("MlmeUpdateHtTxRates<=== \n"));
}
...
//	RTMP_IO_WRITE32(pAd, PWR_PIN_CFG, 0x0C);		  // Turn off BBP, RF

//	RTMP_IO_WRITE32(pAd, WPDMA_RST_IDX, 0x01003f);		  // reset DMA
}
...
//	RTMP_IO_WRITE32(pAd, PWR_PIN_CFG, 0x03);		  // Turn on BBP, RF
	...
	IN	BOOLEAN			bScan) 
{
	...
	UCHAR index;//, Value; //BbpReg, Value;
	UINT32	Value;
	...
	if (Channel <= 14)
	{
		...
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R86, (0x44 - GET_LNA_GAIN(pAd)));
	...
	IN USHORT TbttNumToNextWakeUp) 
{
	...
	AsicSendCommandToMcu(pAd, 0x30, 0xff, 0x00, 0x02);   // send POWER-SAVE command to MCU. Timeout 100us.
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	ptr = pAd->BeaconBuf;
	for (i=0; i< pAd->BeaconTxWI.MPDUtotalByteCount; i++)
	{
...
//#ifndef WIFI_TEST
		if( pAd->CommonCfg.bEnableTxBurst )		
			Ac0Cfg.field.AcTxop = 0x30; //cpu2le16(0x30); // Suggest by John for TxBurst in HT Mode
		else
...
//#endif					
		...
		{
			Ac2Cfg.field.AcTxop = 192; //cpu2le16(192);	// AC_VI: 192*32us ~= 6ms
			Ac3Cfg.field.AcTxop = 96; //cpu2le16(96);		// AC_VO: 96*32us  ~= 3ms
		}
		else
		{
			Ac2Cfg.field.AcTxop = 96; //cpu2le16(96);		// AC_VI: 96*32us ~= 3ms
			Ac3Cfg.field.AcTxop = 48; //cpu2le16(48);		// AC_VO: 48*32us ~= 1.5ms
		}
		...
		if (pAd->CommonCfg.PhyMode == PHY_11B)
		{
			csr1.field.Ac2Txop = 192;//cpu2le16(192);		// AC_VI: 192*32us ~= 6ms
			csr1.field.Ac3Txop = 96;//cpu2le16(96);		// AC_VO: 96*32us  ~= 3ms
		}
		else
		{
			csr1.field.Ac2Txop = 96;//cpu2le16(96);		// AC_VI: 96*32us ~= 3ms
			csr1.field.Ac3Txop = 48;//cpu2le16(48);		// AC_VO: 48*32us ~= 1.5ms
		}
	...
	else
	{
		...
		//Ac0Cfg.field.AcTxop =  pEdcaParm->Txop[QID_AC_BE] ? cpu2le16(pEdcaParm->Txop[QID_AC_BE]) : 0x20;
		Ac0Cfg.field.Cwmin= pEdcaParm->Cwmin[QID_AC_BE];
		Ac0Cfg.field.Cwmax = pEdcaParm->Cwmax[QID_AC_BE];
		Ac0Cfg.field.Aifsn = pEdcaParm->Aifsn[QID_AC_BE]; //+1;

		Ac1Cfg.field.AcTxop =  pEdcaParm->Txop[QID_AC_BK];
		//Ac1Cfg.field.AcTxop =  pEdcaParm->Txop[QID_AC_BK] ? cpu2le16(pEdcaParm->Txop[QID_AC_BK]) : 0x10;
		Ac1Cfg.field.Cwmin = pEdcaParm->Cwmin[QID_AC_BK]; //+2; 
		Ac1Cfg.field.Cwmax = pEdcaParm->Cwmax[QID_AC_BK];
		//Ac1Cfg.field.Aifsn = (pEdcaParm->Aifsn[QID_AC_BK] < 5) ? (pEdcaParm->Aifsn[QID_AC_BK]) : 5;
...
//#endif // WIFI_TEST //
		...
		//      DMA Register has a copy too.
		//========================================================
		csr0.field.Ac0Txop = Ac0Cfg.field.AcTxop; //cpu2le16(pEdcaParm->Txop[QID_AC_BE]);					
		csr0.field.Ac1Txop = Ac1Cfg.field.AcTxop; //cpu2le16(pEdcaParm->Txop[QID_AC_BK]);
		RTMP_IO_WRITE32(pAd, WMM_TXOP0_CFG, csr0.word);

		csr1.field.Ac2Txop = Ac2Cfg.field.AcTxop; // cpu2le16(pEdcaParm->Txop[QID_AC_VI]);
		csr1.field.Ac3Txop = Ac3Cfg.field.AcTxop; //cpu2le16(pEdcaParm->Txop[QID_AC_VO]);
		RTMP_IO_WRITE32(pAd, WMM_TXOP1_CFG, csr1.word);
