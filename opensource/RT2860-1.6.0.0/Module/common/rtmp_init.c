...
#if 0
#if ME_98		
		NdisAllocateSpinLock(&pAd->DispatchLock);
#endif
#endif
	...
	IN	PUCHAR			mac_addr)
{
	...
	UCHAR			t1,t2,t3,t4;
//#ifdef WIFI_TEST	
	ULONG			default_tx_power = 0x66666666;
//#endif // WIFI_TEST //
	...
	DBGPRINT(RT_DEBUG_TRACE, ("--> E2PROM_CSR = 0x%lx\n", data));
	...
	else
	{
		...
		{
			AtoH(macptr, &pAd->PermanentAddress[j], 2);
			macptr=macptr+3;
		}	
	...
	value = RTMP_EEPROM_READ16(pAd, EEPROM_NIC1_OFFSET);
	pAd->EEPROMDefaultValue[0] = value;

	value = RTMP_EEPROM_READ16(pAd, EEPROM_NIC2_OFFSET);
	pAd->EEPROMDefaultValue[1] = value;

	value = RTMP_EEPROM_READ16(pAd, 0x38);	// Country Region
	...
	// 0. 11b/g
	{
		...
		Power.word = RTMP_EEPROM_READ16(pAd, 0x6E);
		pAd->TssiMinusBoundaryG[4] = Power.field.Byte0;
		pAd->TssiMinusBoundaryG[3] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0x70);
		pAd->TssiMinusBoundaryG[2] = Power.field.Byte0;
		pAd->TssiMinusBoundaryG[1] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0x72);
		pAd->TssiRefG   = Power.field.Byte0; /* reference value [0] */
		pAd->TssiPlusBoundaryG[1] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0x74);
		pAd->TssiPlusBoundaryG[2] = Power.field.Byte0;
		pAd->TssiPlusBoundaryG[3] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0x76);
	...
	// 1. 11a
	{
		Power.word = RTMP_EEPROM_READ16(pAd, 0xD4);
		pAd->TssiMinusBoundaryA[4] = Power.field.Byte0;
		pAd->TssiMinusBoundaryA[3] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0xD6);
		pAd->TssiMinusBoundaryA[2] = Power.field.Byte0;
		pAd->TssiMinusBoundaryA[1] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0xD8);
		pAd->TssiRefA   = Power.field.Byte0;
		pAd->TssiPlusBoundaryA[1] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0xDA);
		pAd->TssiPlusBoundaryA[2] = Power.field.Byte0;
		pAd->TssiPlusBoundaryA[3] = Power.field.Byte1;
		Power.word = RTMP_EEPROM_READ16(pAd, 0xDC);
	...
	value = RTMP_EEPROM_READ16(pAd, EEPROM_RSSI_BG_OFFSET+2);
	pAd->BGRssiOffset2 = value & 0x00ff;
	pAd->ALNAGain1 = (value >> 8);
	value = RTMP_EEPROM_READ16(pAd, EEPROM_LNA_OFFSET);
	...
	value = RTMP_EEPROM_READ16(pAd, (EEPROM_RSSI_A_OFFSET+2));
	...
	// Get LED Setting.
	//
	value = RTMP_EEPROM_READ16(pAd, 0x3a);
	pAd->LedCntl.word = (value&0xff00) >> 8;
	value = RTMP_EEPROM_READ16(pAd, EEPROM_LED1_OFFSET);
	pAd->Led1 = value;
	value = RTMP_EEPROM_READ16(pAd, EEPROM_LED2_OFFSET);
	pAd->Led2 = value;
	value = RTMP_EEPROM_READ16(pAd, EEPROM_LED3_OFFSET);
	pAd->Led3 = value;
		
	//
	// Get Txpower per Rate & power delta for 40MHz.
	//
	DBGPRINT(RT_DEBUG_TRACE, ("Txpower per Rate\n"));
#if 0
	for (i=0; i<5; i++)
	{
		value = RTMP_EEPROM_READ16(pAd, EEPROM_TXPOWER_BYRATE + i*4);
		data = value;
		...
		DBGPRINT_RAW(RT_DEBUG_TRACE, ("%x ", data));
	}
#else
	/* default tx power =
	   0x66666666, 0x66666666, 0x66666666, 0x66666666, 0x66666666

	   adjust delta power based on reference tempature =
	   0x66666666, 0x77776666, 0x77775566, 0x66665566, 0xffff6666,

	   only 11N rate is adjusted */
...
//  RTMP_IO_WRITE32(pAd, MAC_CSR1, 0x4);
	...
	if (bHardReset == TRUE)
	{
		...
		{
			for (i = 0; i < HW_BEACON_OFFSET; i++)
				RTMP_IO_WRITE8(pAd, pAd->BeaconOffset[apidx] + i, 0x00); 
		}	
	}
...
//  RTMPSetPhyMode(pAd, PHY_11BG_MIXED);   // default in 11BG mixed mode
	...
	for(j = BSS0; j < pAd->ApCfg.BssidNum; j++)
	{
		...
		NdisZeroMemory(&pAd->ApCfg.MBSSID[j].PMKIDCache, sizeof(NDIS_802_11_PMKID));
	...
	pAd->ate.TxCount = 200;//TX_RING_SIZE;
...
//And LED mode is not supported in ATE mode
#ifdef RALINK_ATE
