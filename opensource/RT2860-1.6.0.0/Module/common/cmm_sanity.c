...
#ifdef BIG_ENDIAN
	*(USHORT *)(&pAddFrame->BaParm) = SWAP16(*(USHORT *)(&pAddFrame->BaParm));
	pAddFrame->TimeOutValue = SWAP16(pAddFrame->TimeOutValue);
	pAddFrame->BaStartSeq.word = SWAP16(pAddFrame->BaStartSeq.word); 
#endif
...
#ifdef BIG_ENDIAN
	*(USHORT *)(&pAddFrame->BaParm) = SWAP16(*(USHORT *)(&pAddFrame->BaParm));
	pAddFrame->StatusCode = SWAP16(pAddFrame->StatusCode);
	pAddFrame->TimeOutValue = SWAP16(pAddFrame->TimeOutValue);
#endif
	if (pAddFrame->BaParm.BAPolicy != IMMED_BA)
	{
...
#ifdef BIG_ENDIAN
	*(USHORT *)(&pDelFrame->DelbaParm) = SWAP16(*(USHORT *)(&pDelFrame->DelbaParm));
	pDelFrame->ReasonCode = SWAP16(pDelFrame->ReasonCode);
#endif
...
#ifdef BIG_ENDIAN
	pTimestamp->u.LowPart = SWAP32(pTimestamp->u.LowPart);
	pTimestamp->u.HighPart = SWAP32(pTimestamp->u.HighPart);
#endif
    Ptr += TIMESTAMP_LEN;
...
#ifdef BIG_ENDIAN
				*(USHORT *)(&pHtCapability->HtCapInfo) = SWAP16(*(USHORT *)(&pHtCapability->HtCapInfo));
				*(USHORT *)(&pHtCapability->ExtHtCapInfo) = SWAP16(*(USHORT *)(&pHtCapability->ExtHtCapInfo));
...
#ifdef BIG_ENDIAN
				*(USHORT *)(&AddHtInfo->AddHtInfo2) = SWAP16(*(USHORT *)(&AddHtInfo->AddHtInfo2));
				*(USHORT *)(&AddHtInfo->AddHtInfo3) = SWAP16(*(USHORT *)(&AddHtInfo->AddHtInfo3));
#endif // BIG_ENDIAN //               
			}
			else
			{
		...
		break;
            ...
            case IE_VENDOR_SPECIFIC:
				...
				*/
                ...
                {
#if 0 	// Let pVIE dedicate to wpa1 and wpa2 ie              
...
// QOS obselete in CCX2 TestPlan ver3.54
                if ((pAd->StaCfg.CCX2Control.field.QOSEnable) && (NdisEqualMemory(pEid->Octet, Ccx2QosInfo, 4)))
                {
                    ...
                    if (pAd->StaCfg.CCX2Control.field.Enable)
                    {
...
//                  pQosCapability->bMoreDataAck    = pEid->Octet[0] & 0x08;
    ...
    if ((pAd->CommonCfg.Channel > 14) && ((Sanity & 0x4) == 0))
#endif // CONFIG_AP_SUPPORT //
    {
#ifdef CONFIG_AP_SUPPORT
        *pChannel = pAd->CommonCfg.Channel;
#endif // CONFIG_AP_SUPPORT //
        Sanity |= 0x4;		
    }
...
#ifdef BIG_ENDIAN
	*((USHORT *)&EapolKeyInfo) = SWAP16(*((USHORT *)&EapolKeyInfo));
#endif // BIG_ENDIAN //
	...
	if (bReplayDiff)
	{
		...
		if (pAd->CommonCfg.bWirelessEvent)
			RTMPSendWirelessEvent(pAd, IW_REPLAY_COUNTER_DIFF_EVENT_FLAG, pEntry); 
	...
	if (MsgType != EAPOL_PAIR_MSG_1)
	{
        ...
        if (!NdisEqualMemory(pMIC, mic, LEN_KEY_DESC_MIC))
        {
			...
			if (pAd->CommonCfg.bWirelessEvent)
				RTMPSendWirelessEvent(pAd, IW_MIC_DIFF_EVENT_FLAG, pEntry); 
