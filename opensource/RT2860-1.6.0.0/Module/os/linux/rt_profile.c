...
#if 0
    //WscPinCode
	if(RTMPGetKeyParameter("ApCliWscPinCode", tmpbuf, 10, buffer))
	{
	...
	for (idx=0; idx<MAX_MBSSID_NUM; idx++) {
		...
		if(RTMPGetKeyParameter(tok_str, tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer))
		{
			for (i=0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++) 
			{
				...
				NdisMoveMemory(pAd->ApCfg.MBSSID[idx].AccessControlList.Entry[i].Addr, macAddress, ETH_LENGTH_OF_ADDRESS);				
			}
	...
	if (RTMPGetKeyParameter("RADIUS_Server", tmpbuf, 256, buffer))
	{
		for (i=0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_MBSSID_NUM); macptr = rstrtok(NULL,";"), i++) 
		{
			if (rtinet_aton(macptr, &ip_addr))
	     	{
	            pAd->ApCfg.MBSSID[i].radius_ip = ip_addr;  
				DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_ip=%s(%x)\n", i, macptr, pAd->ApCfg.MBSSID[i].radius_ip));
			}	    
		}
	}
	// RADIUS_Port
	if (RTMPGetKeyParameter("RADIUS_Port", tmpbuf, 32, buffer))
	{
		for (i=0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_MBSSID_NUM); macptr = rstrtok(NULL,";"), i++) 
		{	  
            pAd->ApCfg.MBSSID[i].radius_port = simple_strtol(macptr, 0, 10); 
			DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_port=%d\n", i, pAd->ApCfg.MBSSID[i].radius_port));
		}
	}
	// RADIUS_Key
	if (RTMPGetKeyParameter("RADIUS_Key", tmpbuf, 512, buffer))
	{
		if (strlen(tmpbuf) > 0)
			bUsePrevFormat = TRUE;
		...
		{	  
			if (strlen(macptr) > 0)
			{
				pAd->ApCfg.MBSSID[i].radius_key_len = strlen(macptr); 
				NdisMoveMemory(pAd->ApCfg.MBSSID[i].radius_key, macptr, strlen(macptr));
				DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_key=%s, len=%d\n", i, 
															pAd->ApCfg.MBSSID[i].radius_key, 
															pAd->ApCfg.MBSSID[i].radius_key_len));
			}
		}
	}
	...
	if (!bUsePrevFormat)
	{
		for (i = 0; i < MAX_MBSSID_NUM; i++)
		{
			...
			if (RTMPGetKeyParameter(tok_str, tmpbuf, 128, buffer))
			{
				if (strlen(tmpbuf) > 0)
				{
					pAd->ApCfg.MBSSID[i].radius_key_len = strlen(tmpbuf); 
					NdisMoveMemory(pAd->ApCfg.MBSSID[i].radius_key, tmpbuf, strlen(tmpbuf));
					DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), update radius_key=%s, len=%d\n", i, 
																pAd->ApCfg.MBSSID[i].radius_key, 
																pAd->ApCfg.MBSSID[i].radius_key_len));
				}	
				...
				else
				{
					...
					if (TRUE)
					{
						...
						for (i = 0; i < pAd->ApCfg.BssidNum; i++)
						{
							...
							if(RTMPGetKeyParameter(tok_str, tmpbuf, 33, buffer))
							{
						...
						if (bSSIDxIsUsed == FALSE)
						{
							if(RTMPGetKeyParameter("SSID", tmpbuf, 256, buffer))
							{			
...
//#endif
					...
					if(TRUE)
					{
						...
						for (i = 0; i < pAd->ApCfg.BssidNum; i++)
						{
							...
							if(RTMPGetKeyParameter(tok_str, tmpbuf, 65, buffer))
							{
	...
	IN	CHAR		  *pInput)
{
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		DBGPRINT(RT_DEBUG_TRACE, ("FixedTxMode = %d\n", Value));

		if (Value == FIXED_TXMODE_CCK || Value == FIXED_TXMODE_OFDM)
		{
			pAd->CommonCfg.FixedTxMode = Value;
		}	
		else 
		{
			pAd->CommonCfg.FixedTxMode  = FIXED_TXMODE_NONE;
			DBGPRINT(RT_DEBUG_WARN, ("Invalid FixedTxMode value (%d), ignored, Fix Tx Mode = NONE.\n", Value));
		}
	}
	...
	// MSC
	if (RTMPGetKeyParameter("HT_MCS", pValueStr, 25, pInput))
	{
		Value = simple_strtol(pValueStr, 0, 10);

		//if (Value >= 0 && Value < MCS_AUTO)
		if ((Value >= 0 && Value <= 15) || (Value == 32))
		{
			OPSTATUS_CLEAR_FLAG(pAd, fOP_STATUS_TX_RATE_SWITCH_ENABLED); 
			pAd->CommonCfg.RegTransmitSetting.field.MCS  = Value;
		}
		else
		{
