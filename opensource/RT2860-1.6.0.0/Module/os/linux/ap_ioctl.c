					...
					//assign the STATE_MACHINE type
					{
                        ...
                        if (pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscConfStatus == WSC_SCSTATE_UNCONFIGURED)
                        {
									 ...
									 !pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscUPnPNodeInfo.bUPnPInProgress)
                            {
                                ...
                                {
                                    WscGetConfForUpnp(pAd, &pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl);
	...
	IN	INT                 cmd)
{
    ...
    switch(cmd)
    {
		case RT_OID_VERSION_INFO:
			...
			sprintf(&driverVersion[0], "%s", DRIVER_VERSION);
			driverVersion[7] = '\0';
...
//        DBGPRINT(RT_DEBUG_INFO, ("rt2860_ioctl I/F(ra%d)(flags=%d): cmd = 0x%08x\n", pObj->ioctl_if, net_dev->priv_flags, cmd));
    }
...
//	    	        DBGPRINT(RT_DEBUG_INFO, ("rt2860_ioctl I/F(ra%d)(flags=%d): cmd = 0x%08x\n", index, net_dev->priv_flags, cmd));
	        ...
	        {
//	        	DBGPRINT(RT_DEBUG_ERROR, ("rt2860_ioctl can not find I/F\n"));
	            return -ENETDOWN;
	        }
	    }
	    else    // ioctl command from I/F(ra0)
	    {
	    	pAd= net_dev->priv;
    	    pObj->ioctl_if = MAIN_MBSSID;
//	        DBGPRINT(RT_DEBUG_ERROR, ("rt2860_ioctl can not find I/F and use default: cmd = 0x%08x\n", cmd));
	    }
	...
	else if (net_dev->priv_flags == INT_APCLI)
	{
		...
		for (index = 0; index < MAX_APCLI_NUM; index++)
		{
			if (pAd->ApCfg.ApCliTab[index].dev == net_dev)
			{
				...
				DBGPRINT(RT_DEBUG_INFO, ("rt2860_ioctl I/F(apcli%d)(flags=%x): cmd = 0x%08x\n", pObj->ioctl_if, net_dev->priv_flags, cmd));
				break;
			}
		...
		case SIOCGIWRATE:  //get default bit rate (bps)
            {
                ...
                if (pAd->CommonCfg.HTPhyMode.field.MODE >= MODE_HTMIX)
                {
                	rate_index = 16 + ((UCHAR)pAd->CommonCfg.HTPhyMode.field.BW *16) + ((UCHAR)pAd->CommonCfg.HTPhyMode.field.ShortGI *32) + ((UCHAR)pAd->CommonCfg.HTPhyMode.field.MCS);
                }
                else if (pAd->CommonCfg.HTPhyMode.field.MODE == MODE_OFDM)
                	rate_index = (UCHAR)(pAd->CommonCfg.HTPhyMode.field.MCS) + 4;
                else 
                	rate_index = (UCHAR)(pAd->CommonCfg.HTPhyMode.field.MCS);
                if (rate_index < 0)
                    rate_index = 0;
                if (rate_index > 255)
                    rate_index = 255;
	...
	IN	PUCHAR			arg)
{
	...
	// If out of range, set it to the first channel in current region.
	...
	IN	PUCHAR			arg)
{
	...
	{
		pAd->CommonCfg.FragmentThreshold = FragThresh;
	}
	else
		return FALSE; //Invalid argument 
	...
	IN	PUCHAR			arg)
{
	...
	printk("\n%-19s%-4s%-4s%-4s%-8s%-7s%-7s%-7s%-10s%-6s%-6s%-6s%-6s\n",
		   "MAC", "AID", "PSM", "WMM", "MIMOPS", "RSSI0", "RSSI1", "RSSI2", "PhMd", "BW", "MCS", "SGI", "STBC");
...
/* 
    ==========================================================================
    Description:
        It only shall be queried by 802.1x daemon.        
	Arguments:
	...
	IN struct iwreq *wrq)
{
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++)
	{
		...
		if (pAd->ApCfg.MBSSID[apidx].radius_ip != 0)
		{
			RadiusConf.RadiusInfo[apidx].radius_ip = pAd->ApCfg.MBSSID[apidx].radius_ip;
			RadiusConf.RadiusInfo[apidx].radius_port = pAd->ApCfg.MBSSID[apidx].radius_port;
			RadiusConf.RadiusInfo[apidx].radius_key_len = pAd->ApCfg.MBSSID[apidx].radius_key_len;
			if (RadiusConf.RadiusInfo[apidx].radius_key_len > 0)
				NdisMoveMemory(RadiusConf.RadiusInfo[apidx].radius_key, pAd->ApCfg.MBSSID[apidx].radius_key, pAd->ApCfg.MBSSID[apidx].radius_key_len);
		}
		...
		while ((this_char = strsep((char **)&ptr, ",")) != NULL)
		{
			...
			{ //Read
				...
				if (sscanf(this_char, "%d", &(bbpId)) == 1)
				{
					if (bbpId <= 136)
					{
						//In ATE mode, we do not load 8051 firmware.
		...
		while ((this_char = strsep((char **)&ptr, ",")) != NULL)
		{
			...
			{ //Read
				...
				if(strlen(this_char) == 4)
				{
					...
					if (macAddr < 0xFFFF)
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("MacAddr=%lx, MacValue=%lx\n", macAddr, macValue));
						sprintf(msg+strlen(msg), "[0x%08lX]:%08lX  ", macAddr , macValue);
						DBGPRINT(RT_DEBUG_INFO, ("msg=%s\n", msg));
					}
					else
					{//Invalid parametes, so default printk all bbp
			...
			{ //Write
...

				{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("MacAddr=%02lx, MacValue=0x%lx\n", macAddr, macValue));
...
static VOID RTMPIoctlGetMacTable(
	IN PRTMP_ADAPTER pAd, 
	IN struct iwreq *wrq)
{
	INT i;
...
#endif	// endif - move to cmm_info.c
/* 
    ==========================================================================
    Description:
	...
	IN  PWSC_CTRL       pWscControl)
{
    ...
    pWscControl->Wsc2MinsTimerRunning = TRUE;
    pWscControl->WscStatus = STATUS_WSC_LINK_UP;
	WscSendUPnPConfReqMsg(pAd, pAd->ApCfg.MBSSID[MAIN_MBSSID].Ssid, pAd->ApCfg.MBSSID[MAIN_MBSSID].Bssid, 3, 0);

    pWscControl->bWscTrigger = TRUE;
	DBGPRINT(RT_DEBUG_TRACE, ("WscGetConfForUpnp:: trigger WSC state machine\n"));
