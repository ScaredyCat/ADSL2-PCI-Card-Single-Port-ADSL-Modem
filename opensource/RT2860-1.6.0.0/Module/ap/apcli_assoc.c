	...
	OUT APCLI_STATE_MACHINE_FUNC Trans[]) 
{
	...
	ApCliStateMachineInit(S, (APCLI_STATE_MACHINE_FUNC*)Trans, APCLI_MAX_ASSOC_STATE, APCLI_MAX_ASSOC_MSG, (APCLI_STATE_MACHINE_FUNC)ApCliDrop, APCLI_ASSOC_IDLE, APCLI_ASSOC_MACHINE_BASE);

	// first column
	ApCliStateMachineSetAction(S, APCLI_ASSOC_IDLE, APCLI_MT2_MLME_ASSOC_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliMlmeAssocReqAction);
	ApCliStateMachineSetAction(S, APCLI_ASSOC_IDLE, APCLI_MT2_MLME_DISASSOC_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliMlmeDisassocReqAction);
	ApCliStateMachineSetAction(S, APCLI_ASSOC_IDLE, APCLI_MT2_PEER_DISASSOC_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliPeerDisassocAction);
   
	// second column
	ApCliStateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_MLME_ASSOC_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliInvalidStateWhenAssoc);
	ApCliStateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_MLME_DISASSOC_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliInvalidStateWhenDisassociate);
	ApCliStateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_PEER_DISASSOC_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliPeerDisassocAction);
	ApCliStateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_PEER_ASSOC_RSP, (APCLI_STATE_MACHINE_FUNC)ApCliPeerAssocRspAction);
	ApCliStateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_ASSOC_TIMEOUT, (APCLI_STATE_MACHINE_FUNC)ApCliAssocTimeoutAction);
	...
	IN PVOID SystemSpecific3)
{
	...
	ApCliMlmeEnqueue(pAd, APCLI_ASSOC_STATE_MACHINE, APCLI_MT2_ASSOC_TIMEOUT, 0, NULL, 0);
    MlmeHandler(pAd);
	...
	
{
	...
	if (pAd->ApCfg.ApCliTab[ifIndex].bBlockAssoc == TRUE)
	{
		...
		*pCurrState = APCLI_ASSOC_IDLE;
		...
		ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_ASSOC_RSP,
...
#if 0 /* the field of SupportedHtPhy.bPreNHt never used. */
	...
	IN USHORT ifIndex) 
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) 
	{
		...
		*pCurrState = APCLI_ASSOC_IDLE;
		...
		ApCliCtrlMsg.Status = MLME_FAIL_NO_RESOURCE;
		ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_DEASSOC_RSP,
			sizeof(APCLI_CTRL_MSG_STRUCT), &ApCliCtrlMsg, ifIndex);
		return;
	}
	...
	if(PeerDisassocSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Reason))
	{
		if (MAC_ADDR_EQUAL(pAd->MlmeAux.Bssid, Addr2))
		{
			*pCurrState = APCLI_ASSOC_IDLE;
			...
			ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_PEER_DISCONNECT_REQ, 0, NULL, ifIndex);
	...
	IN USHORT ifIndex) 
{
	...
	*pCurrState = APCLI_ASSOC_IDLE;
	...
	ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_ASSOC_REQ_TIMEOUT, 0, NULL, ifIndex);
