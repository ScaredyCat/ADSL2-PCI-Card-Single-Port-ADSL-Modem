 ...
 ***************************************************************************
    ...
    Note:
    1. Call RT2860_ApCli_Init() in init function and
       call RT2860_ApCli_Remove() in close function

    2. MAC of ApCli-interface is initialized in RT2860_ApCli_Init()
...
#define APCLI_MAX_DEV_NUM	32
	...
	for(apcli_index = 0; apcli_index < MAX_APCLI_NUM; apcli_index++)
	{
		...
		ad_p->ApCfg.ApCliTab[apcli_index].CurrentAddress[ETH_LENGTH_OF_ADDRESS - 1] =
			(ad_p->ApCfg.ApCliTab[apcli_index].CurrentAddress[ETH_LENGTH_OF_ADDRESS - 1] + ad_p->ApCfg.BssidNum) & 0xFF;
	...
	IN	PNET_DEV	dev_p)
{
	...
	for (ifIndex = 0; ifIndex < MAX_APCLI_NUM; ifIndex++)
	{
		if (ad_p->ApCfg.ApCliTab[ifIndex].dev == dev_p)
		{
			...
			{
				ApCliMlmeEnqueue(ad_p, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_DISCONNECT_REQ, 0, NULL, ifIndex);
				MlmeHandler(ad_p);
	...
	IN PNET_DEV			dev_p)
{
	...
	for(apcliIndex = 0; apcliIndex < MAX_APCLI_NUM; apcliIndex++)
	{
		...
		if (pApCli[apcliIndex].dev == dev_p)
		{
			...
			/* transmit the packet */
			return rt2860_packet_xmit(RTPKT_TO_OSPKT(skb_p));
		}
    }
	...
	IN INT 					cmd)
{
	...
	/* do real IOCTL */
	return rt2860_ioctl(dev_p, rq_p, cmd);
	...
	do
	{
		...
		if (pMacEntry)
		{
			...
			{			
				...
				BssIdx = pAd->ApCfg.BssidNum + ifIndex;
			...
			if (pAd->MlmeAux.HtCapabilityLen != 0)
			{
				...
				if (pAd->CommonCfg.RegTransmitSetting.field.MCS != MCS_AUTO)
				{
					...
					else if (pMacEntry->MaxHTPhyMode.field.MCS > pAd->CommonCfg.HTPhyMode.field.MCS)
					{
						// STA supports fixed MCS 
						pMacEntry->MaxHTPhyMode.field.MCS = pAd->CommonCfg.HTPhyMode.field.MCS;
					}
				}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for(ifIndex = 0; ifIndex < MAX_APCLI_NUM; ifIndex++)
	{
			...
			&& (pApCliEntry->Valid == FALSE))
		{
			...
			ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_JOIN_REQ, 0, NULL, ifIndex);
		}
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for(ifIndex = 0; ifIndex < MAX_APCLI_NUM; ifIndex++)
	{
		...
		{
			DBGPRINT(RT_DEBUG_TRACE, ("(%s) ApCli interface[%d] startdown.\n", __FUNCTION__, ifIndex));
			ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_DISCONNECT_REQ, 0, NULL, ifIndex);
		}
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for(index = 0; index < MAX_APCLI_NUM; index++)
	{
			...
			&& (RTMP_TIME_AFTER(pAd->Mlme.Now32 , (pApCliEntry->ApCliRcvBeaconTime + (4 * OS_HZ)))))
		{
			...
			ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_DISCONNECT_REQ, 0, NULL, index);
			MlmeHandler(pAd);
		}
	}
    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
	...
	GenRandom(pAd, pAd->ApCfg.ApCliTab[IfIndex].SNonce, (IfIndex + MIN_NET_DEVICE_FOR_APCLI));  
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	{											
		...
		if(pApCliEntry->GroupCipher == Ndis802_11WEPEnabled)
        {
        	UCHAR *pPayload = (UCHAR *)pRxBlk->pData + LENGTH_802_11;
            	
            if (RTMPDecryptData(pAd, pPayload, (pRxWI->MPDUtotalByteCount - LENGTH_802_11), pEntry->MatchAPCLITabIdx) == FALSE)
   			{
				printk("ERROR : Software decrypt WEP data fails.\n");	
				return FALSE; // give up this frame
			}
            else 
				pRxWI->MPDUtotalByteCount -= 8;  //Minus IV[4] & ICV[4]

			DBGPRINT(RT_DEBUG_INFO, ("RTMPDecryptData WEP data Complete \n"));							
		}
		// handle TKIP decryption
		else if(pApCliEntry->GroupCipher == Ndis802_11Encryption2Enabled)
		{	
			if (RTMPSoftDecryptTKIP(pAd, pRxBlk->pData, pRxWI->MPDUtotalByteCount, 0, pApCliEntry->SharedKey))
			{
				DBGPRINT(RT_DEBUG_INFO, ("RTMPSoftDecryptTKIP Complete \n"));							
				pRxWI->MPDUtotalByteCount -= 20;  //Minus 8 bytes MIC, 8 bytes IV/EIV, 4 bytes ICV
			}
            else
			{
		...
		{
			if (RTMPSoftDecryptAES(pAd, pRxBlk->pData, pRxWI->MPDUtotalByteCount , pApCliEntry->SharedKey))
			{							
				DBGPRINT(RT_DEBUG_INFO, ("RTMPSoftDecryptAES Complete \n"));								
				pRxWI->MPDUtotalByteCount -= 16;  //8 bytes MIC, 8 bytes IV/EIV (CCMP Header)
			}
			else
			{
