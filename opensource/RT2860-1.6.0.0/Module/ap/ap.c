...
//	GenRandom(pAd,pAd->ApCfg.GMK);
//	GenRandom(pAd,pAd->ApCfg.GNonce);
//	CountGTK(pAd->ApCfg.GMK, (UCHAR*)pAd->ApCfg.GNonce, pAd->CurrentAddress, GTK, TKIP_GTK_LENGTH);
//	NdisMoveMemory(pAd->SharedKey[BSS0][ApCfg.MBSSID[apidx].DefaultKeyId].Key, GTK, 16);
//	NdisMoveMemory(pAd->SharedKey[BSS0][ApCfg.MBSSID[apidx].DefaultKeyId].TxMic, &GTK[16], 8);
//	NdisMoveMemory(pAd->SharedKey[BSS0][ApCfg.MBSSID[apidx].DefaultKeyId].RxMic, &GTK[24], 8);            
...
//			pAd->ApCfg.MBSSID[apidx].CapabilityInfo |= 0x0200;
	...
	RTMPPrepareRadarDetectParams(pAd);
		...
		for (i=1; i<255; i++)
		{
			...
			for (j=0; j<HW_WCID_ENTRY_SIZE; j++)
			{
				RTMP_IO_WRITE8(pAd, offset+j, 0);
			}
		}
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++)
	{
		...
		// Init TKIP Group-Key-related variables
		GenRandom(pAd, pAd->ApCfg.MBSSID[apidx].GMK, apidx);
		GenRandom(pAd, pAd->ApCfg.MBSSID[apidx].GNonce, apidx);
	...
	AsicUpdateProtect(pAd, 0,  ALLN_SETPROTECT, FALSE, FALSE);
	...
	IN PUCHAR pAddr) 
{
	...
	if (pEntry && (pEntry->ValidAsCLI || pEntry->ValidAsApCli || pEntry->ValidAsWDS))
	{
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr))
		{
			...
			AsicRemovePairwiseKeyEntry(pAd, BSS0, (UCHAR)wcid);
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) 
	{
		...
		if (pEntry->NoDataIdleCount >= MAC_TABLE_AGEOUT_TIME)
		{
			...
			if (pAd->CommonCfg.bWirelessEvent)
				RTMPSendWirelessEvent(pAd, IW_AGEOUT_EVENT_FLAG, pEntry); 
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	//      if (pAd->Counters8023.GoodReceives)
			...
			DBGPRINT(RT_DEBUG_TRACE,("Msleep at ch#%d to collect RX=%lu, RSSI=%d, CRC error =%ld, False CCA =%ld\n", 
		...
		for (dirty = 10; dirty <= 12; dirty++)
		{
			...
			if (candidate_num)
			{
				for (ch = 1; ch <= pAd->ChannelListNum; ch ++)
				{
