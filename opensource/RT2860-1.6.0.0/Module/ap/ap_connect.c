...
//	for(i=0; i<MAX_MBSSID_NUM; i++)
	...
	{
		for (j=0; j<TXWI_SIZE; j++)  // 16-bytes TXINFO field
	    {
	        RTMP_IO_WRITE8(pAd, pAd->BeaconOffset[i] + j, 0);
	    }
	}
...
#ifdef APCLI_SUPPORT
	// Note: AP-Client use one isolate MAC address.
	//       increase number of MAC usage while AP-Client function including.
	NumOfMacs = pAd->ApCfg.BssidNum + MAX_APCLI_NUM;
#else
	NumOfMacs = pAd->ApCfg.BssidNum;
#endif
...
//#endif /* MBSS_SUPPORT */
	...
	if (pAd->ApCfg.BssidNum > 1)
		regValue |= ((pAd->ApCfg.BssidNum - 1)<<18);	
