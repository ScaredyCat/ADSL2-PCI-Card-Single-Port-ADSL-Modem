    ...
    IN MAC_TABLE_ENTRY  *pEntry) 
{
	...
	if (pAd->CommonCfg.bWirelessEvent)
		RTMPSendWirelessEvent(pAd, IW_MIC_ERROR_EVENT_FLAG, pEntry); 
    ...
    if (pAd->ApCfg.CMTimerRunning == TRUE)
    {
		...
		if (pAd->CommonCfg.bWirelessEvent)
			RTMPSendWirelessEvent(pAd, IW_COUNTER_MEASURES_EVENT_FLAG, pEntry); 
		
        // renew GTK
        GenRandom(pAd, pAd->ApCfg.MBSSID[pEntry->apidx].GNonce, pEntry->apidx);
...
#ifdef BIG_ENDIAN
	*((USHORT *)&peerKeyInfo) = SWAP16(*((USHORT *)&peerKeyInfo));
#endif // BIG_ENDIAN //
    ...
    IN ULONG			TimeInterval) 
{
    ...
    {       
		...
		GenRandom(pAd, pEntry->ANonce, apidx);  
    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
    ...
    do
    {
		...
		if (pEntry->AuthMode == Ndis802_11AuthModeWPA2 || pEntry->AuthMode == Ndis802_11AuthModeWPA2PSK)
		{
			...
			if (pAd->CommonCfg.bWirelessEvent)
				RTMPSendWirelessEvent(pAd, IW_SET_KEY_DONE_WPA2_EVENT_FLAG, pEntry); 
    ...
    IN UINT             MsgLen) 
{
    ...
    do
    {
		...
		else
		{
			...
			if (pAd->CommonCfg.bWirelessEvent)
				RTMPSendWirelessEvent(pAd, IW_SET_KEY_DONE_WPA1_EVENT_FLAG, pEntry); 
    ...
    IN PVOID SystemSpecific3) 
{
	...
	if ((pEntry) && (pEntry->ValidAsCLI == TRUE) && (pEntry->EnqueueStartForPSKTimerRunning == TRUE))
	{
        ...
        switch (pEntry->AuthMode)
        {
            ...
            case Ndis802_11AuthModeWPA2PSK:
                ...
                {    
					...
					if (pAd->CommonCfg.bWirelessEvent)
						RTMPSendWirelessEvent(pAd, IW_GROUP_HS_TIMEOUT_EVENT_FLAG, pEntry); 
                ...
                else if (pEntry->ReTryCounter > (PEER_MSG1_RETRY_TIMER_CTR + 3))
                {
					...
					if (pAd->CommonCfg.bWirelessEvent)
						RTMPSendWirelessEvent(pAd, IW_PAIRWISE_HS_TIMEOUT_EVENT_FLAG, pEntry);
    ...
    IN USHORT           Reason)
{
    ...
    if (pEntry)
    {
		...
		if (pAd->CommonCfg.bWirelessEvent)
			RTMPSendWirelessEvent(pAd, IW_DISASSOC_EVENT_FLAG, pEntry); 
