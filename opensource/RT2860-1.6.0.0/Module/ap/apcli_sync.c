	...
	OUT APCLI_STATE_MACHINE_FUNC Trans[])
{
	...
	ApCliStateMachineInit(Sm, (APCLI_STATE_MACHINE_FUNC*)Trans, APCLI_MAX_SYNC_STATE, APCLI_MAX_SYNC_MSG, (APCLI_STATE_MACHINE_FUNC)ApCliDrop, APCLI_SYNC_IDLE, APCLI_SYNC_MACHINE_BASE);

	// column 1
	ApCliStateMachineSetAction(Sm, APCLI_SYNC_IDLE, APCLI_MT2_MLME_PROBE_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliMlmeProbeReqAction);

	//column 2
	ApCliStateMachineSetAction(Sm, APCLI_JOIN_WAIT_PROBE_RSP, APCLI_MT2_MLME_PROBE_REQ, (APCLI_STATE_MACHINE_FUNC)ApCliInvalidStateWhenJoin);
	ApCliStateMachineSetAction(Sm, APCLI_JOIN_WAIT_PROBE_RSP, APCLI_MT2_PEER_PROBE_RSP, (APCLI_STATE_MACHINE_FUNC)ApCliPeerProbeRspAtJoinAction);
	ApCliStateMachineSetAction(Sm, APCLI_JOIN_WAIT_PROBE_RSP, APCLI_MT2_PROBE_TIMEOUT, (APCLI_STATE_MACHINE_FUNC)ApCliProbeTimeoutAtJoinAction);
	...
	IN PVOID SystemSpecific3)
{
	...
	ApCliMlmeEnqueue(pAd, APCLI_SYNC_STATE_MACHINE, APCLI_MT2_PROBE_TIMEOUT, 0, NULL, 0);
	MlmeHandler(pAd);
	...
	IN USHORT ifIndex)
{
	...
	ApCliEnqueueProbeRequest(pAd, Info->SsidLen, Info->Ssid, ifIndex);
...
//	UINT FrameLen = 0;
	...
	UCHAR AddHtInfoLen;
	UCHAR NewChannel, CentralChannel;
		...
		if (matchFlag)
		{
			...
			if(bssidEqualFlag == TRUE)
			{
				*pCurrState = APCLI_SYNC_IDLE;
				...
				ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_PROBE_RSP,
	...
	IN USHORT ifIndex) 
{
	...
	*pCurrState = SYNC_IDLE;
	...
	{
		ApCliMlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_JOIN_REQ_TIMEOUT, 0, NULL, ifIndex);
	}
