    ...
    OUT HT_CAPABILITY_IE *pHtCapability)
{
    ...
    *pHtCapabilityLen= 0;	
                ...
                if ((eid_ptr->Len <= MAX_LEN_OF_RSNIE) && (eid_ptr->Len > MIN_LEN_OF_RSNIE))
                {
                    ...
                    if (!RTMPCheckMcast(pAd, eid_ptr, pEntry))
                    {
						...
						if (pAd->CommonCfg.bWirelessEvent)
							RTMPSendWirelessEvent(pAd, IW_RSNIE_SANITY_FAIL_EVENT_FLAG, pEntry); 
    ...
    {
        DBGPRINT(RT_DEBUG_WARN, ("PeerAssocReqSanity - success\n"));
        return TRUE;
    }
}
                ...
                if (be2cpu32(*(UINT *)&eid_ptr->Octet) == WSC_OUI)
                {
    				...
    				{	
    					...
    					if (32 >= *SsidLen)	//Well, I think that it must be TRUE!
    					{
    						...
    						WscSendUPnPMessage(pAd, WSC_OPCODE_UPNP_MGMT, WSC_UPNP_MGMT_SUB_PROBE_REQ, pBuf, bufLen, 0, 0, &Fr->Hdr.Addr2);
                		}
                        if (pBuf)
                            kfree(pBuf);
                		break;
