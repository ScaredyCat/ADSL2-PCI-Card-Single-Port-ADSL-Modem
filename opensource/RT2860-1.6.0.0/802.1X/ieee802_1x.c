...
static void ieee802_1x_send(rtapd *rtapd, struct sta_info *sta, u8 type, u8 *data, size_t datalen)
{
	...
	{
        if (RT_ioctl(rtapd->ioctl_sock, RTPRIV_IOCTL_RADIUS_DATA, buf, len, sta->ApIdx, 0))
...
//			  Ap_free_sta(rtapd, sta);
		...
		case 1:
			...
			if( sta->eapol_sm->authSuccess && sta->eapol_key_sign_len == 0 && sta->eapol_key_crypt_len == 0 )
			{
				...
				if (RT_ioctl(rtapd->ioctl_sock, RTPRIV_IOCTL_STATIC_WEP_COPY, (char *)&MacAddr, sizeof(MacAddr), sta->ApIdx, 0))
				{				   
...
void ieee802_1x_tx_key(rtapd *rtapd, struct sta_info *sta, u8 id)
{
	...
	if (sta->ethertype== ETH_P_PRE_AUTH)
	{
		if (RT_ioctl(rtapd->ioctl_sock, RTPRIV_IOCTL_ADD_PMKID_CACHE, (char *)&WepKey, sizeof(NDIS_802_11_KEY), 0, 0))
		{
	...
	{
		if (RT_ioctl(rtapd->ioctl_sock, RTPRIV_IOCTL_ADD_WPA_KEY, (char *)&WepKey, sizeof(NDIS_802_11_KEY), sta->ApIdx, 0))
		{
...
void ieee802_1x_receive(rtapd *rtapd, u8 *sa, u8 *apidx, u8 *buf, size_t len, u16 ethertype)
{
	...
	switch (hdr->type)
	{
		case IEEE802_1X_TYPE_EAP_PACKET:
			DBGPRINT(RT_DEBUG_TRACE,"Handle EAP_PACKET from ra%d\n",sta->ApIdx);
			handle_eap(sta, (buf + LENGTH_8021X_HDR), datalen);
			break;

		case IEEE802_1X_TYPE_EAPOL_START:
			DBGPRINT(RT_DEBUG_TRACE,"Handle EAPOL_START from ra%d\n",sta->ApIdx);
