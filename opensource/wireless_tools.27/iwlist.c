		...
		int		count)		/* Args count */
{
  ...
  if(iw_get_range_info(skfd, ifname, &range) < 0)
      ...
      if(iw_get_ext(skfd, ifname, SIOCGIWFREQ, &wrq) >= 0)
	{
	  ...
	  iw_print_freq(buffer, sizeof(buffer), freq, wrq.u.freq.pad);
		   ...
		   int		count)		/* Args count */
{
  ...
  if(iw_get_range_info(skfd, ifname, &range) < 0)
	...
	printf("%-8.16s  No bit-rates ? Please update driver...\n", ifname);
		   ...
		   int		count)		/* Args count */
{
  ...
  if((iw_get_range_info(skfd, ifname, &range) < 0) ||
	...
	printf("%-8.16s  No transmit-powers ? Please update driver...\n\n", ifname);
      else
	{
		     ...
		     int		has_range)
{
  ...
  switch(event->cmd)
	...
	iw_print_freq(buffer, sizeof(buffer), freq, event->u.freq.pad);
	printf("                    %s\n", buffer);
      }
      break;
    case SIOCGIWMODE:
