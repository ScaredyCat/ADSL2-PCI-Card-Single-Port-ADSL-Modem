  ...
  if (mfs->filename)
    {
      ...
      if (stat (mfs->filename, &statbuf) == 0)
        {
          /* mfs->ptr = calloc (statbuf.st_size, 1); */
		  mfs->ptr = calloc (statbuf.st_size, sizeof(char));
          if (mfs->ptr)
            {
              FILE *fp;
              mfs->capacity = statbuf.st_size;
              ...
              if (fp)
                {
                  /* size_t r = fread (mfs->ptr, mfs->size, 1, fp); */
				  size_t r = fread (mfs->ptr, 1, mfs->size, fp);
                  if (r != mfs->size)
                    status = EIO;
                  fclose (fp);
                }
...
CCLAPI RCODE	mimeMemStrmNew(stream_t *stream, const char *filename, int flags)
{
  ...
  stream_set_open (*stream, _memory_open1, mfs);
  ...
  stream_set_flush (*stream, _memory_flush, mfs);
