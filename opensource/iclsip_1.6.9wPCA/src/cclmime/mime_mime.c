...
struct mimeMimeObj
{
	...
	int					tparts;
	int					nmtp_parts;		
	struct _mimeMimePartObj	**mtp_parts;		/* list of parts in the msg */
	char 				*boundary;
	int					cur_offset;
	int					cur_part;
	int					part_offset;
...
static int _mime_parse_mpart_message(MimeMime mime);
static int _mime_append_part(MimeMime mime, MimeMsg msg, int offset, int len, int lines);
static char* _mime_get_param(char *field_body, const char *param, int *len);
static int _mime_setup_buffers(MimeMime mime);
static void _mime_append_header_line(MimeMime mime);
static int _mime_is_multipart_digest(MimeMime mime);
...
static int _mime_set_content_type(MimeMime mime);


CCLAPI RCODE	mimeMimeNew(MimeMime *pmime, MimeMsg msg, int flags)
{
	MimeMime 	mime = NULL;
	int 	ret = 0;
	size_t 	size;
	...
	if ( msg )  {
		if ( ( ret = message_get_header(msg, &(mime->hdrs)) ) == 0 ) {
			...
			if  (ret == 0 ) {
				...
				mimeMsgGetBdy(msg, &body);
				/* body_get_stream(body, &(mime->stream)); */
				mimeBdyGetStrm(body, &(mime->stream));
			}
		}
	}
	else {
...
CCLAPI void		mimeMimeFree(MimeMime *pmime)
{
	MimeMime mime;
...
CCLAPI RCODE	mimeMimeGetNumParts(MimeMime mime, int *nmtp_parts)
{
	int 		ret = 0;
...
CCLAPI 
RCODE	mimeMimeGetPart(MimeMime mime, size_t part, MimeMsg *msg)
{
	size_t 				nmtp_parts;
	int					ret = 0, flags = 0;
	stream_t			stream;
	body_t				body;
	/* struct _mimeMimePartObj	*mime_part; */
	struct _mime_part	*mime_part;

	/* if ( ( ret = mime_get_num_parts(mime, &nmtp_parts ) ) == 0 ) { */
	if ( ( ret = mimeMimeGetNumParts(mime, &nmtp_parts ) ) == 0 ) {
...
CCLAPI RCODE	mimeMimeAddPart(MimeMime mime, MimeMsg msg)
{
	int ret;
...
CCLAPI RCODE	mimeMimeGetMsg(MimeMime mime, MimeMsg *msg)
{
...
CCLAPI RCODE	mimeMimeSetBoundary(MimeMime mime, char *bdry)
{
	if(bdry!=NULL){
		if(mime->boundary)
			free(mime->boundary);
		if((mime->boundary=calloc(1, strlen(bdry)+1))==NULL)
			return ENOMEM;
		strcpy(mime->boundary, bdry);
		mime->boundary_len=strlen(bdry);
	}else{
		return EINVAL;
	}
	return 0;
...
CCLAPI RCODE	mimeMimeGetBoundary(MimeMime mime, char *bdry)
{
	if(mime->boundary && bdry!=NULL)
		strcpy(bdry, mime->boundary);
	else{
		return EINVAL;
	}
	return 0;
...
CCLAPI RCODE	mimeMimeGetBoundaryLen(MimeMime mime, int *len)
{
	if(mime->boundary && len!=NULL)
		*len=(int) strlen(mime->boundary);
	else return EINVAL;
