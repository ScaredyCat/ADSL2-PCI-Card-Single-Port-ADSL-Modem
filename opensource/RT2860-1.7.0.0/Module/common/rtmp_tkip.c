	...
	IN	UCHAR			apidx)
{
	...
	if (apidx >= MIN_NET_DEVICE_FOR_APCLI && ((apidx - MIN_NET_DEVICE_FOR_APCLI) < MAX_APCLI_NUM))
	...
	if ((RTMP_GET_PACKET_WCID(pPacket) != MCAST_WCID) && (*pSrc & 0x01))
	{
		RTMPInitMICEngine(
			pAd,
			pKey->Key,
...
#if 0	// No need handle next packet			
		// Next descriptor			
		RTMP_QueryNextPacketInfo(&pPacket, &PacketInfo, &pSrcBufVA, &SrcBufLen);
