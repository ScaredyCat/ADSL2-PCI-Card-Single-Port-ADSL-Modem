	...
	IN PNDIS_PACKET  pPacket)
{
	...
	// Currently, our fragment only support when a unicast packet send as NOT A-RALINK, NOT A-MSDU, NOT 11n mode.
	...
	IN TX_BLK *pTxBlk)
{
	...
	else
	{
...
#ifdef CONFIG_AP_SUPPORT
#ifdef WDS_SUPPORT
		if(pTxBlk->pMacEntry->ValidAsWDS == TRUE)
			TX_BLK_SET_FLAG(pTxBlk, fTX_bWDSEntry);
#endif // WDS_SUPPORT //
#ifdef APCLI_SUPPORT
		if(pTxBlk->pMacEntry->ValidAsApCli == TRUE)
		{
			PNDIS_PACKET 	apCliPkt = NULL;
			
			// For each tx packet, update our MAT convert engine databases.
			apCliPkt = (PNDIS_PACKET)MATEngineTxHandle(pAd, pPacket, pTxBlk->pMacEntry->MatchAPCLITabIdx);
			if(apCliPkt)
			{
			...
			pTxBlk->pApCliEntry = &pAd->ApCfg.ApCliTab[pTxBlk->pMacEntry->MatchAPCLITabIdx];
		...
		if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED) && CLIENT_STATUS_TEST_FLAG(pTxBlk->pMacEntry, fCLIENT_STATUS_WMM_CAPABLE))
		...
		{
			if (RTMP_GET_PACKET_LOWRATE(pPacket) ||
                ((pAd->OpMode == OPMODE_AP) && (pTxBlk->pMacEntry->MaxHTPhyMode.field.MODE == MODE_CCK) && (pTxBlk->pMacEntry->MaxHTPhyMode.field.MCS == RATE_1)))
			{	// Specific packet, i.e., bDHCPFrame, bEAPOLFrame, need force low rate.
				...
				if (IS_HT_STA(pTxBlk->pMacEntry) && 
					...
					((pAd->CommonCfg.bRdg == TRUE) && CLIENT_STATUS_TEST_FLAG(pTxBlk->pMacEntry, fCLIENT_STATUS_RDG_CAPABLE)))
				{
					TX_BLK_CLEAR_FLAG(pTxBlk, fTX_bWMM);
			...
			if ((IS_HT_RATE(pTxBlk->pMacEntry) == FALSE) && 
				(CLIENT_STATUS_TEST_FLAG(pTxBlk->pMacEntry, fCLIENT_STATUS_PIGGYBACK_CAPABLE)))
...
#ifdef CONFIG_AP_SUPPORT
	if (MAC_ADDR_EQUAL(GET_OS_PKT_DATAPTR(pTxBlk->pPacket), GET_OS_PKT_DATAPTR(pPacket))) // unicast to same STA
		return TRUE;
	else
#endif // CONFIG_AP_SUPPORT //
		return FALSE;
...
//		RTMP_IRQ_LOCK(&pAd->irq_lock, IrqFlags);
		...
		if ((pAd->ate.Mode != ATE_STOP) && (pAd->ate.bQATxStart == TRUE))
		{
			if (pAd->ate.QID == QueIdx)
			{
				...
				{
					DBGPRINT(RT_DEBUG_OFF,("all Tx is done\n"));
					// Tx status enters idle mode.
					pAd->ate.TxStatus = 0;
				}
				else if (!(pAd->ate.Mode & ATE_TXFRAME))
				{ 
					/* not complete sending yet, but someone press the Stop TX bottom. */
					DBGPRINT(RT_DEBUG_OFF,("not complete sending yet, but someone pressed the Stop TX bottom\n"));
				}
				else
				{
...
{
	UINT32		Ac0Base;
	UINT32		Ac0HwIdx, Ac0SwIdx, AC0freeIdx;
//	PULONG	 pTxD;
	int			i;
	UINT32	*ptemp;
//	PRXD_STRUC		pRxD;
