...
//#endif // WIFI_TEST //
	...
	for (i=0; i<5; i++)
	{
		RT28xx_EEPROM_READ16(pAd, EEPROM_TXPOWER_BYRATE + i*4, value);
		data = value;
		...
		RT28xx_EEPROM_READ16(pAd, EEPROM_TXPOWER_BYRATE + i*4 + 2, value);
		if (bApwrdeltaMinus == FALSE)
		{
			t1 = (value&0xf)+(Apwrdelta);
			if (t1 > 0xf)
...
//#endif // WIFI_TEST //
		...
		if (data != 0xffffffff)
			RTMP_IO_WRITE32(pAd, TX_PWR_CFG_0 + i*4, data);
		DBGPRINT_RAW(RT_DEBUG_TRACE, ("%x,  Adata40M = %lx,  Gdata40M = %lx \n", data, Adata40M, Gdata40M));
	}
#endif
	DBGPRINT(RT_DEBUG_TRACE, ("\n"));
