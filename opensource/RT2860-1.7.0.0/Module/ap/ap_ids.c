 ...
 ****************************************************************************
     ...
     Who         When          What
     --------    ----------    ----------------------------------------------
     Albert Yang 11-02-2007    created for RT2860
	...
	IN USHORT			type)
{
	...
	switch (type)
	{
		case SUBTYPE_ASSOC_REQ:
			...
			//printk("RcvdAssocReqCount=%d\n", pAd->ApCfg.RcvdAssocReqCount);
			break;

		case SUBTYPE_REASSOC_REQ:
          	pAd->ApCfg.RcvdReassocReqCount ++;
			//printk("RcvdReassocReqCount=%d\n", pAd->ApCfg.RcvdReassocReqCount);
			break;

		case SUBTYPE_PROBE_REQ:
          	pAd->ApCfg.RcvdProbeReqCount ++;
			//printk("RcvdProbeReqCount=%d\n", pAd->ApCfg.RcvdProbeReqCount);
			break;	

		case SUBTYPE_DISASSOC:
          	pAd->ApCfg.RcvdDisassocCount ++;
			//printk("RcvdDisassocCount=%d\n", pAd->ApCfg.RcvdDisassocCount);
			break;

		case SUBTYPE_DEAUTH:
          	pAd->ApCfg.RcvdDeauthCount ++;
			//printk("RcvdDeauthCount=%d\n", pAd->ApCfg.RcvdDeauthCount);
			break;

		case SUBTYPE_AUTH:
          	pAd->ApCfg.RcvdAuthCount ++;
			//printk("RcvdAuthCount=%d\n", pAd->ApCfg.RcvdAuthCount);
			break;	
...
{	
	//IdsEnable
	if(RTMPGetKeyParameter("IdsEnable", tmpbuf, 10, buffer))
	{						
	...
	if(RTMPGetKeyParameter("EapReqFooldThreshold", tmpbuf, 10, buffer))
	{						
