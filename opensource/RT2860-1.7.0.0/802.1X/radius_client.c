...
static void Radius_client_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	{	
		...
		{
			DBGPRINT(RT_DEBUG_WARN, "Radius_client_timer : ready to change RADIUS server for ra%d\n", i);		
...
{		
	...
	{
		DBGPRINT(RT_DEBUG_WARN, "Radius_client_init_auth: can't create auth RADIUS socket for ra%d (it's invalid IP)\n", apidx);
		return -1;
	}
