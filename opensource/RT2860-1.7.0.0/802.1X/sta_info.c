...
struct sta_info* Ap_get_sta(rtapd *apd, u8 *sa, u8 *apidx, u16 ethertype, int stop)
{
	...
	if (s == NULL)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE,"Create a new STA(in ra%d)\n", s->ApIdx);
	...
	else
	{
		...
		DBGPRINT(RT_DEBUG_TRACE,"A STA has existed(in ra%d)\n", s->ApIdx);
	}	
...
void Ap_handle_session_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (RT_ioctl(apd->ioctl_sock, RTPRIV_IOCTL_RADIUS_DATA, buf, len, sta->ApIdx, 0))
	{
